...
void WaterfallWidget::addLine(const QVector<quint8> &line, double minHz, double maxHz)
{
    ...
    // Previously FFT rows continued to accumulate in m_pendingGpuRows and, on
    // restore, were drained several rows per video frame. That replayed many
    // seconds of FT history in a few hundred milliseconds and made normal FT
    // tone changes look like broad comb/squiggle artefacts. The waterfall is
    // display-only, so discard presentation rows while hidden/minimized and
    // restart the GL history cleanly on the first visible row. Decoder audio is
    // untouched.
    ...
    if (presentationSuppressed) {
        if (!m_presentationSuspended) {
            ...
            emit runtimeDiagnostic(QStringLiteral(
                "Waterfall presentation suspended: window hidden/minimized; display-only FFT rows are discarded until restore"));
        }
        return;
    }
    if (m_presentationSuspended) {
        ...
        m_gpuWriteRow = 0;
        emit runtimeDiagnostic(QStringLiteral(
            "Waterfall presentation resumed: GL history reset after hidden/minimized window"));
    }
