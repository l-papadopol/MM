...
void RttyScopeWidget::paintEvent(QPaintEvent *event)
{
    ...
    // Logical Mark/Space captions follow the resolved polarity.  The underlying
    // X/Y trace still represents the two physical tone channels.
    QFont captionFont = painter.font();
    captionFont.setPointSizeF(qMax(7.0, captionFont.pointSizeF() - 1.0));
    captionFont.setBold(true);
    painter.setFont(captionFont);
    painter.setPen(QColor(150, 255, 180, 205));
    const QString leftLabel = m_reverse ? QStringLiteral("SPACE") : QStringLiteral("MARK");
    const QString rightLabel = m_reverse ? QStringLiteral("MARK") : QStringLiteral("SPACE");
