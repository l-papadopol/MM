                                         ...
                                         int itemCount) noexcept
{
    ...
    // The supplied value is the UTC age of a sample timeline anchored at
    // capture start.  On a real USB device it also contains hardware-clock
    // drift and backend buffering, so it can grow while capture is continuous.
    // It is useful telemetry, but not a trustworthy worker-pressure signal.
    Q_UNUSED(audioQueueLatencyMs)
    std::lock_guard<std::mutex> lock(m_mutex);
    m_audioQueueLatencyEwmaMs = 0.0;
    m_audioPressureJobCount = 0;
