    ...
    : m_topology(detectTopology())
{
    ...
    // FT4/FT8 decoding is a short, deadline-bound burst. Use every logical
    // processor visible to the process during the decode job; audio, CAT and
    // GUI remain on separate threads and are not part of this dedicated pool.
    // The budget is fixed for the whole capture and never reduced silently.
    m_reservedLogicalProcessors = 0;
    m_maxLiveWorkers = logical;
    m_lastAdjustment = QStringLiteral("initial full-CPU FT decode budget");
}
...
int SystemResourceManager::recommendedWorkers(WorkClass workClass, int itemCount) const noexcept
{
    ...
    case WorkClass::FtGate:
        // The early gate is the QSO deadline path. Use the full dedicated pool
        // so results reach the sequencer before the opposite TX boundary.
        target = live;
        break;
                                         ...
                                         int itemCount) noexcept
{
    ...
    m_lastAdjustment = QStringLiteral("fixed full-CPU live decode budget (%1 workers); telemetry only")
        .arg(m_maxLiveWorkers);
    Q_UNUSED(workClass)
    Q_UNUSED(elapsedMs)
