...
{
    if (combo == nullptr || combo->property("cockpitComboPopupHardened").toBool()) return;
    ...
    if (QAbstractItemView *view = combo->view()) {
        ...
        // Use the cockpit theme colors on every platform.  The popup surface
        // should match the global dark background but stay slightly lighter so
        // the dropdown detaches clearly from the main panel.
