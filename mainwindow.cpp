...
void MainWindow::setupRadioTelescopePage()
{
    ...
    m_spinRadioTelescopeDwellMs->setRange(200, 60000);
    m_spinRadioTelescopeDwellMs->setSingleStep(100);
    m_spinRadioTelescopeDwellMs->setSuffix(QStringLiteral(" ms"));

    m_spinRadioTelescopeSettleMs = new QSpinBox(cfgGroup);
    m_spinRadioTelescopeSettleMs->setRange(0, 10000);
    m_spinRadioTelescopeSettleMs->setSingleStep(100);
    ...
    grid->addWidget(new QLabel(uiText("radio_telescope_scan_geometry", "Scan geometry"), cfgGroup), 0, 0);
    grid->addWidget(m_cmbRadioTelescopeRotatorMode, 0, 1, 1, 3);
    grid->addWidget(new QLabel(uiText("radio_telescope_beamwidth", "Antenna beam width"), cfgGroup), 1, 0);
    grid->addWidget(m_spinRadioTelescopeBeamWidthDeg, 1, 1);
    grid->addWidget(new QLabel(uiText("radio_telescope_dwell", "Dwell / tile"), cfgGroup), 1, 2);
    grid->addWidget(m_spinRadioTelescopeDwellMs, 1, 3);
    grid->addWidget(new QLabel(uiText("radio_telescope_settle", "Settle after move"), cfgGroup), 2, 0);
    grid->addWidget(m_spinRadioTelescopeSettleMs, 2, 1);
    grid->addWidget(new QLabel(uiText("radio_telescope_low_freq", "Audio lower bound"), cfgGroup), 3, 0);
    grid->addWidget(m_spinRadioTelescopeLowHz, 3, 1);
    grid->addWidget(new QLabel(uiText("radio_telescope_high_freq", "Audio upper bound"), cfgGroup), 3, 2);
    grid->addWidget(m_spinRadioTelescopeHighHz, 3, 3);
    grid->addWidget(m_chkRadioTelescopeInterpolate, 4, 0, 1, 4);
    grid->addWidget(m_chkRadioTelescopeAutoPointPeak, 5, 0, 1, 4);
    grid->addWidget(m_chkRadioTelescopeAllowNoRotator, 6, 0, 1, 4);
    grid->addWidget(m_lblRadioTelescopeSummary, 7, 0, 1, 4);
    grid->addWidget(m_lblRadioTelescopeStatus, 8, 0, 1, 4);
    grid->addWidget(m_lblRadioTelescopeProgress, 9, 0, 1, 4);
    ...
    m_btnRadioTelescopeClearMap = new QPushButton(uiText("radio_telescope_clear_heatmap", "Clear heatmap"), cfgGroup);
    ...
    btns->addWidget(m_btnRadioTelescopeStartScan);
    btns->addWidget(m_btnRadioTelescopeStopScan);
    btns->addWidget(m_btnRadioTelescopeClearMap);
    btns->addWidget(m_btnRadioTelescopeExportCsv);
    btns->addWidget(m_btnRadioTelescopePointMax);
    btns->addStretch(1);
...
void MainWindow::loadRadioTelescopeSettings()
{
    ...
    const int defaultSettle = qBound(0, m_settings.rotatorProfiles[activeProfile].settleDelayMs, 10000);
    const int dwell = qBound(200, settings.value(QStringLiteral("RadioTelescope/dwellMs"), RadioTelescopeMode::defaultDwellMs()).toInt(), 60000);
    const int settle = qBound(0, settings.value(QStringLiteral("RadioTelescope/settleMs"), defaultSettle).toInt(), 10000);
...
void MainWindow::updateRadioTelescopeHeatmapWidget()
{
    ...
    if (tiles.isEmpty()) {
        const double baseAzStep = qMax(2.0, beam);
        const double elStep = qMax(2.0, beam * 0.8660254);
        if (useElevation) {
            int row = 0;
            for (double el = 0.0; el <= 90.0 + 0.01; el += elStep, ++row) {
                const double cosEl = qMax(0.08, std::cos(qDegreesToRadians(qMin(89.0, el))));
                const double azStep = qBound(baseAzStep, baseAzStep / cosEl, 120.0);
                const double offset = (row % 2 == 0) ? 0.0 : azStep * 0.5;
                for (double az = offset; az < 360.0 + offset - 0.01; az += azStep) {
                    RadioTelescopeHeatmapWidget::TileSample s;
                    s.azimuthDeg = std::fmod(az, 360.0);
                    if (s.azimuthDeg < 0.0) s.azimuthDeg += 360.0;
                    s.elevationDeg = qMin(90.0, el);
                    s.beamWidthDeg = beam;
                    s.valid = false;
                    tiles.append(s);
                }
            ...
            for (double az = 0.0; az < 360.0 - 0.01; az += baseAzStep) {
                RadioTelescopeHeatmapWidget::TileSample s;
                s.azimuthDeg = az;
                s.elevationDeg = 0.0;
                s.beamWidthDeg = beam;
    ...
    if (useElevation) {
        ...
        for (double el = 0.0; el <= 90.0 + 0.01; el += elStep, ++row) {
            ...
            for (double az = offset; az < 360.0 + offset - 0.01; az += azStep) {
                ...
                tile.azimuthDeg = std::fmod(az, 360.0);
                if (tile.azimuthDeg < 0.0) tile.azimuthDeg += 360.0;
                tile.elevationDeg = qMin(90.0, el);
        ...
        if (rotatorOnline && !ringTiles.isEmpty()) {
            const double parkAz = m_catRotatorController->config().parkAzimuth;
            int nearest = 0;
            auto azError = [](double a, double b) {
                while (a < 0.0) a += 360.0;
                while (a >= 360.0) a -= 360.0;
            ...
            for (int i = 1; i < ringTiles.size(); ++i) {
                if (azError(ringTiles[i].azimuthDeg, parkAz) < azError(ringTiles[nearest].azimuthDeg, parkAz)) {
                    nearest = i;
                }
            }
            std::rotate(ringTiles.begin(), ringTiles.begin() + nearest, ringTiles.end());
        }
...
double MainWindow::measureRadioTelescopeBandPower(const AudioBlock &block) const
{
    ...
    const int fs = qMax(1, block.sampleRate);
    const int n = block.samples.size();
    const int stepHz = qMax(10, qMin(50, (highHz - lowHz) / 16 + 1));
    ...
    double powerSum = 0.0;
    int bins = 0;
    for (int freq = lowHz; freq <= highHz; freq += stepHz) {
        const double coeff = 2.0 * std::cos((kTwoPi * static_cast<double>(freq)) / static_cast<double>(fs));
        double q0 = 0.0;
        double q1 = 0.0;
        double q2 = 0.0;
        double windowNorm = 0.0;
        for (int i = 0; i < n; ++i) {
            const double w = 0.5 - 0.5 * std::cos((kTwoPi * static_cast<double>(i)) / qMax(1.0, static_cast<double>(n - 1)));
            const double x = (static_cast<double>(block.samples[i]) - mean) * w;
            q0 = coeff * q1 - q2 + x;
            q2 = q1;
            q1 = q0;
            windowNorm += w * w;
        ...
        power /= qMax(1.0e-12, windowNorm * static_cast<double>(n));
        powerSum += power;
        ++bins;
    }

    return bins > 0 ? powerSum / static_cast<double>(bins) : 0.0;
}
