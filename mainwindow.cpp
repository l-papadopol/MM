...
#include "modems/msk144/Msk144RxCore.h"
...
QVector<float> mmResampleFloatTo12k(const QVector<float> &mono, int sampleRate)
{
    ...
    if (sampleRate == Msk144RxCore::internalSampleRate()) return mono;
    const double ratio = static_cast<double>(Msk144RxCore::internalSampleRate()) / static_cast<double>(sampleRate);
