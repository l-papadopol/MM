...
#include "modems/radiotelescope/RadioTelescopeMode.h"
...
QString catSettingsKey(const AppSettings &s)
{
    return QStringList{
        ...
        s.hamlibTxAudioRoute, s.hamlibTransmitAudioSource, s.pttMethod, s.pttPortName
    }.join(QStringLiteral("\x1f"));
}

QString rotatorSettingsKey(const AppSettings &s)
{
    QStringList parts;
    parts << (s.rotatorEnabled ? QStringLiteral("1") : QStringLiteral("0"))
          << (s.rotatorAutoConnect ? QStringLiteral("1") : QStringLiteral("0"))
          << (s.rotatorTrackSelectedQso ? QStringLiteral("1") : QStringLiteral("0"))
          << (s.rotatorTrackOnlyWhenQsoActive ? QStringLiteral("1") : QStringLiteral("0"))
          << QString::number(s.rotatorActiveProfile);
    for (int i = 0; i < 3; ++i) {
        const auto &p = s.rotatorProfiles[i];
        parts << p.label << p.bandsCsv << p.peakSearchAlgorithm << QString::number(p.hamlibModel)
              << p.path << QString::number(p.baudRate) << QString::number(p.pollIntervalMs)
              << (p.useElevation ? QStringLiteral("1") : QStringLiteral("0"))
              << (p.overlap ? QStringLiteral("1") : QStringLiteral("0"))
              << p.azimuthGeometryPreset << QString::number(p.azimuthStopDeg, 'f', 3)
              << (p.autoReverseOnStall ? QStringLiteral("1") : QStringLiteral("0"))
              << QString::number(p.noMovementTimeoutMs) << QString::number(p.noMovementThresholdDeg, 'f', 3)
              << QString::number(p.parkAzimuth, 'f', 3) << QString::number(p.parkElevation, 'f', 3)
              << QString::number(p.targetToleranceDeg) << QString::number(p.antennaBeamWidthDeg, 'f', 3)
              << QString::number(p.azimuthMinDeg, 'f', 3) << QString::number(p.azimuthMaxDeg, 'f', 3)
              << QString::number(p.elevationMinDeg, 'f', 3) << QString::number(p.elevationMaxDeg, 'f', 3)
              << QString::number(p.azimuthMsPerDeg, 'f', 3) << QString::number(p.elevationMsPerDeg, 'f', 3)
              << QString::number(p.startupDelayMs) << QString::number(p.settleDelayMs)
              << QString::number(p.txGuardMarginMs);
        for (const auto &b : p.bandSettings) {
            parts << b.band << (b.enabled ? QStringLiteral("1") : QStringLiteral("0"))
                  << QString::number(b.azimuthSearchSpanDeg, 'f', 3)
                  << QString::number(b.elevationSearchSpanDeg, 'f', 3)
                  << (b.autoPeakEnabled ? QStringLiteral("1") : QStringLiteral("0"));
        }
    }
    return parts.join(QStringLiteral("\x1f"));
}

bool uiAppearanceSettingsChanged(const AppSettings &a, const AppSettings &b)
{
    return a.uiTheme != b.uiTheme || a.uiUseThemeFont != b.uiUseThemeFont ||
           a.uiFontFamily != b.uiFontFamily || a.uiFontPointSize != b.uiFontPointSize;
}

bool decodeTableSettingsChanged(const AppSettings &a, const AppSettings &b)
{
    return a.decodeTablePreset != b.decodeTablePreset ||
           a.decodeTableFontFamily != b.decodeTableFontFamily ||
           a.decodeTableFontPointSize != b.decodeTableFontPointSize ||
           a.decodeTableRowHeightPx != b.decodeTableRowHeightPx;
}
...
void fftRadix2Local(std::vector<std::complex<double>> &a)
{
    ...
    for (int len = 2; len <= n; len <<= 1) {
        ...
        for (int i = 0; i < n; i += len) {
            ...
            for (int j = 0; j < half; ++j) {
                const std::complex<double> u = a[static_cast<size_t>(i + j)];
                const std::complex<double> v = a[static_cast<size_t>(i + j + half)] * w;
...
double trimmedMean(QVector<double> values, double lowTrimFraction, double highTrimFraction)
{
    if (values.isEmpty()) {
        return 0.0;
    }
    ...
    int first = qBound(0, static_cast<int>(std::floor(static_cast<double>(n) * lowTrimFraction)), n - 1);
    int lastExclusive = n - qBound(0, static_cast<int>(std::floor(static_cast<double>(n) * highTrimFraction)), n - 1);
...
/**
 * @brief Robust Welch PSD band power for Radio Telescope tiles.
 *
 * This replaces the early Goertzel-bank prototype.  Each audio block is split
 * into Hann-windowed, 50%-overlapped FFT segments.  The one-sided PSD is
 * integrated inside the selected audio slice.  A trimmed mean over frequency
 * bins suppresses narrow carriers/spurs so the result follows broadband noise
 * more than one accidental whistle.  The caller still averages consecutive
 * block results during the tile dwell interval.
 */
double radioTelescopeWelchBandPower(const QVector<float> &samples, int sampleRate, int lowHz, int highHz)
{
    const int n = samples.size();
    ...
    lowHz = qBound(1, lowHz, static_cast<int>(0.48 * static_cast<double>(fs)));
    highHz = qBound(lowHz + 1, highHz, static_cast<int>(0.48 * static_cast<double>(fs)));
    if (highHz <= lowHz) {
        return 0.0;
    }

    int segmentSize = previousPowerOfTwo(qMin(n, 4096));
    if (segmentSize < 256) {
        segmentSize = previousPowerOfTwo(n);
    }
    if (segmentSize < 128) {
        return 0.0;
    }
    const int hop = qMax(1, segmentSize / 2);
    const double df = static_cast<double>(fs) / static_cast<double>(segmentSize);
    const int firstBin = qBound(1, static_cast<int>(std::ceil(static_cast<double>(lowHz) / df)), (segmentSize / 2) - 1);
    const int lastBin = qBound(firstBin, static_cast<int>(std::floor(static_cast<double>(highHz) / df)), segmentSize / 2);
    if (lastBin < firstBin) {
        return 0.0;
    }
    ...
    QVector<double> segmentBandPowers;
    std::vector<std::complex<double>> fft(static_cast<size_t>(segmentSize));

    for (int start = 0; start + segmentSize <= n; start += hop) {
        for (int i = 0; i < segmentSize; ++i) {
            const double x = (static_cast<double>(samples.at(start + i)) - globalMean) * window.at(i);
            fft[static_cast<size_t>(i)] = std::complex<double>(x, 0.0);
        }
        fftRadix2Local(fft);

        QVector<double> binPowers;
        binPowers.reserve(lastBin - firstBin + 1);
        for (int bin = firstBin; bin <= lastBin; ++bin) {
            const std::complex<double> v = fft[static_cast<size_t>(bin)];
            double psd = std::norm(v) / (static_cast<double>(fs) * windowPower);
            if (bin > 0 && bin < segmentSize / 2) {
                psd *= 2.0; // one-sided PSD
            }
            binPowers.append(qMax(0.0, psd * df)); // integrated power represented by this FFT bin
        }

        if (!binPowers.isEmpty()) {
            // Reject a little low-end leakage and more high-end narrow carriers.
            const double perBinPower = trimmedMean(binPowers, 0.05, 0.20);
            segmentBandPowers.append(perBinPower * static_cast<double>(binPowers.size()));
        }
    }

    if (segmentBandPowers.isEmpty()) {
        return 0.0;
    }
    ...
    m_rigThread(new QThread(this))
{
    ...
    if (m_rxAudioRecorder != nullptr && m_rxAudioRecorderThread != nullptr) {
        ...
        connect(m_rxAudioRecorderThread, &QThread::finished,
                m_rxAudioRecorder, &QObject::deleteLater);
        m_rxAudioRecorderThread->start();
    }
    ...
    qRegisterMetaType<CwDiagnosticPoint>("CwDiagnosticPoint");
    qRegisterMetaType<QVector<CwDiagnosticPoint>>("QVector<CwDiagnosticPoint>");
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    ...
    // of relying on quitOnLastWindowClosed.
...
void MainWindow::setupUiState()
{
    ...
    ui->cmbMode->addItem(RadioTelescopeMode::modeName());
...
QWidget *MainWindow::createCwTerminalPage()
{
    QWidget *page = new QWidget(m_mainDisplayStack);
    page->setAutoFillBackground(false);
    page->setStyleSheet(QStringLiteral(
        "QPlainTextEdit { padding: 4px; font-family: DejaVu Sans Mono, Consolas, monospace; font-size: 9pt; }"
    ...
    m_lblCwReceptionTitle = new QLabel(uiText("cw_reception", "CW reception"), page);
    m_lblCwReceptionTitle->setStyleSheet(QStringLiteral("font-weight: 600; font-size: 9pt;"));
    layout->addWidget(m_lblCwReceptionTitle);

    QSplitter *splitter = new QSplitter(Qt::Horizontal, page);
    splitter->setChildrenCollapsible(false);
    splitter->setHandleWidth(5);

    const auto makeReceiverPanel = [this, splitter](bool primary) {
        QFrame *panel = new QFrame(splitter);
        panel->setFrameShape(QFrame::StyledPanel);
        panel->setProperty("cockpitPanel", true);
        panel->setObjectName(primary ? QStringLiteral("cwRxAPanel") : QStringLiteral("cwRxBPanel"));

        QVBoxLayout *panelLayout = new QVBoxLayout(panel);
        panelLayout->setContentsMargins(6, 5, 6, 6);
        ...
        panelLayout->addWidget(terminal, 1);

        if (primary) {
            m_lblCwRxATitle = title;
            m_btnCwClearRx = clearButton;
            m_txtCwRx = terminal;
        } else {
            m_lblCwRxBTitle = title;
            m_btnCwClearRxB = clearButton;
            m_txtCwRxB = terminal;
        }
        return panel;
    };

    splitter->addWidget(makeReceiverPanel(true));
    splitter->addWidget(makeReceiverPanel(false));
    ...
    m_txtCwTx = new QPlainTextEdit(page);
    m_txtCwTx->setMinimumHeight(50);
    m_txtCwTx->setMaximumHeight(68);
    m_txtCwTx->setLineWrapMode(QPlainTextEdit::WidgetWidth);

    m_btnCwSend = new QPushButton(QString::fromUtf8("➤"), page);
    m_btnCwSend->setMinimumWidth(52);
    m_btnCwSend->setMinimumHeight(50);

    inputLayout->addWidget(m_txtCwTx, 1);
    inputLayout->addWidget(m_btnCwSend);
    layout->addLayout(inputLayout);

    // Preserve the legacy pointers used by TX/file actions without consuming a
    // visible utility row. RX clearing is now provided independently in each
    // receiver panel.
    m_btnCwLoadTxText = new QPushButton(page);
    m_btnCwClearTx = new QPushButton(page);
    m_btnCwLoadTxText->hide();
    m_btnCwClearTx->hide();
...
MainWindow::QsoFormWidgets *MainWindow::qsoFormForTerminal(QPlainTextEdit *terminal) const
{
    ...
    if (terminal == m_txtCwRx || terminal == m_txtCwRxB) {
        return m_cwQsoForm;
    }
...
bool MainWindow::eventFilter(QObject *watched, QEvent *event)
{
    if (event != nullptr && event->type() == QEvent::MouseButtonRelease) {
        ...
        if (terminal == m_txtRttyRx || terminal == m_txtBpsk31Rx ||
            terminal == m_txtMfskRx || terminal == m_txtCwRx || terminal == m_txtCwRxB) {
...
void MainWindow::setupTextTerminalPages()
{
    ...
    m_cwDisplayPage = createCwTerminalPage();
    ...
    if (m_btnCwClearRx != nullptr) m_btnCwClearRx->setText(uiText("cw_clear_rx_a", "Clear RX A"));
    if (m_btnCwClearRxB != nullptr) m_btnCwClearRxB->setText(uiText("cw_clear_rx_b", "Clear RX B"));
    if (m_txtCwRx != nullptr) {
        m_txtCwRx->setPlaceholderText(uiText("cw_rx_a_placeholder", "Decoded CW from the green RX A marker appears here."));
    }
    if (m_txtCwRxB != nullptr) {
        m_txtCwRxB->setPlaceholderText(uiText("cw_rx_b_placeholder", "Decoded CW from the blue RX B marker appears here. Right-click the waterfall to enable or retune RX B."));
    }
    if (m_txtCwTx != nullptr) {
        m_txtCwTx->setPlaceholderText(uiText("placeholder.cw_tx", "Type CW text to transmit, then press ➤."));
    }
    m_cwDisplayPage = wrapTextDisplayPageWithMap(m_cwDisplayPage, uiText("tab_cw", "CW"),
                                                  QStringLiteral("CW"), nullptr,
                                                  &m_cwDisplayTabs);
    if (m_cwDisplayTabs != nullptr) {
        m_cwDiagnosticWidget = new CwDiagnosticWidget(m_cwDisplayTabs);
        m_cwDiagnosticWidget->setTextTranslator([this](const QString &key, const QString &fallback) {
            return uiText(key, fallback);
        });
        // Explicit stable keys keep the runtime diagnostic controls in the
        // canonical multilingual dictionary even though the widget asks for
        // them through a callback.
        const QStringList cwDiagnosticTranslations = {
            uiText("cw_diag_oldest", "Jump to the oldest paper"),
            uiText("cw_diag_back", "Move the paper backward"),
            uiText("cw_diag_live", "Return to the live edge"),
            uiText("cw_diag_forward", "Move the paper forward"),
            uiText("cw_diag_newest", "Jump to the live edge"),
            uiText("cw_diag_zoom_out", "Show more time"),
            uiText("cw_diag_zoom_in", "Magnify dits and dahs"),
            uiText("cw_diag_clear", "Clear this diagnostic paper"),
            uiText("cw_diag_live_button", "LIVE"),
            uiText("cw_diag_fft_caption", "Measured PSD ±300 Hz"),
            uiText("cw_diag_paper_caption", "Virtual paper"),
            uiText("cw_diag_rx_a", "RX A · green"),
            uiText("cw_diag_rx_b", "RX B · blue")
        };
        Q_UNUSED(cwDiagnosticTranslations);
        m_cwDisplayTabs->insertTab(1, m_cwDiagnosticWidget,
                                   uiText("cw_diagnostics_tab", "Diagnostics"));
    }
    ...
    m_radioTelescopeDisplayPage = new QWidget(m_mainDisplayStack);
    QVBoxLayout *rtLayout = new QVBoxLayout(m_radioTelescopeDisplayPage);
    rtLayout->setContentsMargins(6, 6, 6, 6);
    rtLayout->setSpacing(5);

    QLabel *rtCaption = new QLabel(uiText("radio_telescope_caption", "Radio telescope sky heatmap"), m_radioTelescopeDisplayPage);
    rtCaption->setStyleSheet(QStringLiteral("font-weight: 600;"));
    m_radioTelescopeHeatmapWidget = new RadioTelescopeHeatmapWidget(m_radioTelescopeDisplayPage);

    rtLayout->addWidget(rtCaption);
    rtLayout->addWidget(m_radioTelescopeHeatmapWidget, 1);
    m_mainDisplayStack->addWidget(m_radioTelescopeDisplayPage);
...
void MainWindow::updateCentralDisplayForMode(const QString &modeName)
{
    ...
    const bool textMode = (modeName == RttyDecoder::modeName() ||
                           ...
                           RadioTelescopeMode::isMode(modeName) ||
    ...
    // Let the waterfall scale proportionally with the main window. In CW it
    // shows user A/B markers and persistent carrier lanes, never decoded glyphs.
    if (ui->grpWaterfall != nullptr) {
    ...
    if (RadioTelescopeMode::isMode(modeName) && m_radioTelescopeDisplayPage != nullptr) {
        m_mainDisplayStack->setCurrentWidget(m_radioTelescopeDisplayPage);
        return;
    }
...
QString MainWindow::shortModeLabel(const QString &modeName) const
{
    ...
    if (RadioTelescopeMode::isMode(modeName)) {
        return QStringLiteral("Radio Telescope");
    }
...
void MainWindow::setupModeMenu()
{
    ...
    addModeAction(RadioTelescopeMode::modeName());
...
void MainWindow::setupCwPage()
{
    ...
    QGroupBox *settingsGroup = new QGroupBox(uiText("cw_settings_group", "CW Morse RX/TX settings"), m_pageCwSettings);
    QGridLayout *grid = new QGridLayout(settingsGroup);
    grid->setContentsMargins(8, 8, 8, 8);
    grid->setHorizontalSpacing(6);
    grid->setVerticalSpacing(6);
    ...
    for (QSpinBox *spin : {m_spinCwWpmA, m_spinCwWpmB}) {
        spin->setRange(5, 50);
        spin->setSingleStep(1);
        spin->setSuffix(" WPM");
    }
    m_spinCwTxWpm->setRange(5, 80);
    m_spinCwTxWpm->setSingleStep(1);
    m_spinCwTxWpm->setSuffix(" WPM");
    m_chkCwAutoWpmA = new QCheckBox(uiText("cw_auto_short", "Auto"), settingsGroup);
    m_chkCwAutoWpmB = new QCheckBox(uiText("cw_auto_short", "Auto"), settingsGroup);
    m_lblCwTrackedWpmA = new QLabel(QStringLiteral("--"), settingsGroup);
    m_lblCwTrackedWpmB = new QLabel(QStringLiteral("--"), settingsGroup);
    m_lblCwTrackedWpmA->setMinimumWidth(42);
    ...
    m_chkCwAutoBandwidth = new QCheckBox(uiText("cw_auto_short", "Auto"), settingsGroup);
    m_chkCwAfc = new QCheckBox(uiText("cw_afc_label", "AFC"), settingsGroup);
    ...
    grid->addWidget(m_spinCwToneHz, 0, 1, 1, 3);
    grid->addWidget(rxALabel, 1, 0);
    grid->addWidget(m_chkCwAutoWpmA, 1, 1);
    grid->addWidget(m_spinCwWpmA, 1, 2);
    grid->addWidget(m_lblCwTrackedWpmA, 1, 3);
    grid->addWidget(rxBLabel, 2, 0);
    grid->addWidget(m_chkCwAutoWpmB, 2, 1);
    ...
    grid->addWidget(m_chkCwAfc, 5, 0, 1, 1);
    grid->addWidget(new QLabel(uiText("cw_afc_range", "AFC range"), settingsGroup), 5, 1);
    grid->addWidget(m_spinCwAfcRangeHz, 5, 2, 1, 2);
    grid->addWidget(m_lblCwDualRx, 6, 0, 1, 3);
    grid->addWidget(m_btnCwDisableSecondary, 6, 3);
...
void MainWindow::loadCwSettingsToUi()
{
    if (m_spinCwToneHz == nullptr ||
        ...
        m_chkCwAutoWpmA == nullptr ||
        m_chkCwAutoWpmB == nullptr ||
        m_spinCwBandwidthHz == nullptr ||
        m_chkCwAutoBandwidth == nullptr ||
        m_chkCwAfc == nullptr ||
    ...
    const QSignalBlocker blockAutoWpmA(m_chkCwAutoWpmA);
    const QSignalBlocker blockAutoWpmB(m_chkCwAutoWpmB);
    const QSignalBlocker blockBandwidth(m_spinCwBandwidthHz);
    const QSignalBlocker blockAutoBandwidth(m_chkCwAutoBandwidth);
    ...
    m_spinCwWpmA->setValue(qBound(5, m_settings.cwWpmA, 50));
    m_spinCwWpmB->setValue(qBound(5, m_settings.cwWpmB, 50));
    m_spinCwTxWpm->setValue(qBound(5, m_settings.cwTxWpm, 80));
    m_chkCwAutoWpmA->setChecked(m_settings.cwAutoWpmA);
    m_chkCwAutoWpmB->setChecked(m_settings.cwAutoWpmB);
    m_spinCwWpmA->setEnabled(!m_settings.cwAutoWpmA);
    m_spinCwWpmB->setEnabled(!m_settings.cwAutoWpmB);
    if (m_lblCwTrackedWpmA != nullptr) m_lblCwTrackedWpmA->setText(QStringLiteral("--"));
    ...
    m_chkCwAutoBandwidth->setChecked(m_settings.cwAutoBandwidth);
    m_spinCwBandwidthHz->setEnabled(!m_settings.cwAutoBandwidth);
...
void MainWindow::setupRadioTelescopePage()
{
    if (ui == nullptr || ui->stkModeSettings == nullptr) {
        return;
    }

    m_pageRadioTelescopeSettings = new QWidget(ui->stkModeSettings);
    QVBoxLayout *outer = new QVBoxLayout(m_pageRadioTelescopeSettings);
    outer->setContentsMargins(6, 6, 6, 6);
    outer->setSpacing(6);

    QGroupBox *cfgGroup = new QGroupBox(uiText("radio_telescope_settings", "Radio telescope scan settings"), m_pageRadioTelescopeSettings);
    ...
    auto prepareSpinBoxForTyping = [](QAbstractSpinBox *spin) {
        if (spin == nullptr) {
            return;
        }
    ...
    m_cmbRadioTelescopeRotatorMode = new QComboBox(cfgGroup);
    m_cmbRadioTelescopeRotatorMode->addItem(uiText("radio_telescope_rotator_altaz", "Alt-Az sky disk"), QStringLiteral("altaz"));
    m_cmbRadioTelescopeRotatorMode->addItem(uiText("radio_telescope_rotator_az_only", "Azimuth ring"), QStringLiteral("az"));
    m_cmbRadioTelescopeRotatorMode->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);
    m_cmbRadioTelescopeRotatorMode->setMinimumWidth(168);

    m_spinRadioTelescopeBeamWidthDeg = new QDoubleSpinBox(cfgGroup);
    m_spinRadioTelescopeBeamWidthDeg->setRange(2.0, 120.0);
    m_spinRadioTelescopeBeamWidthDeg->setDecimals(1);
    m_spinRadioTelescopeBeamWidthDeg->setSuffix(QStringLiteral(" °"));
    prepareSpinBoxForTyping(m_spinRadioTelescopeBeamWidthDeg);

    m_spinRadioTelescopeLowHz = new QSpinBox(cfgGroup);
    m_spinRadioTelescopeLowHz->setRange(0, 3500);
    m_spinRadioTelescopeLowHz->setSingleStep(50);
    m_spinRadioTelescopeLowHz->setSuffix(QStringLiteral(" Hz"));
    prepareSpinBoxForTyping(m_spinRadioTelescopeLowHz);
    m_spinRadioTelescopeHighHz = new QSpinBox(cfgGroup);
    m_spinRadioTelescopeHighHz->setRange(0, 3500);
    m_spinRadioTelescopeHighHz->setSingleStep(50);
    m_spinRadioTelescopeHighHz->setSuffix(QStringLiteral(" Hz"));
    prepareSpinBoxForTyping(m_spinRadioTelescopeHighHz);

    m_spinRadioTelescopeDwellMs = new QSpinBox(cfgGroup);
    m_spinRadioTelescopeDwellMs->setRange(500, 120000);
    m_spinRadioTelescopeDwellMs->setSingleStep(500);
    m_spinRadioTelescopeDwellMs->setSuffix(QStringLiteral(" ms"));
    prepareSpinBoxForTyping(m_spinRadioTelescopeDwellMs);

    m_spinRadioTelescopeSettleMs = new QSpinBox(cfgGroup);
    m_spinRadioTelescopeSettleMs->setRange(0, 30000);
    m_spinRadioTelescopeSettleMs->setSingleStep(250);
    m_spinRadioTelescopeSettleMs->setSuffix(QStringLiteral(" ms"));
    prepareSpinBoxForTyping(m_spinRadioTelescopeSettleMs);

    m_chkRadioTelescopeInterpolate = new QCheckBox(uiText("radio_telescope_interpolate", "Interpolate adjacent tiles"), cfgGroup);
    m_chkRadioTelescopeAutoPointPeak = new QCheckBox(uiText("radio_telescope_auto_point_peak", "Run rotator auto-peak after scan"), cfgGroup);
    m_chkRadioTelescopeAllowNoRotator = new QCheckBox(uiText("radio_telescope_allow_no_rotator", "Bench test without rotator (no real sky map)"), cfgGroup);
    m_chkRadioTelescopeAllowNoRotator->setToolTip(uiText("radio_telescope_allow_no_rotator_tip", "Disabled by default. Use only for audio/heatmap bench tests: the antenna will not move, so the map is not a real sky survey."));
    m_lblRadioTelescopeSummary = new QLabel(cfgGroup);
    m_lblRadioTelescopeSummary->setWordWrap(true);
    m_lblRadioTelescopeStatus = new QLabel(uiText("radio_telescope_status_idle", "Heatmap idle"), cfgGroup);
    m_lblRadioTelescopeStatus->setWordWrap(true);
    m_lblRadioTelescopeProgress = new QLabel(uiText("radio_telescope_progress_idle", "Scan stopped"), cfgGroup);
    m_lblRadioTelescopeProgress->setWordWrap(true);

    int cfgRow = 0;
    auto addWideControl = [&](const QString &labelText, QWidget *widget) {
        QLabel *label = new QLabel(labelText, cfgGroup);
        label->setWordWrap(true);
        grid->addWidget(label, cfgRow++, 0);
        grid->addWidget(widget, cfgRow++, 0);
    };
    addWideControl(uiText("radio_telescope_scan_geometry", "Scan geometry"), m_cmbRadioTelescopeRotatorMode);
    addWideControl(uiText("radio_telescope_beamwidth", "Antenna beam width"), m_spinRadioTelescopeBeamWidthDeg);
    addWideControl(uiText("radio_telescope_dwell", "Dwell / tile"), m_spinRadioTelescopeDwellMs);
    addWideControl(uiText("radio_telescope_settle", "Settle after move"), m_spinRadioTelescopeSettleMs);
    addWideControl(uiText("radio_telescope_low_freq", "Audio lower bound"), m_spinRadioTelescopeLowHz);
    addWideControl(uiText("radio_telescope_high_freq", "Audio upper bound"), m_spinRadioTelescopeHighHz);
    grid->addWidget(m_chkRadioTelescopeInterpolate, cfgRow++, 0);
    grid->addWidget(m_chkRadioTelescopeAutoPointPeak, cfgRow++, 0);
    grid->addWidget(m_chkRadioTelescopeAllowNoRotator, cfgRow++, 0);
    grid->addWidget(m_lblRadioTelescopeSummary, cfgRow++, 0);
    grid->addWidget(m_lblRadioTelescopeStatus, cfgRow++, 0);
    grid->addWidget(m_lblRadioTelescopeProgress, cfgRow++, 0);

    QGridLayout *btns = new QGridLayout();
    btns->setContentsMargins(0, 0, 0, 0);
    btns->setHorizontalSpacing(6);
    btns->setVerticalSpacing(6);
    m_btnRadioTelescopeStartScan = new QPushButton(uiText("radio_telescope_start_scan", "Start scan"), cfgGroup);
    m_btnRadioTelescopeStopScan = new QPushButton(uiText("radio_telescope_stop_scan", "Stop scan"), cfgGroup);
    m_btnRadioTelescopeClearMap = new QPushButton(uiText("radio_telescope_reset_map", "Reset map"), cfgGroup);
    m_btnRadioTelescopeClearMap->setToolTip(uiText("radio_telescope_reset_map_tip", "Clear measured tiles and redraw the empty scan grid."));
    m_btnRadioTelescopeExportCsv = new QPushButton(uiText("radio_telescope_export_csv", "Export CSV"), cfgGroup);
    m_btnRadioTelescopePointMax = new QPushButton(uiText("radio_telescope_point_max", "Auto peak"), cfgGroup);
    m_btnRadioTelescopePointMax->setToolTip(uiText("radio_telescope_point_max_tip", "Use the selected rotator auto-peak algorithm and the measured heatmap to refine or point the strongest direction."));
    QList<QPushButton *> rtButtons { m_btnRadioTelescopeStartScan, m_btnRadioTelescopeStopScan, m_btnRadioTelescopeClearMap, m_btnRadioTelescopeExportCsv, m_btnRadioTelescopePointMax };
    for (QPushButton *button : rtButtons) {
        button->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);
        button->setMinimumWidth(88);
    }
    btns->addWidget(m_btnRadioTelescopeStartScan, 0, 0);
    btns->addWidget(m_btnRadioTelescopeStopScan, 0, 1);
    btns->addWidget(m_btnRadioTelescopeClearMap, 1, 0);
    btns->addWidget(m_btnRadioTelescopeExportCsv, 1, 1);
    btns->addWidget(m_btnRadioTelescopePointMax, 2, 0, 1, 2);
    btns->setColumnStretch(0, 1);
    btns->setColumnStretch(1, 1);

    QGroupBox *dataGroup = new QGroupBox(uiText("radio_telescope_measurements", "Measured tiles"), m_pageRadioTelescopeSettings);
    QVBoxLayout *dataLayout = new QVBoxLayout(dataGroup);
    dataLayout->setContentsMargins(8, 8, 8, 8);
    dataLayout->setSpacing(4);
    m_tableRadioTelescopeSamples = new QTableWidget(0, 4, dataGroup);
    m_tableRadioTelescopeSamples->setHorizontalHeaderLabels(QStringList() << "Az" << "El" << "Noise" << "UTC");
    m_tableRadioTelescopeSamples->setSelectionBehavior(QAbstractItemView::SelectRows);
    m_tableRadioTelescopeSamples->setSelectionMode(QAbstractItemView::SingleSelection);
    m_tableRadioTelescopeSamples->setEditTriggers(QAbstractItemView::NoEditTriggers);
    m_tableRadioTelescopeSamples->verticalHeader()->setVisible(false);
    m_tableRadioTelescopeSamples->horizontalHeader()->setStretchLastSection(true);
    m_tableRadioTelescopeSamples->setColumnWidth(0, 44);
    m_tableRadioTelescopeSamples->setColumnWidth(1, 44);
    m_tableRadioTelescopeSamples->setColumnWidth(2, 82);
    m_tableRadioTelescopeSamples->setMinimumHeight(180);
    applyDecodeTableVisualSettings(m_tableRadioTelescopeSamples);
    dataLayout->addWidget(m_tableRadioTelescopeSamples, 1);

    outer->addWidget(cfgGroup);
    outer->addLayout(btns);
    outer->addWidget(dataGroup, 1);

    ui->stkModeSettings->addWidget(m_pageRadioTelescopeSettings);

    loadRadioTelescopeSettings();

    {
        QSettings settings(AppSettings::settingsFilePath(), QSettings::IniFormat);
        const QString savedGeometry = settings.value(QStringLiteral("RadioTelescope/geometry"), QStringLiteral("auto")).toString().trimmed().toLower();
        if (savedGeometry.isEmpty() || savedGeometry == QStringLiteral("auto")) {
            const int configuredProfile = qBound(0, m_settings.rotatorActiveProfile, 2);
            const bool useElevation = m_settings.rotatorProfiles[configuredProfile].useElevation;
            m_cmbRadioTelescopeRotatorMode->setCurrentIndex(useElevation ? 0 : 1);
        }
    }

    connect(&m_radioTelescopeScanTimer, &QTimer::timeout, this, &MainWindow::advanceRadioTelescopeScan, Qt::UniqueConnection);
    m_radioTelescopeScanTimer.setInterval(180);

    auto persistAndApply = [this]() {
        saveRadioTelescopeSettings();
        applyRadioTelescopeSettings();
    };

    connect(m_btnRadioTelescopeStartScan, &QPushButton::clicked, this, [this]() {
        QString reason;
        if (!radioTelescopeRotatorReadyForScan(&reason)) {
            appendLog(QStringLiteral("Radio Telescope scan blocked: ") + reason);
            if (m_lblRadioTelescopeStatus != nullptr) {
                m_lblRadioTelescopeStatus->setText(reason);
            }
            return;
        }
        if (!m_rxRunning && ui != nullptr && ui->cmbMode != nullptr && RadioTelescopeMode::isMode(ui->cmbMode->currentText())) {
            startRx();
            QTimer::singleShot(700, this, [this]() { startRadioTelescopeScan(); });
            return;
        }
        startRadioTelescopeScan();
    });
    connect(m_btnRadioTelescopeStopScan, &QPushButton::clicked, this, [this]() { stopRadioTelescopeScan(false); });
    connect(m_btnRadioTelescopeClearMap, &QPushButton::clicked, this, &MainWindow::clearRadioTelescopeHeatmap);
    connect(m_btnRadioTelescopeExportCsv, &QPushButton::clicked, this, &MainWindow::exportRadioTelescopeCsv);
    connect(m_btnRadioTelescopePointMax, &QPushButton::clicked, this, &MainWindow::pointRadioTelescopeMaxNoise);
    connect(m_cmbRadioTelescopeRotatorMode, qOverload<int>(&QComboBox::currentIndexChanged), this, [persistAndApply](int) { persistAndApply(); });
    connect(m_spinRadioTelescopeBeamWidthDeg, qOverload<double>(&QDoubleSpinBox::valueChanged), this, [persistAndApply](double) { persistAndApply(); });
    connect(m_spinRadioTelescopeLowHz, qOverload<int>(&QSpinBox::valueChanged), this, [persistAndApply](int) { persistAndApply(); });
    connect(m_spinRadioTelescopeHighHz, qOverload<int>(&QSpinBox::valueChanged), this, [persistAndApply](int) { persistAndApply(); });
    connect(m_spinRadioTelescopeDwellMs, qOverload<int>(&QSpinBox::valueChanged), this, [persistAndApply](int) { persistAndApply(); });
    connect(m_spinRadioTelescopeSettleMs, qOverload<int>(&QSpinBox::valueChanged), this, [persistAndApply](int) { persistAndApply(); });
    connect(m_chkRadioTelescopeInterpolate, &QCheckBox::toggled, this, [persistAndApply](bool) { persistAndApply(); });
    connect(m_chkRadioTelescopeAutoPointPeak, &QCheckBox::toggled, this, [persistAndApply](bool) { persistAndApply(); });
    connect(m_chkRadioTelescopeAllowNoRotator, &QCheckBox::toggled, this, [persistAndApply](bool) { persistAndApply(); });

    applyRadioTelescopeSettings();
}

void MainWindow::loadRadioTelescopeSettings()
{
    QSettings settings(AppSettings::settingsFilePath(), QSettings::IniFormat);
    const int activeProfile = qBound(0, m_settings.rotatorActiveProfile, 2);
    const double profileBeam = qBound(2.0, m_settings.rotatorProfiles[activeProfile].antennaBeamWidthDeg, 120.0);
    const double beam = qBound(2.0, settings.value(QStringLiteral("RadioTelescope/beamWidthDeg"), profileBeam).toDouble(), 120.0);
    const int low = qBound(0, settings.value(QStringLiteral("RadioTelescope/lowHz"), RadioTelescopeMode::defaultLowFrequencyHz()).toInt(), 3500);
    int high = qBound(0, settings.value(QStringLiteral("RadioTelescope/highHz"), RadioTelescopeMode::defaultHighFrequencyHz()).toInt(), 3500);
    if (low == 1000 && high == 1100) {
        high = RadioTelescopeMode::defaultHighFrequencyHz();
    }
    const int profileSettle = qBound(0, m_settings.rotatorProfiles[activeProfile].settleDelayMs, 30000);
    const int defaultSettle = qBound(0, qMax(1500, profileSettle), 30000);
    const bool timingDefaultsV2 = settings.value(QStringLiteral("RadioTelescope/timingDefaultsV2"), false).toBool();
    int dwell = qBound(500, settings.value(QStringLiteral("RadioTelescope/dwellMs"), RadioTelescopeMode::defaultDwellMs()).toInt(), 120000);
    int settle = qBound(0, settings.value(QStringLiteral("RadioTelescope/settleMs"), defaultSettle).toInt(), 30000);
    if (!timingDefaultsV2) {
        if (dwell < 3000) {
            dwell = RadioTelescopeMode::defaultDwellMs();
        }
        if (settle < 1200) {
            settle = defaultSettle;
        }
        settings.setValue(QStringLiteral("RadioTelescope/timingDefaultsV2"), true);
        settings.setValue(QStringLiteral("RadioTelescope/dwellMs"), dwell);
        settings.setValue(QStringLiteral("RadioTelescope/settleMs"), settle);
    }
    const bool interp = settings.value(QStringLiteral("RadioTelescope/interpolate"), true).toBool();
    const bool autoPoint = settings.value(QStringLiteral("RadioTelescope/autoPointPeak"), true).toBool();
    const bool allowNoRotator = settings.value(QStringLiteral("RadioTelescope/allowNoRotator"), false).toBool();
    const QString geometry = settings.value(QStringLiteral("RadioTelescope/geometry"), QStringLiteral("auto")).toString().trimmed().toLower();

    if (m_spinRadioTelescopeBeamWidthDeg != nullptr) m_spinRadioTelescopeBeamWidthDeg->setValue(beam);
    if (m_spinRadioTelescopeLowHz != nullptr) m_spinRadioTelescopeLowHz->setValue(low);
    if (m_spinRadioTelescopeHighHz != nullptr) m_spinRadioTelescopeHighHz->setValue(qMax(low + 1, high));
    if (m_spinRadioTelescopeDwellMs != nullptr) m_spinRadioTelescopeDwellMs->setValue(dwell);
    if (m_spinRadioTelescopeSettleMs != nullptr) m_spinRadioTelescopeSettleMs->setValue(settle);
    if (m_chkRadioTelescopeInterpolate != nullptr) m_chkRadioTelescopeInterpolate->setChecked(interp);
    if (m_chkRadioTelescopeAutoPointPeak != nullptr) m_chkRadioTelescopeAutoPointPeak->setChecked(autoPoint);
    if (m_chkRadioTelescopeAllowNoRotator != nullptr) m_chkRadioTelescopeAllowNoRotator->setChecked(allowNoRotator);
    if (m_cmbRadioTelescopeRotatorMode != nullptr && geometry != QStringLiteral("auto")) {
        const int idx = m_cmbRadioTelescopeRotatorMode->findData(geometry);
        if (idx >= 0) m_cmbRadioTelescopeRotatorMode->setCurrentIndex(idx);
    }
}

void MainWindow::saveRadioTelescopeSettings()
{
    if (m_spinRadioTelescopeBeamWidthDeg == nullptr || m_spinRadioTelescopeLowHz == nullptr || m_spinRadioTelescopeHighHz == nullptr ||
        m_spinRadioTelescopeDwellMs == nullptr || m_spinRadioTelescopeSettleMs == nullptr ||
        m_chkRadioTelescopeInterpolate == nullptr || m_chkRadioTelescopeAutoPointPeak == nullptr || m_chkRadioTelescopeAllowNoRotator == nullptr ||
        m_cmbRadioTelescopeRotatorMode == nullptr) {
        return;
    }

    QSettings settings(AppSettings::settingsFilePath(), QSettings::IniFormat);
    settings.setValue(QStringLiteral("RadioTelescope/beamWidthDeg"), m_spinRadioTelescopeBeamWidthDeg->value());
    settings.setValue(QStringLiteral("RadioTelescope/lowHz"), m_spinRadioTelescopeLowHz->value());
    settings.setValue(QStringLiteral("RadioTelescope/highHz"), m_spinRadioTelescopeHighHz->value());
    settings.setValue(QStringLiteral("RadioTelescope/dwellMs"), m_spinRadioTelescopeDwellMs->value());
    settings.setValue(QStringLiteral("RadioTelescope/settleMs"), m_spinRadioTelescopeSettleMs->value());
    settings.setValue(QStringLiteral("RadioTelescope/timingDefaultsV2"), true);
    settings.setValue(QStringLiteral("RadioTelescope/interpolate"), m_chkRadioTelescopeInterpolate->isChecked());
    settings.setValue(QStringLiteral("RadioTelescope/autoPointPeak"), m_chkRadioTelescopeAutoPointPeak->isChecked());
    settings.setValue(QStringLiteral("RadioTelescope/allowNoRotator"), m_chkRadioTelescopeAllowNoRotator->isChecked());
    settings.setValue(QStringLiteral("RadioTelescope/geometry"), m_cmbRadioTelescopeRotatorMode->currentData().toString());

    const int activeProfile = qBound(0, m_settings.rotatorActiveProfile, 2);
    m_settings.rotatorProfiles[activeProfile].antennaBeamWidthDeg = m_spinRadioTelescopeBeamWidthDeg->value();
    QSettings profileSettings(AppSettings::settingsFilePath(), QSettings::IniFormat);
    profileSettings.setValue(QStringLiteral("Rotator/Profile%1/antennaBeamWidthDeg").arg(activeProfile + 1), m_spinRadioTelescopeBeamWidthDeg->value());
}

void MainWindow::syncRadioTelescopeBeamFromActiveRotator()
{
    if (m_spinRadioTelescopeBeamWidthDeg == nullptr) {
        return;
    }
    const int activeProfile = qBound(0, m_settings.rotatorActiveProfile, 2);
    const double beam = qBound(2.0, m_settings.rotatorProfiles[activeProfile].antennaBeamWidthDeg, 120.0);
    if (qAbs(m_spinRadioTelescopeBeamWidthDeg->value() - beam) > 0.05) {
        const QSignalBlocker block(m_spinRadioTelescopeBeamWidthDeg);
        m_spinRadioTelescopeBeamWidthDeg->setValue(beam);
    }
    applyRadioTelescopeSettings();
}

void MainWindow::exportRadioTelescopeCsv()
{
    if (m_radioTelescopeTiles.isEmpty()) {
        appendLog(QStringLiteral("Radio Telescope CSV export skipped: no measured tiles."));
        return;
    }

    const QString defaultName = QStringLiteral("radio_telescope_%1.csv")
        .arg(QDateTime::currentDateTimeUtc().toString(QStringLiteral("yyyyMMdd_HHmmss")));
    const QString filePath = QFileDialog::getSaveFileName(this,
                                                          uiText("radio_telescope_export_csv_title", "Export radio telescope CSV"),
                                                          QDir(QCoreApplication::applicationDirPath()).filePath(defaultName),
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        appendLog(QStringLiteral("Radio Telescope CSV export failed: %1").arg(file.errorString()));
        return;
    }

    QTextStream out(&file);
    out << "azimuth_deg,elevation_deg,beam_width_deg,noise_dbfs,measured\n";
    for (const RadioTelescopeTileRecord &tile : std::as_const(m_radioTelescopeTiles)) {
        out << QString::number(tile.azimuthDeg, 'f', 3) << ','
            << QString::number(tile.elevationDeg, 'f', 3) << ','
            << QString::number(tile.beamWidthDeg, 'f', 3) << ','
            << (tile.measured ? QString::number(tile.noiseDb, 'f', 3) : QString()) << ','
            << (tile.measured ? QStringLiteral("1") : QStringLiteral("0")) << '\n';
    }

    appendLog(QStringLiteral("Radio Telescope CSV exported: %1").arg(filePath));
}


bool MainWindow::radioTelescopeTileMeasuredNear(double azimuthDeg, double elevationDeg, double toleranceDeg) const
{
    auto azError = [](double a, double b) {
        while (a < 0.0) a += 360.0;
    ...
    for (const RadioTelescopeTileRecord &tile : m_radioTelescopeTiles) {
        if (!tile.measured) continue;
        if (azError(tile.azimuthDeg, azimuthDeg) <= toleranceDeg && qAbs(tile.elevationDeg - elevationDeg) <= toleranceDeg) {
            return true;
        }
    }
    return false;
...
bool MainWindow::continueRadioTelescopePeakRefinement()
{
    ...
    const bool useElevation = (m_cmbRadioTelescopeRotatorMode == nullptr ||
                               m_cmbRadioTelescopeRotatorMode->currentData().toString() != QStringLiteral("az")) &&
    ...
    for (const AppSettings::RotatorBandSettings &row : profile.bandSettings) {
        if (!wantedBand.isEmpty() && normalizeBand(row.band) == wantedBand) {
            spanRow = row;
            foundRow = true;
            break;
        }
    ...
    const double beam = m_spinRadioTelescopeBeamWidthDeg != nullptr
                            ? qBound(2.0, m_spinRadioTelescopeBeamWidthDeg->value(), 120.0)
                            : qBound(2.0, profile.antennaBeamWidthDeg, 120.0);
    const double spanAz = foundRow ? qBound(beam * 0.25, spanRow.azimuthSearchSpanDeg, 180.0)
                                   : qBound(beam, beam * 2.0, 180.0);
    const double spanEl = useElevation
                              ? (foundRow ? qBound(beam * 0.25, spanRow.elevationSearchSpanDeg, 180.0)
    ...
    RadioTelescopeTileRecord refineTile;
    refineTile.azimuthDeg = rec.nextAzimuthDeg;
    refineTile.elevationDeg = useElevation ? rec.nextElevationDeg : 0.0;
    refineTile.beamWidthDeg = qMax(2.0, beam * 0.5);
    m_radioTelescopeTiles.append(refineTile);
    m_radioTelescopeCurrentTileIndex = m_radioTelescopeTiles.size() - 1;
    m_radioTelescopeScanPhase = RadioTelescopeScanPhase::Idle;
    m_radioTelescopeScanActive = true;
    m_radioTelescopePeakRefineActive = true;
    --m_radioTelescopePeakRefineStepsRemaining;
    m_radioTelescopeScanTimer.start();
    appendLog(QStringLiteral("Radio Telescope auto-peak %1: probing az %2°, el %3° (%4 step(s) left). %5")
                  .arg(algorithm,
                       QString::number(refineTile.azimuthDeg, 'f', 1),
...
void MainWindow::pointRadioTelescopeMaxNoise()
{
    if (m_catRotatorController == nullptr || !m_catRotatorController->config().enabled || !m_catRotatorController->isConnected()) {
        appendLog(QStringLiteral("Radio Telescope auto-peak skipped: rotator is not connected."));
        return;
    }
    ...
    if (continueRadioTelescopePeakRefinement()) {
        return;
    }
    ...
    if (bestIndex < 0) {
        appendLog(QStringLiteral("Radio Telescope peak point skipped: no measured tile."));
        return;
    }

    const RadioTelescopeTileRecord &tile = m_radioTelescopeTiles[bestIndex];
    m_radioTelescopeTargetAzimuthDeg = tile.azimuthDeg;
    ...
    updateRadioTelescopeHeatmapWidget();
    if (m_radioTelescopeHeatmapWidget != nullptr) {
        m_radioTelescopeHeatmapWidget->setCurrentTarget(tile.azimuthDeg, tile.elevationDeg, true);
    }
    appendLog(QStringLiteral("Radio Telescope fallback point: strongest measured tile az %1°, el %2°, noise %3 dBFS.")
                  .arg(QString::number(tile.azimuthDeg, 'f', 1),
                       QString::number(tile.elevationDeg, 'f', 1),
                       QString::number(tile.noiseDb, 'f', 1)));
}

void MainWindow::applyRadioTelescopeSettings()
{
    if (m_spinRadioTelescopeLowHz == nullptr || m_spinRadioTelescopeHighHz == nullptr) {
        return;
    }

    if (m_spinRadioTelescopeLowHz->value() >= m_spinRadioTelescopeHighHz->value()) {
        const QSignalBlocker blockHigh(m_spinRadioTelescopeHighHz);
        m_spinRadioTelescopeHighHz->setValue(qMin(3500, m_spinRadioTelescopeLowHz->value() + 1));
    }

    QString reason;
    const bool ready = radioTelescopeRotatorReadyForScan(&reason);
    if (m_lblRadioTelescopeStatus != nullptr && !m_radioTelescopeScanActive) {
        m_lblRadioTelescopeStatus->setText(ready
            ? uiText("radio_telescope_status_ready", "Rotator ready. Start scan when RX is active.")
            : reason);
    }
...
void MainWindow::clearRadioTelescopeHeatmap()
{
    if (m_radioTelescopeScanActive || m_radioTelescopeScanTimer.isActive()) {
    ...
    if (m_tableRadioTelescopeSamples != nullptr) {
        m_tableRadioTelescopeSamples->setRowCount(0);
    }
    if (m_lblRadioTelescopeStatus != nullptr) {
        m_lblRadioTelescopeStatus->setText(uiText("radio_telescope_status_idle", "Heatmap idle"));
    }
    if (m_lblRadioTelescopeProgress != nullptr) {
        m_lblRadioTelescopeProgress->setText(uiText("radio_telescope_progress_idle", "Scan stopped"));
    }
    updateRadioTelescopeHeatmapWidget();
}

void MainWindow::updateRadioTelescopeHeatmapWidget()
{
    if (m_radioTelescopeHeatmapWidget == nullptr) {
        return;
    }

    const bool useElevation = m_cmbRadioTelescopeRotatorMode == nullptr || m_cmbRadioTelescopeRotatorMode->currentData().toString() != QStringLiteral("az");
    const double beam = (m_spinRadioTelescopeBeamWidthDeg != nullptr) ? m_spinRadioTelescopeBeamWidthDeg->value() : RadioTelescopeMode::defaultBeamWidthDeg();

    QVector<RadioTelescopeHeatmapWidget::TileSample> tiles;
    tiles.reserve(m_radioTelescopeTiles.isEmpty() ? 128 : m_radioTelescopeTiles.size());
    for (const RadioTelescopeTileRecord &t : std::as_const(m_radioTelescopeTiles)) {
        RadioTelescopeHeatmapWidget::TileSample s;
        s.azimuthDeg = t.azimuthDeg;
        s.elevationDeg = t.elevationDeg;
        s.beamWidthDeg = t.beamWidthDeg;
        s.noiseDb = t.noiseDb;
        s.skyX = t.skyX;
        s.skyY = t.skyY;
        s.hasSkyPosition = t.hasSkyPosition;
        s.valid = t.measured;
        tiles.append(s);
    }

    if (tiles.isEmpty()) {
        auto appendAltAzHoneycombPreview = [&]() {
            const double hexRadius = qBound(0.015, beam / (90.0 * qSqrt(3.0)), 0.45);
            const double dx = qSqrt(3.0) * hexRadius;
            const double dy = 1.5 * hexRadius;
            int row = 0;
            for (double y = -1.0; y <= 1.0 + 1e-9; y += dy, ++row) {
                const double offset = (row % 2 == 0) ? 0.0 : dx * 0.5;
                for (double x = -1.0 - dx; x <= 1.0 + dx; x += dx) {
                    const double cx = x + offset;
                    const double cy = y;
                    const double rr = qSqrt(cx * cx + cy * cy);
                    if (rr > 1.0 + 1.0e-6) {
                        continue;
                    }
                    const double az = std::fmod(qRadiansToDegrees(std::atan2(cx, -cy)) + 360.0, 360.0);
                    const double el = qBound(0.0, 90.0 * (1.0 - rr), 90.0);
                    RadioTelescopeHeatmapWidget::TileSample sample;
                    sample.azimuthDeg = az;
                    sample.elevationDeg = el;
                    sample.beamWidthDeg = beam;
                    sample.skyX = cx;
                    sample.skyY = cy;
                    sample.hasSkyPosition = true;
                    sample.valid = false;
                    tiles.append(sample);
                }
            }
        ...
        auto appendAzRingPreview = [&]() {
            const int count = qMax(1, static_cast<int>(std::ceil(360.0 / qMax(2.0, beam))));
            const double step = 360.0 / static_cast<double>(count);
            constexpr double ringNorm = 0.79;
            for (int i = 0; i < count; ++i) {
                const double az = std::fmod((static_cast<double>(i) + 0.5) * step, 360.0);
                RadioTelescopeHeatmapWidget::TileSample sample;
                sample.azimuthDeg = az;
                sample.elevationDeg = 0.0;
                sample.beamWidthDeg = step;
                const double rad = qDegreesToRadians(az);
                sample.skyX = ringNorm * qSin(rad);
                sample.skyY = -ringNorm * qCos(rad);
                sample.hasSkyPosition = true;
                sample.valid = false;
                tiles.append(sample);
            }
        ...
        if (useElevation) {
            appendAltAzHoneycombPreview();
        } else {
            appendAzRingPreview();
        }
    }

    const bool interp = m_chkRadioTelescopeInterpolate != nullptr && m_chkRadioTelescopeInterpolate->isChecked();
    const bool noRotatorBench = m_chkRadioTelescopeAllowNoRotator != nullptr && m_chkRadioTelescopeAllowNoRotator->isChecked();
    const QString summary = QStringLiteral("Slice %1-%2 Hz · BW %3 Hz\nBeam %4° · Dwell %5 ms · Settle %6 ms\n%7%8")
                                .arg(m_spinRadioTelescopeLowHz != nullptr ? m_spinRadioTelescopeLowHz->value() : RadioTelescopeMode::defaultLowFrequencyHz())
                                .arg(m_spinRadioTelescopeHighHz != nullptr ? m_spinRadioTelescopeHighHz->value() : RadioTelescopeMode::defaultHighFrequencyHz())
                                .arg((m_spinRadioTelescopeLowHz != nullptr && m_spinRadioTelescopeHighHz != nullptr)
                                    ? qAbs(m_spinRadioTelescopeHighHz->value() - m_spinRadioTelescopeLowHz->value())
                                    : qAbs(RadioTelescopeMode::defaultHighFrequencyHz() - RadioTelescopeMode::defaultLowFrequencyHz()))
                                .arg(m_spinRadioTelescopeBeamWidthDeg != nullptr ? QString::number(m_spinRadioTelescopeBeamWidthDeg->value(), 'f', 1) : QStringLiteral("15.0"))
                                .arg(m_spinRadioTelescopeDwellMs != nullptr ? m_spinRadioTelescopeDwellMs->value() : RadioTelescopeMode::defaultDwellMs())
                                .arg(m_spinRadioTelescopeSettleMs != nullptr ? m_spinRadioTelescopeSettleMs->value() : 0)
                                .arg(m_radioTelescopeScanActive ? uiText("radio_telescope_overlay_scanning", "Scanning active") : uiText("radio_telescope_overlay_idle", "Waiting"))
                                .arg(noRotatorBench ? QStringLiteral("\nDRY-RUN: rotator disabled") : QString());

    if (m_lblRadioTelescopeSummary != nullptr) {
        m_lblRadioTelescopeSummary->setText(summary);
    }

    m_radioTelescopeHeatmapWidget->setScanActive(m_radioTelescopeScanActive);
    m_radioTelescopeHeatmapWidget->setTiles(tiles, useElevation, interp);
    m_radioTelescopeHeatmapWidget->setCurrentTarget(m_radioTelescopeTargetAzimuthDeg, m_radioTelescopeTargetElevationDeg, m_radioTelescopeScanActive);
    m_radioTelescopeHeatmapWidget->setOverlayText(QString());
}

bool MainWindow::radioTelescopeRotatorReadyForScan(QString *reason) const
{
    const bool benchNoRotator = m_chkRadioTelescopeAllowNoRotator != nullptr && m_chkRadioTelescopeAllowNoRotator->isChecked();
    if (benchNoRotator) {
        if (reason != nullptr) {
            *reason = uiText("radio_telescope_bench_warning", "Bench test mode: no rotator required; heatmap is not a real sky map.");
        }
        return true;
    }
    ...
    if (m_catRotatorController == nullptr) {
        if (reason != nullptr) {
            *reason = uiText("radio_telescope_no_rotator_controller", "No rotator controller is available. Connect/configure a rotator or explicitly enable bench test without rotator.");
        }
        return false;
    }
    ...
    if (!cfg.enabled) {
        if (reason != nullptr) {
            *reason = uiText("radio_telescope_rotator_disabled", "Rotator is disabled for the current band/profile. Enable the rotator before starting a sky scan.");
        }
        return false;
    }
    ...
    if (!m_catRotatorController->isConnected()) {
        if (reason != nullptr) {
            *reason = uiText("radio_telescope_rotator_disconnected", "Rotator is not connected. Connect the selected rotator before starting a sky scan.");
        }
        return false;
    }

    const bool requestedAltAz = m_cmbRadioTelescopeRotatorMode == nullptr ||
                                m_cmbRadioTelescopeRotatorMode->currentData().toString() != QStringLiteral("az");
    if (requestedAltAz && !cfg.useElevation) {
        if (reason != nullptr) {
            *reason = uiText("radio_telescope_altaz_requires_el", "Alt-Az sky disk requires an elevation-capable rotator. Choose Azimuth ring or enable elevation in the rotator profile.");
        }
        return false;
    }

    if (reason != nullptr) {
        *reason = uiText("radio_telescope_rotator_ready", "Rotator ready for Radio Telescope scan.");
    }
    return true;
}

bool MainWindow::radioTelescopeTileWithinRotatorLimits(double azimuthDeg, double elevationDeg, bool useElevation) const
{
    const bool benchNoRotator = m_chkRadioTelescopeAllowNoRotator != nullptr &&
                                m_chkRadioTelescopeAllowNoRotator->isChecked();
    if (benchNoRotator || m_catRotatorController == nullptr) {
        return true;
    }

    const mm::CatRotatorController::Config cfg = m_catRotatorController->config();
    const double azSpan = cfg.azimuthMaxDeg - cfg.azimuthMinDeg;
    if (azSpan < 359.0) {
        if (azimuthDeg < cfg.azimuthMinDeg - 0.05 || azimuthDeg > cfg.azimuthMaxDeg + 0.05) {
            return false;
        }
    }

    if (useElevation) {
        if (elevationDeg < cfg.elevationMinDeg - 0.05 || elevationDeg > cfg.elevationMaxDeg + 0.05) {
            return false;
        }
    }
...
void MainWindow::startRadioTelescopeScan()
{
    if (m_radioTelescopeScanActive) {
        return;
    }
    if (ui == nullptr || ui->cmbMode == nullptr || !RadioTelescopeMode::isMode(ui->cmbMode->currentText())) {
        return;
    }
    if (!m_rxRunning && (m_audioEngine == nullptr || !m_audioEngine->isRunning())) {
        appendLog(QStringLiteral("Radio Telescope scan start deferred: RX is not running yet."));
        return;
    }

    QString rotatorReason;
    if (!radioTelescopeRotatorReadyForScan(&rotatorReason)) {
        appendLog(QStringLiteral("Radio Telescope scan blocked: ") + rotatorReason);
        if (m_lblRadioTelescopeStatus != nullptr) {
            m_lblRadioTelescopeStatus->setText(rotatorReason);
        }
        return;
    }
    ...
    const bool benchNoRotator = m_chkRadioTelescopeAllowNoRotator != nullptr &&
                                m_chkRadioTelescopeAllowNoRotator->isChecked();
    const bool rotatorOnline = !benchNoRotator &&
                               m_catRotatorController != nullptr &&
                               m_catRotatorController->config().enabled &&
                               m_catRotatorController->isConnected();
    if (rotatorOnline) {
        m_radioTelescopeReturnAzimuthDeg = m_catRotatorController->currentAzimuth();
        m_radioTelescopeReturnElevationDeg = m_catRotatorController->currentElevation();
        m_radioTelescopeReturnPositionValid = true;
        appendLog(QStringLiteral("Radio Telescope scan: saved pre-scan rotator position az %1°, el %2°.")
    ...
    const double beam = (m_spinRadioTelescopeBeamWidthDeg != nullptr)
                            ? m_spinRadioTelescopeBeamWidthDeg->value()
                            : RadioTelescopeMode::defaultBeamWidthDeg();
    const bool useElevation = m_cmbRadioTelescopeRotatorMode == nullptr ||
                              m_cmbRadioTelescopeRotatorMode->currentData().toString() != QStringLiteral("az");

    int skippedByLimits = 0;

    auto rotatePlanNearPark = [&]() {
        if (!rotatorOnline || m_radioTelescopeTiles.isEmpty() || m_catRotatorController == nullptr) {
            return;
        }
        const mm::CatRotatorController::Config cfg = m_catRotatorController->config();
        const double parkAz = cfg.parkAzimuth;
        const double parkEl = useElevation ? cfg.parkElevation : 0.0;
        auto azError = [](double a, double b) {
            while (a < 0.0) a += 360.0;
            while (a >= 360.0) a -= 360.0;
            while (b < 0.0) b += 360.0;
        ...
        int nearest = 0;
        double best = azError(m_radioTelescopeTiles[0].azimuthDeg, parkAz) +
                      (useElevation ? qAbs(m_radioTelescopeTiles[0].elevationDeg - parkEl) : 0.0);
        for (int i = 1; i < m_radioTelescopeTiles.size(); ++i) {
            const double score = azError(m_radioTelescopeTiles[i].azimuthDeg, parkAz) +
                                 (useElevation ? qAbs(m_radioTelescopeTiles[i].elevationDeg - parkEl) : 0.0);
            if (score < best) {
                best = score;
                nearest = i;
            }
        ...
        if (nearest > 0) {
            std::rotate(m_radioTelescopeTiles.begin(), m_radioTelescopeTiles.begin() + nearest, m_radioTelescopeTiles.end());
        }
    };

    if (useElevation) {
        // Build the real scan plan from a true pointy-top honeycomb in projected
        // sky coordinates.  Every hexagon center is converted back to Az/El and
        // commanded to the rotator; the renderer receives the same skyX/skyY
        // center.  This avoids the previous mismatch "angular step first, hex
        // near point second" that could skip or visually misplace cells.
        const double hexRadius = qBound(0.015, beam / (90.0 * qSqrt(3.0)), 0.45);
        const double dx = qSqrt(3.0) * hexRadius;
        const double dy = 1.5 * hexRadius;
        int row = 0;
        for (double y = -1.0; y <= 1.0 + 1e-9; y += dy, ++row) {
            QVector<RadioTelescopeTileRecord> rowTiles;
            const double offset = (row % 2 == 0) ? 0.0 : dx * 0.5;
            for (double x = -1.0 - dx; x <= 1.0 + dx; x += dx) {
                const double cx = x + offset;
                const double cy = y;
                ...
                RadioTelescopeTileRecord tile;
                tile.skyX = cx;
                tile.skyY = cy;
                tile.hasSkyPosition = true;
                tile.azimuthDeg = std::fmod(qRadiansToDegrees(std::atan2(cx, -cy)) + 360.0, 360.0);
                tile.elevationDeg = qBound(0.0, 90.0 * (1.0 - rr), 90.0);
                tile.beamWidthDeg = beam;
                if (radioTelescopeTileWithinRotatorLimits(tile.azimuthDeg, tile.elevationDeg, useElevation)) {
            ...
            for (const RadioTelescopeTileRecord &tile : std::as_const(rowTiles)) {
                m_radioTelescopeTiles.append(tile);
            }
        }
        rotatePlanNearPark();
    } else {
        QVector<RadioTelescopeTileRecord> ringTiles;
        const int count = qMax(1, static_cast<int>(std::ceil(360.0 / qMax(2.0, beam))));
        const double step = 360.0 / static_cast<double>(count);
        constexpr double ringNorm = 0.79;
        for (int i = 0; i < count; ++i) {
            RadioTelescopeTileRecord tile;
            tile.azimuthDeg = std::fmod((static_cast<double>(i) + 0.5) * step, 360.0);
            tile.elevationDeg = 0.0;
            tile.beamWidthDeg = step;
            const double rad = qDegreesToRadians(tile.azimuthDeg);
            tile.skyX = ringNorm * qSin(rad);
            tile.skyY = -ringNorm * qCos(rad);
            tile.hasSkyPosition = true;
            if (radioTelescopeTileWithinRotatorLimits(tile.azimuthDeg, tile.elevationDeg, useElevation)) {
                ringTiles.append(tile);
            } else {
                ++skippedByLimits;
            }
        }

        for (const RadioTelescopeTileRecord &tile : std::as_const(ringTiles)) {
            m_radioTelescopeTiles.append(tile);
        }
        rotatePlanNearPark();
    }
    ...
    if (m_radioTelescopeTiles.isEmpty()) {
        const QString msg = uiText("radio_telescope_no_tiles_in_limits", "Radio Telescope scan blocked: no tile is inside the configured rotator limits/endpoints.");
        appendLog(msg);
        if (m_lblRadioTelescopeStatus != nullptr) {
            m_lblRadioTelescopeStatus->setText(msg);
        }
        return;
    }
    ...
    if (rotatorOnline) {
        const mm::CatRotatorController::Config cfg = m_catRotatorController->config();
        m_radioTelescopeTargetAzimuthDeg = cfg.parkAzimuth;
        m_radioTelescopeTargetElevationDeg = useElevation ? cfg.parkElevation : 0.0;
        m_radioTelescopeScanPhase = RadioTelescopeScanPhase::MovingToPark;
        const int etaMs = qMax(1000, m_catRotatorController->estimatePointingTimeMs(m_radioTelescopeTargetAzimuthDeg,
                                                                                     m_radioTelescopeTargetElevationDeg));
        const int settleMs = (m_spinRadioTelescopeSettleMs != nullptr) ? m_spinRadioTelescopeSettleMs->value() : cfg.settleDelayMs;
        m_radioTelescopePhaseDeadlineMs = QDateTime::currentMSecsSinceEpoch() + etaMs + settleMs;
        m_catRotatorController->setAzEl(m_radioTelescopeTargetAzimuthDeg,
                                        m_radioTelescopeTargetElevationDeg,
        ...
        appendLog(QStringLiteral("Radio Telescope scan: moving to park/start position az %1°, el %2° before first tile.")
                      .arg(QString::number(m_radioTelescopeTargetAzimuthDeg, 'f', 1),
                           QString::number(m_radioTelescopeTargetElevationDeg, 'f', 1)));
    }

    updateRadioTelescopeHeatmapWidget();
    advanceRadioTelescopeScan();
    m_radioTelescopeScanTimer.start();
    appendLog(QStringLiteral("Radio Telescope scan started: %1 tiles%2.")
                  .arg(m_radioTelescopeTiles.size())
                  .arg(skippedByLimits > 0 ? QStringLiteral(", %1 skipped by rotator endpoints/limits").arg(skippedByLimits) : QString()));
}

void MainWindow::stopRadioTelescopeScan(bool automatic)
{
    if (!m_radioTelescopeScanActive && !m_radioTelescopeScanTimer.isActive()) {
        return;
    }
    m_radioTelescopeScanTimer.stop();
    if (m_catRotatorController != nullptr && m_catRotatorController->isConnected()) {
        m_catRotatorController->stop();
    }
    m_radioTelescopeScanActive = false;
    m_radioTelescopeScanPhase = RadioTelescopeScanPhase::Idle;
    m_radioTelescopeCurrentTileIndex = -1;
    if (m_lblRadioTelescopeStatus != nullptr) {
        m_lblRadioTelescopeStatus->setText(automatic
            ? uiText("radio_telescope_status_done", "Scan complete: automatic stop.")
    ...
    if (automatic && m_chkRadioTelescopeAutoPointPeak != nullptr && m_chkRadioTelescopeAutoPointPeak->isChecked()) {
        if (!m_radioTelescopePeakRefineActive) {
            m_radioTelescopePeakRefineStepsRemaining = 8;
        }
        if (continueRadioTelescopePeakRefinement()) {
            return;
        }
    ...
    if (automatic && m_radioTelescopeReturnPositionValid &&
        m_catRotatorController != nullptr && m_catRotatorController->isConnected()) {
        m_catRotatorController->setAzEl(m_radioTelescopeReturnAzimuthDeg,
                                        m_radioTelescopeReturnElevationDeg,
                                        QStringLiteral("radio telescope return to pre-scan position"));
        appendLog(QStringLiteral("Radio Telescope scan: returning rotator to pre-scan position az %1°, el %2°.")
                      .arg(QString::number(m_radioTelescopeReturnAzimuthDeg, 'f', 1),
                           QString::number(m_radioTelescopeReturnElevationDeg, 'f', 1)));
        if (m_lblRadioTelescopeStatus != nullptr) {
        ...
        // Manual stop must not surprise the operator by moving the antenna.
        m_radioTelescopeReturnPositionValid = false;
    }

    appendLog(automatic ? QStringLiteral("Radio Telescope scan completed.") : QStringLiteral("Radio Telescope scan stopped by operator."));

    if (automatic && ui != nullptr && ui->cmbMode != nullptr && RadioTelescopeMode::isMode(ui->cmbMode->currentText()) &&
        m_audioEngine != nullptr && m_audioEngine->isRunning()) {
        appendLog(QStringLiteral("Radio Telescope scan complete: stopping RX audio capture."));
        stopRx();
    }
}

void MainWindow::advanceRadioTelescopeScan()
{
    if (!m_radioTelescopeScanActive || m_radioTelescopeTiles.isEmpty()) {
        return;
    }

    const qint64 nowMs = QDateTime::currentMSecsSinceEpoch();
    const int dwellMs = (m_spinRadioTelescopeDwellMs != nullptr) ? m_spinRadioTelescopeDwellMs->value() : RadioTelescopeMode::defaultDwellMs();
    const int settleMs = (m_spinRadioTelescopeSettleMs != nullptr) ? m_spinRadioTelescopeSettleMs->value() : 0;
    const bool useElevation = m_cmbRadioTelescopeRotatorMode == nullptr || m_cmbRadioTelescopeRotatorMode->currentData().toString() != QStringLiteral("az");

    if (m_radioTelescopeScanPhase == RadioTelescopeScanPhase::MovingToPark) {
        if (m_catRotatorController == nullptr || !m_catRotatorController->isConnected()) {
            QString reason;
            radioTelescopeRotatorReadyForScan(&reason);
            appendLog(QStringLiteral("Radio Telescope scan aborted before first tile: ") + reason);
            if (m_lblRadioTelescopeStatus != nullptr) {
                m_lblRadioTelescopeStatus->setText(reason);
            }
            stopRadioTelescopeScan(false);
        ...
        const bool ready = m_catRotatorController->isReadyForTarget(m_radioTelescopeTargetAzimuthDeg,
                                                                    m_radioTelescopeTargetElevationDeg) ||
                           nowMs >= m_radioTelescopePhaseDeadlineMs;
        if (!ready) {
            return;
        }

        m_radioTelescopeScanPhase = RadioTelescopeScanPhase::Idle;
        m_radioTelescopeCurrentTileIndex = 0;
        if (m_lblRadioTelescopeStatus != nullptr) {
            m_lblRadioTelescopeStatus->setText(uiText("radio_telescope_park_ready", "Park/start position reached; starting sky scan."));
        }
        appendLog(QStringLiteral("Radio Telescope scan: park/start position reached; scanning first tile."));
    }

    if (m_radioTelescopeScanPhase == RadioTelescopeScanPhase::Settling) {
        if (nowMs < m_radioTelescopePhaseDeadlineMs) {
            return;
        }
        m_radioTelescopeScanPhase = RadioTelescopeScanPhase::Sampling;
        m_radioTelescopePhaseDeadlineMs = nowMs + dwellMs;
        m_radioTelescopeAccumulatedPower = 0.0;
        m_radioTelescopeAccumulatedBlocks = 0;
        if (m_lblRadioTelescopeStatus != nullptr) {
    ...
    if (m_radioTelescopeScanPhase == RadioTelescopeScanPhase::Sampling) {
        if (nowMs < m_radioTelescopePhaseDeadlineMs) {
            return;
        }
        if (m_radioTelescopeCurrentTileIndex >= 0 && m_radioTelescopeCurrentTileIndex < m_radioTelescopeTiles.size()) {
            RadioTelescopeTileRecord &tile = m_radioTelescopeTiles[m_radioTelescopeCurrentTileIndex];
            const double avgPower = (m_radioTelescopeAccumulatedBlocks > 0)
                                        ? (m_radioTelescopeAccumulatedPower / static_cast<double>(m_radioTelescopeAccumulatedBlocks))
                                        : 1.0e-12;
            tile.noiseDb = 10.0 * std::log10(qMax(avgPower, 1.0e-12));
            tile.measured = true;
            if (m_tableRadioTelescopeSamples != nullptr) {
                const int row = m_tableRadioTelescopeSamples->rowCount();
                m_tableRadioTelescopeSamples->insertRow(row);
                m_tableRadioTelescopeSamples->setItem(row, 0, new QTableWidgetItem(QString::number(tile.azimuthDeg, 'f', 0)));
                m_tableRadioTelescopeSamples->setItem(row, 1, new QTableWidgetItem(QString::number(tile.elevationDeg, 'f', 0)));
                m_tableRadioTelescopeSamples->setItem(row, 2, new QTableWidgetItem(QString::number(tile.noiseDb, 'f', 1) + QStringLiteral(" dBFS")));
                m_tableRadioTelescopeSamples->setItem(row, 3, new QTableWidgetItem(QDateTime::currentDateTimeUtc().toString(QStringLiteral("HH:mm:ss"))));
                m_tableRadioTelescopeSamples->scrollToBottom();
            }
        }
        updateRadioTelescopeHeatmapWidget();
    ...
    if (m_radioTelescopeCurrentTileIndex < 0) {
        m_radioTelescopeCurrentTileIndex = 0;
    }

    if (m_radioTelescopeCurrentTileIndex >= m_radioTelescopeTiles.size()) {
        stopRadioTelescopeScan(true);
        return;
    }

    RadioTelescopeTileRecord &tile = m_radioTelescopeTiles[m_radioTelescopeCurrentTileIndex];
    if (m_radioTelescopeScanPhase == RadioTelescopeScanPhase::Idle) {
        m_radioTelescopeTargetAzimuthDeg = tile.azimuthDeg;
        m_radioTelescopeTargetElevationDeg = useElevation ? tile.elevationDeg : 0.0;

        const bool benchNoRotator = m_chkRadioTelescopeAllowNoRotator != nullptr && m_chkRadioTelescopeAllowNoRotator->isChecked();
        if (m_catRotatorController != nullptr && m_catRotatorController->config().enabled && m_catRotatorController->isConnected()) {
            m_catRotatorController->setAzEl(m_radioTelescopeTargetAzimuthDeg, m_radioTelescopeTargetElevationDeg, QStringLiteral("radio telescope tile scan"));
            const int etaMs = qMax(1000, m_catRotatorController->estimatePointingTimeMs(m_radioTelescopeTargetAzimuthDeg,
                                                                                         m_radioTelescopeTargetElevationDeg));
            ...
            QString reason;
            radioTelescopeRotatorReadyForScan(&reason);
            appendLog(QStringLiteral("Radio Telescope scan aborted: ") + reason);
            if (m_lblRadioTelescopeStatus != nullptr) {
                m_lblRadioTelescopeStatus->setText(reason);
            }
            stopRadioTelescopeScan(false);
    ...
    if (m_radioTelescopeScanPhase == RadioTelescopeScanPhase::WaitingForRotator) {
        if (m_catRotatorController == nullptr || !m_catRotatorController->config().enabled || !m_catRotatorController->isConnected()) {
            QString reason;
            radioTelescopeRotatorReadyForScan(&reason);
            appendLog(QStringLiteral("Radio Telescope scan aborted: ") + reason);
        ...
        const bool ready = m_catRotatorController->isReadyForTarget(m_radioTelescopeTargetAzimuthDeg, m_radioTelescopeTargetElevationDeg) ||
                           nowMs >= m_radioTelescopePhaseDeadlineMs;
        if (ready) {
            m_radioTelescopeScanPhase = RadioTelescopeScanPhase::Settling;
            m_radioTelescopePhaseDeadlineMs = nowMs + settleMs;
...
void MainWindow::handleRadioTelescopeRotatorPosition(double azimuthDeg, double elevationDeg)
{
    m_radioTelescopeCurrentAzimuthDeg = azimuthDeg;
    m_radioTelescopeCurrentElevationDeg = elevationDeg;
    if (m_radioTelescopeHeatmapWidget != nullptr && !m_radioTelescopeScanActive) {
        m_radioTelescopeHeatmapWidget->setCurrentTarget(azimuthDeg, elevationDeg, false);
    }
}

double MainWindow::measureRadioTelescopeBandPower(const AudioBlock &block) const
{
    if (block.samples.isEmpty() || m_spinRadioTelescopeLowHz == nullptr || m_spinRadioTelescopeHighHz == nullptr) {
        return 0.0;
    }

    const int lowHz = qMin(m_spinRadioTelescopeLowHz->value(), m_spinRadioTelescopeHighHz->value());
    const int highHz = qMax(m_spinRadioTelescopeLowHz->value(), m_spinRadioTelescopeHighHz->value());
    return radioTelescopeWelchBandPower(block.samples, qMax(1, block.sampleRate), lowHz, highHz);
}
...
void MainWindow::setupMsk144Page()
{
    ...
    for (const int hz : {50, 100, 200, 500}) {
        m_cmbMsk144FrequencyTolerance->addItem(QStringLiteral("%1 Hz").arg(hz), hz);
    }
    const int savedMskFTol = QSettings(AppSettings::settingsFilePath(), QSettings::IniFormat)
                                 .value(QStringLiteral("MSK144/fToleranceHz"), 200).toInt();
    int savedMskFTolIndex = m_cmbMsk144FrequencyTolerance->findData(savedMskFTol);
    if (savedMskFTolIndex < 0) savedMskFTolIndex = m_cmbMsk144FrequencyTolerance->findData(200);
    m_cmbMsk144FrequencyTolerance->setCurrentIndex(savedMskFTolIndex);
    ...
    ++row;
    grid->addWidget(new QLabel(uiText("msk144_f_tolerance", "F Tol"), rxGroup), row, 0);
    grid->addWidget(m_cmbMsk144FrequencyTolerance, row, 1);
    ...
    connect(m_cmbMsk144FrequencyTolerance, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this, apply]() {
        QSettings settings(AppSettings::settingsFilePath(), QSettings::IniFormat);
        settings.setValue(QStringLiteral("MSK144/fToleranceHz"),
                          m_cmbMsk144FrequencyTolerance != nullptr ? m_cmbMsk144FrequencyTolerance->currentData().toInt() : 200);
        apply();
    });
...
void MainWindow::setupFt8Page()
{
    ...
    m_chkFt8CallerQueue = new QCheckBox(uiText("ft8_queue_callers", "Manage queued callers"), controlGroup);
    m_chkFt8CallerQueue->setToolTip(uiText("ft8_queue_callers_tooltip",
    ...
    controlGrid->addWidget(m_chkFt8CallerQueue, 3, 0, 1, 3);
    controlGrid->addWidget(m_lblFt8SessionQueue, 3, 3, 1, 3);
    updateFt8SessionQueueUi();
    ...
    QGroupBox *clockGroup = new QGroupBox(uiText("ft_period_clock", "FT periods"), m_pageFt8Settings);
    ...
    sequencerLayout->addWidget(clockGroup);
    ...
    // FT WAV analysis is kept as a manual diagnostic tool; bundled auto-tests were removed from production builds.
    ...
    settingsLayout->addWidget(sequencerGroup);
...
void MainWindow::loadFt8SettingsToUi()
{
    ...
    const QSignalBlocker blockCallerQueue(m_chkFt8CallerQueue);
    ...
    if (m_chkFt8CallerQueue != nullptr) {
        m_chkFt8CallerQueue->setChecked(m_settings.ft8QueueCallers);
    }
    updateFt8SessionQueueUi();
...
void MainWindow::setupHelpTooltips()
{
    ...
    if (m_spinCwToneHz != nullptr) {
        ...
        setHelpText(m_chkCwAutoWpmB, uiText("cw_auto_wpm_help", "Estimate Morse speed continuously. The speed field is used as the acquisition hint until enough timing evidence is available."));
        setHelpText(m_spinCwTxWpm, uiText("cw_tx_wpm_help", "CW transmit speed. This is independent from the automatic RX A/RX B speed estimates."));
        setHelpText(m_spinCwBandwidthHz, uiText("cw_bandwidth_help", "Manual CW selectivity used when Auto is off. Acquisition remains wider so AFC can still find the keyed carrier."));
        if (m_chkCwAutoBandwidth != nullptr)
            setHelpText(m_chkCwAutoBandwidth, uiText("cw_auto_bandwidth_help", "Choose selectivity independently for RX A and RX B from WPM, lock quality and nearby carriers."));
        setHelpText(m_chkCwAfc, uiText("cw_afc_help", "Track small tone offsets and drift independently for RX A and RX B without moving the operator markers."));
        setHelpText(m_spinCwAfcRangeHz, uiText("cw_afc_range_help", "Maximum internal AFC correction around each CW marker. Use a small range in crowded bands to avoid following a neighbouring station."));
        setHelpText(m_txtCwRx, uiText("cw_rx_a_help", "Decoded Morse text from the exact green RX A receiver."));
        setHelpText(m_txtCwRxB, uiText("cw_rx_b_help", "Decoded Morse text from the exact blue RX B receiver."));
        setHelpText(m_txtCwTx, uiText("cw_tx_help", "Text to transmit as CW. Unsupported characters are skipped or converted to spacing."));
        setHelpText(m_btnCwSend, uiText("cw_send_help", "Transmit the CW text, or stop the current CW transmission when already sending."));
        setHelpText(m_btnCwClearRx, uiText("cw_clear_rx_a_help", "Clear only RX A text and reset RX A acquisition; RX B keeps running."));
        setHelpText(m_btnCwClearRxB, uiText("cw_clear_rx_b_help", "Clear only RX B text and reset RX B acquisition; RX A keeps running."));
    }
...
void MainWindow::setupProcessingConnections()
{
    ...
    connect(m_txAudioEngine, &TxAudioEngine::audioBlockReady,
            this, [this](const AudioBlock &block) {
                ...
                // FT4/FT8 TX is a generated GFSK waveform, not receiver audio.
                // Feeding it through the RX waterfall leveler produces large
                // synthetic symmetric skirts and a white column that look like
                // RF splatter even though they are only a local display artefact.
                // Keep the last RX waterfall frame frozen throughout FT TX.
                if (ui != nullptr && ui->cmbMode != nullptr &&
    ...
    if (m_ftTxWorker != nullptr) {
        // The dedicated FT worker must never feed its locally generated TX
        // waveform into the RX waterfall.  The waterfall remains an RX
        // instrument and freezes while the audio input is stopped for TX.
    ...
    // RX A/B text originates from the two native exact-tone receivers. The
    // full-passband scanner supplies carrier lanes only, never random glyphs.
    ...
    connect(m_cwDecoder, &CwDecoder::skimmerOverlaysChanged,
            this, [this](const QStringList &, const QVector<double> &, const QVector<float> &) {
                if (m_waterfallWidget == nullptr || ui == nullptr || ui->cmbMode == nullptr ||
                    ui->cmbMode->currentText() != CwDecoder::modeName()) {
                    return;
                }
                // Exact-tone and future full-band skimmer text is presented as
                // continuous lane text in the CW panes.  Random single glyphs
                // over the waterfall are intentionally disabled.
                m_waterfallWidget->clearVerticalTextTrails();
                m_waterfallWidget->setTextOverlays({});
            },
            Qt::QueuedConnection);

    connect(m_cwDecoder, &CwDecoder::diagnosticSamplesReady,
            this, [this](int rank, const QVector<CwDiagnosticPoint> &samples) {
                if (m_cwDiagnosticWidget != nullptr)
                    m_cwDiagnosticWidget->appendSamples(rank, samples);
            }, Qt::QueuedConnection);
    connect(m_cwDecoder, &CwDecoder::diagnosticSpectrumReady,
            this, [this](int rank, const QVector<float> &offsetsHz,
                ...
                if (m_cwDiagnosticWidget != nullptr)
                    m_cwDiagnosticWidget->setSpectrum(
                        rank, offsetsHz, inputPsdDb, filteredPsdDb,
    ...
    connect(m_cwDecoder, &CwDecoder::diagnosticResetRequested,
            this, [this](int rank) {
                if (m_cwDiagnosticWidget != nullptr)
    ...
    connect(m_cwDecoder, &CwDecoder::statusChanged,
            this, &MainWindow::handleWeatherFaxStatus);
    connect(m_cwDecoder, &CwDecoder::statusChanged,
            this, [this](const QString &) { updateCwDualRxStatusLabel(); },
            Qt::QueuedConnection);
    ...
    connect(m_cwDecoder, &CwDecoder::speedEstimateChanged,
            this, [this](double wpm) {
                if (!(wpm > 0.1) || !std::isfinite(wpm)) return;
                m_cwTrackedWpmA = wpm;
                if (m_lblCwTrackedWpmA != nullptr) {
                    m_lblCwTrackedWpmA->setText(QString::number(wpm, 'f', 1));
                }
                updateCwDualRxStatusLabel();
                updateWaterfallMarkers();
            });

    connect(m_cwDecoder, &CwDecoder::receiverSpeedEstimateChanged,
            this, [this](int rank, double wpm) {
                if (!(wpm > 0.1) || !std::isfinite(wpm)) return;
                if (rank <= 0) {
                    m_cwTrackedWpmA = wpm;
                    if (m_lblCwTrackedWpmA != nullptr) {
                    ...
                    m_cwTrackedWpmB = wpm;
                    if (m_lblCwTrackedWpmB != nullptr) {
                        m_lblCwTrackedWpmB->setText(m_cwSecondaryEnabled
                                                        ? QString::number(wpm, 'f', 1)
                                                        : QStringLiteral("--"));
                    }
                }
                updateCwDualRxStatusLabel();
    ...
    connect(m_ft8RxDecoder, &Ft8RxDecoder::decodeBatchStarted,
            this, &MainWindow::handleFt8DecodeBatchStarted,
            Qt::QueuedConnection);
    connect(m_ft8RxDecoder, &Ft8RxDecoder::decodeReady,
            this, &MainWindow::handleFt8DecodeReady,
            Qt::QueuedConnection);
    connect(m_ft8RxDecoder, &Ft8RxDecoder::decodeBatchFinished,
            this, &MainWindow::handleFt8DecodeBatchFinished,
            Qt::QueuedConnection);
...
void MainWindow::applyUiLanguage()
{
    ...
    if (m_btnCwClearRx != nullptr) m_btnCwClearRx->setText(uiText("cw_clear_rx_a", "Clear RX A"));
    if (m_btnCwClearRxB != nullptr) m_btnCwClearRxB->setText(uiText("cw_clear_rx_b", "Clear RX B"));
    if (m_txtCwRx != nullptr) m_txtCwRx->setPlaceholderText(uiText("cw_rx_a_placeholder", "Decoded CW from the green RX A marker appears here."));
    if (m_txtCwRxB != nullptr) m_txtCwRxB->setPlaceholderText(uiText("cw_rx_b_placeholder", "Decoded CW from the blue RX B marker appears here. Right-click the waterfall to enable or retune RX B."));
    if (m_cwDisplayTabs != nullptr && m_cwDiagnosticWidget != nullptr) {
        const int diagIndex = m_cwDisplayTabs->indexOf(m_cwDiagnosticWidget);
        if (diagIndex >= 0) m_cwDisplayTabs->setTabText(diagIndex, uiText("cw_diagnostics_tab", "Diagnostics"));
        m_cwDiagnosticWidget->retranslateUi();
    }
    if (m_chkFt8CallerQueue != nullptr) {
        m_chkFt8CallerQueue->setText(uiText("ft8_queue_callers", "Manage queued callers"));
        m_chkFt8CallerQueue->setToolTip(uiText("ft8_queue_callers_tooltip",
            "Keeps stations that call you directly while another FT QSO is active in a FIFO queue. MM still transmits only one QSO at a time and automatically serves the next caller after completion."));
    }
    if (m_lblFt8SessionQueue != nullptr) {
        m_lblFt8SessionQueue->setToolTip(uiText("ft8_session_stats_tooltip",
            "Counts completed FT4/FT8 QSOs since MadModem was started and shows callers waiting in the passive FIFO."));
        updateFt8SessionQueueUi();
    }
...
void MainWindow::setupUiConnections()
{
    ...
    m_actionStartRxRecording = new QAction(this);
    m_actionStopRxRecording = new QAction(this);
    m_actionStartRxRecording->setIcon(QIcon::fromTheme(QStringLiteral("media-record")));
    m_actionStopRxRecording->setIcon(style()->standardIcon(QStyle::SP_MediaStop));
    ...
    if (ui->menuFile != nullptr) {
        ...
        ui->menuFile->insertAction(ui->actionExit, m_actionStartRxRecording);
        ui->menuFile->insertAction(ui->actionExit, m_actionStopRxRecording);
        ui->menuFile->insertSeparator(ui->actionExit);
    }
    ...
    connect(m_actionStartRxRecording, &QAction::triggered,
            this, &MainWindow::startRxAudioRecording);
    connect(m_actionStopRxRecording, &QAction::triggered,
            this, &MainWindow::stopRxAudioRecording);
    updateRxAudioRecordingActions();
    ...
    for (QSpinBox *spin : {m_spinCwWpmA, m_spinCwWpmB, m_spinCwTxWpm}) {
        if (spin != nullptr) {
    ...
    if (m_chkCwAutoWpmA != nullptr) {
        connect(m_chkCwAutoWpmA, &QCheckBox::toggled,
                this, [this](bool checked) {
                    if (m_spinCwWpmA != nullptr) m_spinCwWpmA->setEnabled(!checked);
                    applyCwSettings();
                });
    }
    if (m_chkCwAutoWpmB != nullptr) {
        connect(m_chkCwAutoWpmB, &QCheckBox::toggled,
                this, [this](bool checked) {
                    if (m_spinCwWpmB != nullptr) m_spinCwWpmB->setEnabled(!checked);
                    applyCwSettings();
                });
    }
    ...
    if (m_chkCwAutoBandwidth != nullptr) {
        connect(m_chkCwAutoBandwidth, &QCheckBox::toggled,
                this, [this](bool checked) {
                    if (m_spinCwBandwidthHz != nullptr) m_spinCwBandwidthHz->setEnabled(!checked);
                    applyCwSettings();
                });
    }
    ...
    if (m_btnCwClearRxB != nullptr) {
        connect(m_btnCwClearRxB, &QPushButton::clicked,
                this, &MainWindow::clearCwRxBText);
    }
    ...
    if (m_cmbFt8Band != nullptr) {
        connect(m_cmbFt8Band, QOverload<int>::of(&QComboBox::currentIndexChanged),
                this, [this](int) {
                    ...
                    updateFtBandFrequencyUi();
                    updateCatRotatorQsoTarget(QStringLiteral("FT band changed"));
                });
    }
    ...
    if (m_tableFt8QsoHistory != nullptr) {
        m_tableFt8QsoHistory->setToolTip(uiText("ft_history_activation_tooltip",
            "Double-click a valid FT QSO activity row to select its correspondent/message. Press TX explicitly to transmit."));
    ...
    if (m_chkFt8CallerQueue != nullptr) {
        connect(m_chkFt8CallerQueue, &QCheckBox::toggled, this, [this](bool enabled) {
            m_settings.ft8QueueCallers = enabled;
            if (!enabled) {
                clearFt8CallerQueue(QStringLiteral("operator disabled caller queue"));
            } else {
                updateFt8SessionQueueUi();
                QTimer::singleShot(0, this, [this]() { activateNextFt8QueuedCaller(); });
            }
            applyFt8Settings();
        });
    }
    ...
    connect(m_audioEngine, &AudioEngine::diagnosticMessage,
            this, [this](const QString &message) {
                appendLog(message);
            });

    if (m_rxAudioRecorder != nullptr) {
        connect(m_rxAudioRecorder, &RxAudioRecorder::recordingStarted,
                this, [this](const QString &fileName, int sampleRate) {
                    m_rxAudioRecordingActive = true;
                    m_pendingRxRecordingPath.clear();
                    appendLog(uiText("log.rx_recording_started",
                                     "RX audio recording started:") +
                              QStringLiteral(" %1 (%2 Hz mono PCM)")
                                  .arg(fileName)
                                  .arg(sampleRate));
                    statusBar()->showMessage(uiText("status.rx_recording",
                                                    "Recording RX audio") +
                                             QStringLiteral(": %1")
                                                 .arg(QFileInfo(fileName).fileName()));
                    updateRxAudioRecordingActions();
                }, Qt::QueuedConnection);
        connect(m_rxAudioRecorder, &RxAudioRecorder::recordingStopped,
                this, [this](const QString &fileName, quint64 samples, qint64 bytes) {
                    m_rxAudioRecordingActive = false;
                    const double seconds = m_audioEngine != nullptr && m_audioEngine->sampleRate() > 0
                        ? static_cast<double>(samples) / m_audioEngine->sampleRate() : 0.0;
                    appendLog(uiText("log.rx_recording_stopped",
                                     "RX audio recording saved:") +
                              QStringLiteral(" %1 (%2 s, %3 bytes)")
                                  .arg(fileName)
                                  .arg(seconds, 0, 'f', 1)
                                  .arg(bytes));
        ...
        connect(m_rxAudioRecorder, &RxAudioRecorder::recordingError,
                this, [this](const QString &message) {
                    m_rxAudioRecordingActive = false;
                    m_pendingRxRecordingPath.clear();
                    appendLog(message);
                    QMessageBox::warning(this, uiText("audio_recording_title",
                                                      "RX audio recording"),
                                         message);
                    statusBar()->clearMessage();
                    updateRxAudioRecordingActions();
...
void MainWindow::applyDecodeTableVisualSettings(QTableWidget *table)
{
    ...
    QFont f = table->font();
    const QString decodeFamily = m_settings.decodeTableFontFamily.trimmed();
    if (!decodeFamily.isEmpty()) {
        f.setFamily(decodeFamily);
    }
...
void MainWindow::applyPersistentSettingsToRuntime(const AppSettings *previousSettings)
{
    const bool fullApply = (previousSettings == nullptr);
    const bool appearanceChanged = fullApply || uiAppearanceSettingsChanged(*previousSettings, m_settings);
    const bool decodeVisualChanged = fullApply || decodeTableSettingsChanged(*previousSettings, m_settings);
    const bool catChanged = fullApply || catSettingsKey(*previousSettings) != catSettingsKey(m_settings);
    const bool rotatorChanged = fullApply || rotatorSettingsKey(*previousSettings) != rotatorSettingsKey(m_settings);

    if (appearanceChanged) {
        applyUiAppearanceSettings();
    }
    if (decodeVisualChanged) {
    ...
    const bool deviceSelectionChanged = fullApply ||
        previousSettings->audioInputName != m_settings.audioInputName ||
        previousSettings->audioOutputName != m_settings.audioOutputName ||
        previousSettings->pttPortName != m_settings.pttPortName;
...
void MainWindow::qsyRigToSelectedFtBand()
{
    ...
    if (m_rigController != nullptr && !m_rigController->isConnected()) {
        // Queue CAT configuration before the frequency write.  Queued calls to
        // the rig controller are processed in order, so selecting an FT band
        // also tunes a freshly-enabled rig without requiring a separate Test CAT.
        invokeRigConfigureFromSettings();
    }
    invokeRigSetFrequency(targetHz);
    appendLog(QStringLiteral("%1 CAT QSY queued: %2 -> %3.").arg(modeName, band, formatRigFrequency(targetHz)));
}
...
void MainWindow::setReceiverRunning(bool running)
{
    ...
    if (m_spinCwToneHz != nullptr) {
        ...
        if (m_chkCwAutoWpmA != nullptr) m_chkCwAutoWpmA->setEnabled(configurable);
        if (m_chkCwAutoWpmB != nullptr) m_chkCwAutoWpmB->setEnabled(configurable);
        if (m_spinCwWpmA != nullptr) m_spinCwWpmA->setEnabled(configurable && !(m_chkCwAutoWpmA != nullptr && m_chkCwAutoWpmA->isChecked()));
        if (m_spinCwWpmB != nullptr) m_spinCwWpmB->setEnabled(configurable && !(m_chkCwAutoWpmB != nullptr && m_chkCwAutoWpmB->isChecked()));
        if (m_spinCwTxWpm != nullptr) m_spinCwTxWpm->setEnabled(configurable);
        if (m_chkCwAutoBandwidth != nullptr) m_chkCwAutoBandwidth->setEnabled(configurable);
    ...
    if (m_spinFt8RxFreq != nullptr) {
        ...
        if (m_btnFt8AnalyzeWav != nullptr) m_btnFt8AnalyzeWav->setEnabled(!m_txRunning && !m_offlineAnalysisActive);
        if (m_btnFtDecodeAnalyzeWav != nullptr) m_btnFtDecodeAnalyzeWav->setEnabled(!m_txRunning && !m_offlineAnalysisActive);
    }
...
void MainWindow::handleModeChanged(const QString &modeName)
{
    ...
    if (Ft8Mode::isFamilyMode(modeName)) {
        if (!m_ftCallerQueueMode.isEmpty() && m_ftCallerQueueMode != ftPerfModeName) {
            clearFt8CallerQueue(QStringLiteral("FT mode changed"));
        }
        m_ftCallerQueueMode = ftPerfModeName;
    } else {
        if (!m_ft8CallerQueue.isEmpty()) {
            clearFt8CallerQueue(QStringLiteral("left FT mode"));
        }
        m_ftCallerQueueMode.clear();
    }
    if (Ft8Mode::isFamilyMode(modeName) && m_ft8RecentLiveDecodeModeName != ftPerfModeName) {
        m_ft8RecentLiveDecodeMs.clear();
        m_ft8RecentLiveDecodeSlots.clear();
        m_ft8RecentLiveDecodeModeName = ftPerfModeName;
    }

    if (!RadioTelescopeMode::isMode(modeName) && m_radioTelescopeScanActive) {
        stopRadioTelescopeScan(false);
    }
    ...
    if (modeName == WeatherFaxDecoder::modeName()) {
        ...
        if (m_pageRadioTelescopeSettings != nullptr) {
...
void MainWindow::updateWaterfallMarkers()
{
    ...
    if (m_txtCwRx != nullptr) {
        m_txtCwRx->setMinimumHeight(cwMode ? 150 : 190);
        m_txtCwRx->setMaximumHeight(QWIDGETSIZE_MAX);
    }
    if (m_txtCwRxB != nullptr) {
        m_txtCwRxB->setMinimumHeight(cwMode ? 150 : 190);
        m_txtCwRxB->setMaximumHeight(QWIDGETSIZE_MAX);
    }
    ...
    if (RadioTelescopeMode::isMode(mode)) {
        QVector<FrequencyMarker> markers;
        FrequencyMarker low;
        low.frequencyHz = (m_spinRadioTelescopeLowHz != nullptr) ? m_spinRadioTelescopeLowHz->value() : RadioTelescopeMode::defaultLowFrequencyHz();
        low.label = QStringLiteral("L");
        low.color = QColor(80, 255, 120);
        low.width = 2;
        markers.append(low);
        FrequencyMarker high;
        high.frequencyHz = (m_spinRadioTelescopeHighHz != nullptr) ? m_spinRadioTelescopeHighHz->value() : RadioTelescopeMode::defaultHighFrequencyHz();
        high.label = QStringLiteral("H");
        high.color = QColor(255, 170, 60);
        high.width = 2;
        markers.append(high);
        m_waterfallWidget->setMarkers(markers);
        return;
...
void MainWindow::handleWaterfallFrequencyClicked(double frequencyHz, Qt::MouseButton button)
{
    ...
    if (RadioTelescopeMode::isMode(mode) && m_spinRadioTelescopeLowHz != nullptr && m_spinRadioTelescopeHighHz != nullptr) {
        if (button == Qt::RightButton) {
            m_spinRadioTelescopeHighHz->setValue(qBound(m_spinRadioTelescopeHighHz->minimum(), roundedHz, m_spinRadioTelescopeHighHz->maximum()));
        } else {
            m_spinRadioTelescopeLowHz->setValue(qBound(m_spinRadioTelescopeLowHz->minimum(), roundedHz, m_spinRadioTelescopeLowHz->maximum()));
        }
        applyRadioTelescopeSettings();
        appendLog(QStringLiteral("Radio Telescope audio slice from waterfall: %1-%2 Hz.").arg(m_spinRadioTelescopeLowHz->value()).arg(m_spinRadioTelescopeHighHz->value()));
        return;
    }
...
void MainWindow::handleRxAudioBlock(const AudioBlock &block)
{
    ...
    if (Ft8Mode::isFamilyMode(modeName)) {
        // Routed directly AudioEngine -> Ft8RxDecoder in
        // setupProcessingConnections().  MainWindow never transports, gates
        // or duplicates live FT audio.
        return;
    }
    ...
    if (RadioTelescopeMode::isMode(modeName)) {
        if (m_radioTelescopeScanActive && m_radioTelescopeScanPhase == RadioTelescopeScanPhase::Sampling &&
            ...
            const double power = measureRadioTelescopeBandPower(block);
            if (power > 0.0) {
                m_radioTelescopeAccumulatedPower += power;
                ++m_radioTelescopeAccumulatedBlocks;
            }
        }
        return;
    }
...
void MainWindow::applyCwSettings()
{
    ...
    const bool autoWpmA = m_chkCwAutoWpmA->isChecked();
    const bool autoWpmB = m_chkCwAutoWpmB->isChecked();
    ...
    m_settings.cwWpmB = wpmB;
    m_settings.cwTxWpm = txWpm;
    m_settings.cwAutoWpmA = autoWpmA;
    m_settings.cwAutoWpmB = autoWpmB;
...
void MainWindow::clearCwRxAText()
{
    ...
    if (m_cwDiagnosticWidget != nullptr) m_cwDiagnosticWidget->clearChannel(0);
    if (m_waterfallWidget != nullptr) {
        m_waterfallWidget->clearTextOverlayStream(QStringLiteral("CW_A"));
    }
    updateCwDualRxStatusLabel();
}
...
void MainWindow::clearCwRxBText()
{
    ...
    if (m_cwDiagnosticWidget != nullptr) m_cwDiagnosticWidget->clearChannel(1);
    if (m_waterfallWidget != nullptr) {
...
{
    Q_UNUSED(color);
    QPlainTextEdit *terminal = channel == QLatin1String("B") ? m_txtCwRxB : m_txtCwRx;
...
{
    Q_UNUSED(terminal);
    // RX A and RX B are displayed in separate panes; channel prefixes are no
    // longer inserted into the decoded text.
}
...
void MainWindow::updateCwDualRxStatusLabel()
{
    ...
    const auto localizedState = [this](const QString &state) {
        if (state == QStringLiteral("TRACK")) {
            return uiText("cw_state_track", "TRACK");
        }
        if (state == QStringLiteral("HOLD")) {
            return uiText("cw_state_hold", "HOLD");
        }
        return uiText("cw_state_acquire", "ACQUIRE");
    };

    const int requestedBw = (m_spinCwBandwidthHz != nullptr)
                                ? m_spinCwBandwidthHz->value()
                                : m_settings.cwBandwidthHz;
    const int markerA = (m_spinCwToneHz != nullptr)
...
void MainWindow::applyFt8Settings()
{
    if (m_editFt8DxCall == nullptr || m_editFt8DxGrid == nullptr ||
        ...
        m_chkFt8AutoLog == nullptr || m_chkFt8CallerQueue == nullptr ||
        m_chkFt8HoldTxFreq == nullptr || m_cmbFt8TxStrategy == nullptr ||
    ...
    m_settings.ft8QueueCallers = m_chkFt8CallerQueue->isChecked();
    ...
    if (m_ft8RxDecoder != nullptr) {
        ...
        QMetaObject::invokeMethod(m_ft8RxDecoder, "setMyCall", Qt::QueuedConnection, Q_ARG(QString, stationCallsign()));
        QMetaObject::invokeMethod(m_ft8RxDecoder, "setDxCall", Qt::QueuedConnection, Q_ARG(QString, m_settings.ft8DxCallsign));
        QMetaObject::invokeMethod(m_ft8RxDecoder, "setQsoDeadlineActive", Qt::QueuedConnection, Q_ARG(bool, m_ftSession.qsoActive));
    }
    if (m_ftSlotScheduler != nullptr) {
...
void MainWindow::updateFt8SlotStatus()
{
    ...
    if (m_lblFt8WindowStatus != nullptr) {
        const QString currentPeriod = firstPeriodNow ? QStringLiteral("I") : QStringLiteral("II");
        const QString timingDetail = txWindow
            ? uiText("ft8_tx_window_ends_in", "TX window ends in %1 s")
...
void MainWindow::updateFt8TxBannerUi()
{
    ...
    const bool bannerTxFirst = (m_radioFt8TxFirst != nullptr) ? m_radioFt8TxFirst->isChecked() : true;
    const QTime bannerTime = bannerNowUtc.time();
    const int bannerMsOfDay = (((bannerTime.hour() * 60 + bannerTime.minute()) * 60) + bannerTime.second()) * 1000 + bannerTime.msec();
    const int bannerCyclePosMs = bannerMsOfDay % bannerCycleMs;
    const int bannerSelectedStartMs = bannerTxFirst ? 0 : bannerSlotMs;
    const bool bannerInsideSelectedPeriod = bannerCyclePosMs >= bannerSelectedStartMs &&
                                            bannerCyclePosMs < bannerSelectedStartMs + bannerSlotMs;
    const int bannerSafeImmediateMs = qMax(0,
        (activeFtProfile.shortLabel.compare(QStringLiteral("FT4"), Qt::CaseInsensitive) == 0 ? 300 : 500) - 120);
    const bool immediateSelectedPeriodMissed = schedulerArmed && bannerInsideSelectedPeriod &&
...
void MainWindow::handleFt8DecodePerformance(const Ft8RxDecoder::PerfStats &stats)
{
    ...
    const QString statsModeName = stats.modeName.trimmed().isEmpty()
        ? ((ui != nullptr && ui->cmbMode != nullptr) ? ui->cmbMode->currentText().trimmed().toUpper() : QStringLiteral("FT"))
        : stats.modeName.trimmed().toUpper();
    const QString statsModeLabel = Ft8Mode::profileForMode(statsModeName).shortLabel.trimmed().isEmpty()
        ? statsModeName
        : Ft8Mode::profileForMode(statsModeName).shortLabel;

    if (!stats.offline) {
        if (m_ft8RecentLiveDecodeModeName != statsModeName) {
            m_ft8RecentLiveDecodeMs.clear();
            m_ft8RecentLiveDecodeSlots.clear();
            m_ft8RecentLiveDecodeModeName = statsModeName;
        }
        const QString slotKey = statsModeName + QLatin1Char('|') +
                                (stats.slotUtc.trimmed().isEmpty() ? QStringLiteral("--") : stats.slotUtc.trimmed());
        int existingSlot = -1;
    ...
    if (!stats.offline) {
        appendLog(uiText("ft_audio_timeline_log", "%1 audio timeline: blocks %2, boundary splits %3, sequence gaps %4, UTC pre-pad %5, gap %6 samples, overlap %7 samples, max block age %8 ms")
                      .arg(statsModeLabel)
                      .arg(stats.audioBlocksReceived)
        ...
        if (!stats.resourceAdjustment.trimmed().isEmpty() &&
            ...
            appendLog(QStringLiteral("FT resource policy: ") + stats.resourceAdjustment);
        }
    }
    ...
    if (m_ftDisplayDiagSlotKey == displayDiagSlotKey) {
        appendLog(QStringLiteral("%1 display diagnostic: slot %2, decoder decodes %3, decodeReady signals %4, rows inserted %5, duplicate/update %6, blacklisted %7, table rows %8→%9")
                      .arg(statsModeLabel)
                      .arg(stats.slotUtc.trimmed().isEmpty() ? QStringLiteral("--") : stats.slotUtc.trimmed())
                      .arg(stats.decodeCount)
                      .arg(m_ftDisplayDiagDecodeSignals)
                      .arg(m_ftDisplayDiagRowsInserted)
                      .arg(m_ftDisplayDiagRowsUpdated)
                      .arg(m_ftDisplayDiagBlacklisted)
                      .arg(m_ftDisplayDiagRowsBefore)
                      .arg(m_ftDisplayDiagRowsAfter));
        if (stats.decodeCount > 0 &&
            m_ftDisplayDiagDecodeSignals > 0 &&
            (m_ftDisplayDiagRowsInserted + m_ftDisplayDiagRowsUpdated) <= 0) {
    ...
    if (statsModeLabel == QStringLiteral("FT4") && !stats.offline && stats.totalMs > 800.0) {
        appendLog(QStringLiteral("WARNING: FT4 live decode took %1 ms; target is <800 ms on this path. Check CPU load and live decode depth.")
                      .arg(stats.totalMs, 0, 'f', 0));
    }
...
void MainWindow::scheduleFt8SequencerMessage(const QString &message, const QString &tag)
{
    ...
    const qint64 armCheckNowMs = QDateTime::currentDateTimeUtc().toMSecsSinceEpoch();
    const int requestedTxHz = (m_spinFt8TxFreq != nullptr)
...
bool MainWindow::canStartFt8FullAutoQsoNow() const
{
    ...
    if (m_chkFt8CallerQueue != nullptr && m_chkFt8CallerQueue->isChecked() && !m_ft8CallerQueue.isEmpty()) {
        // Direct callers already waiting in the explicit FIFO always have
        // priority over answering a new generic CQ in Evil/Auto QSO mode.
        return false;
    }
...
bool MainWindow::startFt8FullAutoQsoFromCandidate(const Ft8FullAutoCqCandidate &candidate)
{
    ...
    if (m_ft8RxDecoder != nullptr) {
        QMetaObject::invokeMethod(m_ft8RxDecoder, "setDxCall", Qt::QueuedConnection, Q_ARG(QString, call));
        QMetaObject::invokeMethod(m_ft8RxDecoder, "setQsoDeadlineActive", Qt::QueuedConnection, Q_ARG(bool, true));
    }
    ...
    if (answer.trimmed().isEmpty()) {
        ...
        m_ftSession.clearQso();
        if (m_ft8RxDecoder != nullptr) {
            QMetaObject::invokeMethod(m_ft8RxDecoder, "setQsoDeadlineActive", Qt::QueuedConnection, Q_ARG(bool, false));
        }
        updateFt8SequencerUi();
        return false;
    }
...
void MainWindow::updateFt8SessionQueueUi()
{
    if (m_lblFt8SessionQueue == nullptr) {
        return;
    }
    m_lblFt8SessionQueue->setText(
        uiText("ft8_session_stats", "Session QSOs: %1 · Queued callers: %2")
            .arg(m_ftSessionQsoCount)
            .arg(m_ft8CallerQueue.size()));
}

void MainWindow::clearFt8CallerQueue(const QString &reason)
{
    const int removed = m_ft8CallerQueue.size();
    m_ft8CallerQueue.clear();
    updateFt8SessionQueueUi();
    if (removed > 0 && !reason.trimmed().isEmpty()) {
        appendLog(QString("FT caller queue: cleared %1 waiting caller(s): %2")
                      .arg(removed)
                      .arg(reason.trimmed()));
    }
}

void MainWindow::removeFt8QueuedCaller(const QString &call)
{
    ...
    bool changed = false;
    for (int i = m_ft8CallerQueue.size() - 1; i >= 0; --i) {
        if (FtDecodedText::callMatches(m_ft8CallerQueue.at(i).call, wanted)) {
            m_ft8CallerQueue.removeAt(i);
            changed = true;
        }
    }
    if (changed) {
...
void MainWindow::considerFt8CallerForQueue(const Ft8RxDecoder::Decode &decode, bool blacklistedDecode)
{
    if (blacklistedDecode || m_chkFt8CallerQueue == nullptr || !m_chkFt8CallerQueue->isChecked()) {
        return;
    }
    ...
    const ParsedFt8Message parsed = parseFt8MessageText(decode.message, myCall);
    if (!isFt8DirectReplyToMyCall(parsed, myCall) || parsed.senderCall.isEmpty()) {
        return;
    }

    const QString call = parsed.senderCall.trimmed().toUpper();
    if (call.isEmpty() || FtDecodedText::callMatches(call, myCall)) {
        return;
    }

    // The active correspondent is already owned by the linear sequencer and is
    // never duplicated in the passive waiting queue.
    if (m_ftSession.qsoActive && !m_ftSession.dxCall.trimmed().isEmpty() &&
        FtDecodedText::callMatches(call, m_ftSession.dxCall)) {
        removeFt8QueuedCaller(call);
        return;
    }

    // Terminal acknowledgements from a QSO that is already over are not new
    // callers. They must never resurrect a completed contact through the queue.
    if (parsed.final73 || parsed.rr73 || parsed.rrr || parsed.contestAck) {
        removeFt8QueuedCaller(call);
        return;
    }

    const QDateTime now = QDateTime::currentDateTimeUtc();
    if (!m_lastCompletedFt8Call.isEmpty() &&
        FtDecodedText::callMatches(call, m_lastCompletedFt8Call) &&
        m_lastCompletedFt8Utc.isValid() &&
        qAbs(m_lastCompletedFt8Utc.secsTo(now)) <= 120) {
        return;
    }

    for (Ft8QueuedCaller &queued : m_ft8CallerQueue) {
        if (!FtDecodedText::callMatches(queued.call, call)) {
            continue;
        }
        queued.latestDecode = decode;
        if (!parsed.grid.trimmed().isEmpty() && !FtDecodedText::isAckLikeGridTrap(parsed.grid)) {
            queued.grid = parsed.grid.left(4).toUpper();
        }
        queued.lastSeenUtc = now;
        updateFt8SessionQueueUi();
        return;
    }

    constexpr int kMaxQueuedCallers = 12;
    if (m_ft8CallerQueue.size() >= kMaxQueuedCallers) {
        appendLog(QString("FT caller queue: full (%1); ignoring additional caller %2.")
                      .arg(kMaxQueuedCallers)
                      .arg(call));
        return;
    }

    Ft8QueuedCaller queued;
    queued.latestDecode = decode;
    queued.call = call;
    if (!parsed.grid.trimmed().isEmpty() && !FtDecodedText::isAckLikeGridTrap(parsed.grid)) {
        queued.grid = parsed.grid.left(4).toUpper();
    }
    queued.firstSeenUtc = now;
    queued.lastSeenUtc = now;
    m_ft8CallerQueue.append(queued);
    updateFt8SessionQueueUi();
    appendLog(QString("FT caller queue: %1 queued in FIFO position %2 (%3 dB, %4 Hz).")
                  .arg(call)
                  .arg(m_ft8CallerQueue.size())
                  .arg(decode.snrDb)
...
bool MainWindow::activateNextFt8QueuedCaller()
{
    if (m_chkFt8CallerQueue == nullptr || !m_chkFt8CallerQueue->isChecked() ||
        m_ft8OperatorStopLatched || !Ft8Mode::isFamilyMode(ui->cmbMode->currentText())) {
        return false;
    }
    ...
    const QDateTime now = QDateTime::currentDateTimeUtc();
    constexpr qint64 kCallerQueueMaxAgeSeconds = 5 * 60;
    for (int i = m_ft8CallerQueue.size() - 1; i >= 0; --i) {
        const Ft8QueuedCaller &queued = m_ft8CallerQueue.at(i);
        if (!queued.lastSeenUtc.isValid() || queued.lastSeenUtc.secsTo(now) > kCallerQueueMaxAgeSeconds) {
            appendLog(QString("FT caller queue: expired %1 after more than 5 minutes without a fresh call.")
                          .arg(queued.call));
            m_ft8CallerQueue.removeAt(i);
        }
    }
    ...
    const Ft8QueuedCaller queued = m_ft8CallerQueue.first();
    m_ft8CallerQueue.removeAt(0);
    updateFt8SessionQueueUi();

    // A queued caller is activated through the same single sequencer used by a
    // normal live decode. Select only the opposite UTC period here; no second
    // QSO state machine and no parallel TX path are introduced.
    selectFt8OppositePeriodFromDecode(queued.latestDecode);

    // Drop the completed correspondent's retained UI/session context before the
    // next FIFO item is fed to the normal sequencer. Known grids are maintained
    // by the decoder/session cache independently.
    if (!m_ftSession.qsoActive) {
        m_ftSession.clearQso();
    }
    m_ft8AutoNoResponseCooldown.remove(queued.call);
    appendLog(QString("FT caller queue: activating %1 (FIFO; %2 caller(s) remain).")
                  .arg(queued.call)
                  .arg(m_ft8CallerQueue.size()));
    processFt8SequencerDecode(queued.latestDecode);
    return m_ftSession.qsoActive || m_ft8PendingTxArmed || m_hasDeferredFt8TxPlan;
}

void MainWindow::processFt8SequencerDecode(const Ft8RxDecoder::Decode &decode)
{
    if (m_ft8OperatorStopLatched) {
        return;
    }
    const QString myCall = stationCallsign();
    if (myCall.isEmpty()) {
    ...
    seqContext.txActive = m_txRunning || m_ftTxWorkerRunning ||
                          (m_txAudioEngine != nullptr && m_txAudioEngine->isRunning());
    ...
    if (decision.action == FtQsoSequencer::Action::StopTx) {
        ...
        updateCatRotatorQsoTarget(QStringLiteral("FT QSO abandoned: %1 is working another station")
                                      .arg(abandonedCall.isEmpty() ? QStringLiteral("DX") : abandonedCall));
    ...
    if (m_ftSession.qsoActive || decision.startsQso) {
        removeFt8QueuedCaller(m_ftSession.dxCall.isEmpty() ? parsed.senderCall : m_ftSession.dxCall);
        if (m_ft8RxDecoder != nullptr) {
            QMetaObject::invokeMethod(m_ft8RxDecoder, "setDxCall", Qt::QueuedConnection, Q_ARG(QString, m_ftSession.dxCall));
            QMetaObject::invokeMethod(m_ft8RxDecoder, "setQsoDeadlineActive", Qt::QueuedConnection, Q_ARG(bool, true));
        }
    ...
    case FtQsoSequencer::Action::ArmRow:
        ...
        return;
    case FtQsoSequencer::Action::InterruptAndArmRow:
        // A newer, higher-priority terminal message arrived while an older
        // FT frame was already on air.  FT symbols cannot be changed in place:
        // stop the obsolete frame, queue exactly one final message, and let
        // handleTxStopped() arm it for the next valid UTC period.
        armRow(decision.txRow, decision.tag, decision.nextState);
        if (m_txRunning || m_ftTxWorkerRunning ||
            (m_txAudioEngine != nullptr && m_txAudioEngine->isRunning())) {
            stopImageTx();
...
void MainWindow::handleFt8QsoHistoryDoubleClicked(QTableWidgetItem *item)
{
    ...
    if (direction != QStringLiteral("TX") && m_tableFt8Rx != nullptr) {
        for (int r = m_tableFt8Rx->rowCount() - 1; r >= 0; --r) {
            ...
            if (utcMatches && freqMatches) {
                ...
                const bool previousSelectionOnly = m_ft8SelectionOnly;
                m_ft8SelectionOnly = true;
                handleFt8DecodeDoubleClicked(rxMsg);
                m_ft8SelectionOnly = previousSelectionOnly;
                return;
            }
        }
    }

    // A TX history row may prepare only a normal FT message involving this
    // station.  It never arms a transmission by itself.
    const QString myCall = stationCallsign();
    ...
    int matchedRow = m_ftStandardMessages.findRow(message);
    if (matchedRow < 0 && parts.size() >= 3) {
        const QString payload = parts.value(2).trimmed().toUpper();
        if (parts.value(0).compare(QStringLiteral("CQ"), Qt::CaseInsensitive) == 0) {
            matchedRow = 0;
        } else if (QRegularExpression(QStringLiteral("^[A-R]{2}[0-9]{2}([A-X]{2})?$"),
                                      QRegularExpression::CaseInsensitiveOption).match(payload).hasMatch()) {
            matchedRow = 1;
        } else if (QRegularExpression(QStringLiteral("^[+-]\\d{2}$")).match(payload).hasMatch()) {
            matchedRow = 2;
        } else if (QRegularExpression(QStringLiteral("^R[+-]\\d{2}$")).match(payload).hasMatch()) {
            matchedRow = 3;
        } else if (payload == QStringLiteral("RR73") || payload == QStringLiteral("RRR")) {
            matchedRow = 4;
        } else if (payload == QStringLiteral("73")) {
            matchedRow = 5;
        }
    }
    if (matchedRow >= 0) {
        setFt8ActiveTxRow(matchedRow);
        if (m_tableFt8TxMessages != nullptr) {
            m_tableFt8TxMessages->selectRow(matchedRow);
        }
    }
    if (!call.isEmpty()) {
        updateCatRotatorQsoTarget(QStringLiteral("FT history selection %1").arg(call));
    }
    appendLog(QStringLiteral("FT QSO activity selected only: %1 — press TX to transmit.").arg(message));
    updateFt8SequencerUi();
}
...
void MainWindow::handleFt8DecodeDoubleClicked(QTableWidgetItem *item)
{
    ...
    if (!m_ft8SelectionOnly && !call.isEmpty()) {
    ...
    appendLog(QString("%1 correspondent selected: %2 %3; TX%4 %5.")
                  .arg(Ft8Mode::profileForMode(ui->cmbMode->currentText()).shortLabel,
                       call.isEmpty() ? QString("--") : call,
                       grid.isEmpty() ? QString("--") : grid)
                  .arg(preferredTxRow + 1)
                  .arg(m_ft8SelectionOnly ? QStringLiteral("selected; press TX") : QStringLiteral("queued")) + txPeriodNote);

    if (m_ft8SelectionOnly) {
        if (!call.isEmpty()) {
            updateCatRotatorQsoTarget(QStringLiteral("FT decode selection %1").arg(call));
        }
        updateFt8SequencerUi();
        return;
    }
...
void MainWindow::clearFt8DecodeList()
{
    ...
    m_ft8RecentLiveDecodeMs.clear();
    m_ft8RecentLiveDecodeSlots.clear();
    m_ft8RecentLiveDecodeModeName.clear();
    m_ftDisplayDiagSlotKey.clear();
    m_ftDisplayDiagDecodeSignals = 0;
    m_ftDisplayDiagRowsInserted = 0;
    m_ftDisplayDiagRowsUpdated = 0;
    m_ftDisplayDiagBlacklisted = 0;
    m_ftDisplayDiagRowsBefore = 0;
    m_ftDisplayDiagRowsAfter = 0;
...
void MainWindow::scheduleFt8RetryIfNeeded()
{
    ...
    const QString retryModeLabel = (ui != nullptr && ui->cmbMode != nullptr)
        ? Ft8Mode::profileForMode(ui->cmbMode->currentText()).shortLabel
        : QStringLiteral("FT");
    appendLog(QString("%1 retry armed: %2 unanswered transmission(s) left for %3")
                  .arg(retryModeLabel)
...
void MainWindow::startFt8RxShell()
{
    ...
    // Load the DXCC/prefix database before opening the timed FT capture. Its
    // first lazy construction can take close to one second on Windows and used
    // to consume the only manual-selection interval before the opposite slot.
    (void) CtyCountryFile::instance().isLoaded();
...
{
    if (m_ft8OperatorStopLatched || decodes.isEmpty()) {
        return -1;
    }

    const QString myCall = stationCallsign();
    FtQsoSequencer::Context context = m_ftSession.makeContext(
        myCall,
        (m_spinFt8RxFreq != nullptr) ? m_spinFt8RxFreq->value() : m_settings.ft8RxFrequencyHz,
        (m_spinFt8TxFreq != nullptr) ? m_spinFt8TxFreq->value() : m_settings.ft8TxFrequencyHz);
    context.txActive = m_txRunning || m_ftTxWorkerRunning ||
                       (m_txAudioEngine != nullptr && m_txAudioEngine->isRunning());
    context.startToleranceHz = 25;
    context.stopToleranceHz = 50;

    auto actionRank = [](FtQsoSequencer::Action action) {
        switch (action) {
        case FtQsoSequencer::Action::StopTx:       return 5000;
        case FtQsoSequencer::Action::InterruptAndArmRow: return 4500;
        case FtQsoSequencer::Action::Complete:     return 4000;
        case FtQsoSequencer::Action::ArmRow:       return 3000;
        case FtQsoSequencer::Action::RetryCurrent: return 2000;
        case FtQsoSequencer::Action::None:         return 0;
        case FtQsoSequencer::Action::Ignore:       return -1000;
        }
        return -1000;
    };
    ...
    for (int i = 0; i < decodes.size(); ++i) {
        FtQsoSequencer::Decode seqDecode;
        seqDecode.message = decodes.at(i).message;
        seqDecode.snrDb = decodes.at(i).snrDb;
        seqDecode.frequencyHz = decodes.at(i).frequencyHz;
        seqDecode.slotStartUtcMs = decodes.at(i).slotStartUtcMs;
        seqDecode.slotPeriodMs = decodes.at(i).slotPeriodMs;
        const FtQsoSequencer::Decision decision = FtQsoSequencer::evaluateDecode(seqDecode, context);

        if (decision.action != FtQsoSequencer::Action::StopTx) {
            if (decision.action == FtQsoSequencer::Action::Ignore ||
                !decision.parsed.addressedToMe ||
                decision.parsed.senderCall.isEmpty() ||
                FtDecodedText::callMatches(decision.parsed.senderCall, myCall)) {
                continue;
            }
            if (m_ftSession.qsoActive && !m_ftSession.dxCall.trimmed().isEmpty() &&
                !FtDecodedText::callMatches(decision.parsed.senderCall, m_ftSession.dxCall)) {
                continue;
            }
        }

        int score = actionRank(decision.action);
        score += static_cast<int>(decision.nextState) * 20;
        if (decision.parsed.addressedToMe) score += 300;
        if (!m_ftSession.dxCall.trimmed().isEmpty() &&
            FtDecodedText::callMatches(decision.parsed.senderCall, m_ftSession.dxCall)) score += 500;
        score += qBound(-30, decodes.at(i).snrDb, 20);
        if (score > bestScore) {
            bestScore = score;
            bestIndex = i;
        }
...
void MainWindow::queueFt8SequencerDecode(const Ft8RxDecoder::Decode &decode)
{
    if (!m_ft8DecodeBatchActive) {
        processFt8SequencerDecode(decode);
        return;
    }

    for (Ft8RxDecoder::Decode &queued : m_ft8PendingSequencerBatch) {
        if (queued.slotStartUtcMs == decode.slotStartUtcMs &&
            ...
            if (decode.snrDb > queued.snrDb) {
                queued = decode;
            }
            return;
        }
    }
    m_ft8PendingSequencerBatch.append(decode);
}

void MainWindow::handleFt8DecodeBatchFinished(qint64 slotStartUtcMs, const QString &phase)
{
...
void MainWindow::handleFt8DecodeReady(const Ft8RxDecoder::Decode &decode)
{
    ...
    const QString ftDisplayMode = Ft8Mode::profileForMode(ui->cmbMode->currentText()).shortLabel.trimmed().toUpper();
    const QString ftDisplaySlotKey = ftDisplayMode + QLatin1Char('|') + decode.utc.trimmed();
    if (m_ftDisplayDiagSlotKey != ftDisplaySlotKey) {
        m_ftDisplayDiagSlotKey = ftDisplaySlotKey;
        m_ftDisplayDiagDecodeSignals = 0;
        m_ftDisplayDiagRowsInserted = 0;
        m_ftDisplayDiagRowsUpdated = 0;
        m_ftDisplayDiagBlacklisted = 0;
        m_ftDisplayDiagRowsBefore = m_tableFt8Rx->rowCount();
        m_ftDisplayDiagRowsAfter = m_ftDisplayDiagRowsBefore;
    }
    ++m_ftDisplayDiagDecodeSignals;
    ...
    if (blacklistedDecode) {
        ...
        const QColor bg = mmColourFromSetting(m_settings.ftHighlightCqBackground, QColor(68, 92, 28));
        const QColor fg = mmColourFromSetting(m_settings.ftHighlightCqForeground, QColor(255, 228, 120));
        for (QTableWidgetItem *item : items) {
            item->setForeground(QBrush(fg));
            item->setBackground(QBrush(bg));
            QFont font = item->font();
        ...
        const QColor bg = mmColourFromSetting(m_settings.ftHighlightWorkedBackground, QColor(36, 36, 36));
        const QColor fg = mmColourFromSetting(m_settings.ftHighlightWorkedForeground, QColor(150, 150, 150));
        for (QTableWidgetItem *item : items) {
            item->setForeground(QBrush(fg));
            item->setBackground(QBrush(bg));
        }
...
void MainWindow::startFt8TxShell()
{
    ...
    if (m_tableFt8TxMessages != nullptr) {
        ...
        if (!selected.isEmpty()) {
            ...
            setFt8ActiveTxRow(selectedRow);
        }
    }
    if (selectedRow < 0) {
        selectedRow = m_ftStandardMessages.findRow(message);
    }
    ...
    // Only the explicit TX button turns a prepared selection into a live QSO
    // state.  History/decode selection may prepare call, grid, frequency and
    // standard row, but it never arms or resurrects the sequencer by itself.
    if (selectedRow == 0) {
        m_ftSession.clearQso();
        if (m_ft8RxDecoder != nullptr) {
            QMetaObject::invokeMethod(m_ft8RxDecoder, "setQsoDeadlineActive", Qt::QueuedConnection, Q_ARG(bool, false));
        ...
        const QString dxCall = (m_editFt8DxCall != nullptr)
            ? m_editFt8DxCall->text().trimmed().toUpper()
            : QString();
        const QString dxGrid = (m_editFt8DxGrid != nullptr)
            ? m_editFt8DxGrid->text().trimmed().left(4).toUpper()
            : QString();
        if (!dxCall.isEmpty()) {
            m_ftSession.startQso(dxCall,
                                 dxGrid,
                                 (m_spinFt8RxFreq != nullptr)
                                     ? m_spinFt8RxFreq->value()
                                     : m_settings.ft8RxFrequencyHz);
            if (m_ft8RxDecoder != nullptr) {
                QMetaObject::invokeMethod(m_ft8RxDecoder, "setDxCall", Qt::QueuedConnection, Q_ARG(QString, dxCall));
                QMetaObject::invokeMethod(m_ft8RxDecoder, "setQsoDeadlineActive", Qt::QueuedConnection, Q_ARG(bool, true));
            }
            if (selectedRow == 1) {
                m_ftSession.state = Ft8SequencerState::SendingLocator;
...
mm::CatRotatorController::Config MainWindow::catRotatorConfigFromSettings() const
{
    ...
    if (m_settings.textMyCoordinatesEnabled &&
        std::isfinite(m_settings.textMyLatitudeDeg) &&
        std::isfinite(m_settings.textMyLongitudeDeg) &&
        m_settings.textMyLatitudeDeg >= -90.0 && m_settings.textMyLatitudeDeg <= 90.0 &&
        m_settings.textMyLongitudeDeg >= -180.0 && m_settings.textMyLongitudeDeg <= 180.0) {
        cfg.homeCoordinatesValid = true;
        cfg.homeLongitudeDeg = m_settings.textMyLongitudeDeg;
        cfg.homeLatitudeDeg = m_settings.textMyLatitudeDeg;
...
void MainWindow::setupCatRotatorModule()
{
    if (m_catRotatorController == nullptr) {
        ...
        connect(m_catRotatorController, &mm::CatRotatorController::positionChanged,
            ...
            handleRadioTelescopeRotatorPosition(azimuthDeg, elevationDeg);
        });
...
void MainWindow::showAppSettingsDialogPage(AppSettingsDialog::InitialPage page)
{
    ...
    const qint64 settingsConstructMs = settingsOpenTimer.elapsed();
    ...
    if (m_catRotatorController != nullptr) {
        const mm::CatRotatorController::Config initialRotCfg = m_catRotatorController->config();
        dialog.setRotatorConnectionStatus(initialRotCfg.profileIndex,
                                          m_catRotatorController->isConnected(),
                                          m_catRotatorController->isConnected()
                                              ? uiText("rotator_connected", "Connected")
                                              : uiText("rotator_disconnected", "Disconnected"));
        connect(m_catRotatorController, &mm::CatRotatorController::connectionChanged,
                &dialog, [this, dialogPtr](bool connected) {
            if (dialogPtr && m_catRotatorController != nullptr) {
                const mm::CatRotatorController::Config cfg = m_catRotatorController->config();
                dialogPtr->setRotatorConnectionStatus(cfg.profileIndex,
                                                       connected,
                                                       connected ? uiText("rotator_connected", "Connected")
                                                                 : uiText("rotator_disconnected", "Disconnected"));
            }
        });
        connect(m_catRotatorController, &mm::CatRotatorController::statusChanged,
                &dialog, [this, dialogPtr](const QString &message) {
            if (dialogPtr && m_catRotatorController != nullptr) {
                const mm::CatRotatorController::Config cfg = m_catRotatorController->config();
                dialogPtr->setRotatorConnectionStatus(cfg.profileIndex,
                                                       m_catRotatorController->isConnected(),
                                                       message.trimmed().isEmpty()
                                                           ? (m_catRotatorController->isConnected() ? uiText("rotator_connected", "Connected") : uiText("rotator_disconnected", "Disconnected"))
                                                           : message.trimmed());
            }
        });
        connect(m_catRotatorController, &mm::CatRotatorController::calibrationFinished,
    ...
    connect(&dialog, &AppSettingsDialog::rotatorConnectionRequested,
        ...
        const AppSettings::RotatorProfileSettings &rp = requested.rotatorProfiles[idx];

        mm::CatRotatorController::Config cfg;
        cfg.enabled = true;
        cfg.stationIdentityOk = stationIdentityReady(nullptr);
        cfg.autoConnect = requested.rotatorAutoConnect;
        cfg.showWindowOnStart = false;
        cfg.profileIndex = idx;
        cfg.label = rp.label.trimmed().isEmpty() ? QStringLiteral("Rotator %1").arg(idx + 1) : rp.label.trimmed();
        cfg.bandsCsv = rp.bandsCsv;
        cfg.hamlibModel = qMax(1, rp.hamlibModel);
        cfg.path = rp.path.trimmed();
        cfg.baudRate = qBound(300, rp.baudRate, 1000000);
        cfg.pollIntervalMs = qBound(250, rp.pollIntervalMs, 10000);
        cfg.useElevation = rp.useElevation;
        cfg.overlap = rp.overlap;
        cfg.azimuthGeometryPreset = rp.azimuthGeometryPreset;
        cfg.azimuthStopDeg = rp.azimuthStopDeg;
        cfg.autoReverseOnStall = rp.autoReverseOnStall;
        cfg.noMovementTimeoutMs = rp.noMovementTimeoutMs;
        cfg.noMovementThresholdDeg = rp.noMovementThresholdDeg;
        cfg.parkAzimuth = qBound(0.0, rp.parkAzimuth, 359.9);
        cfg.parkElevation = qBound(-10.0, rp.parkElevation, 180.0);
        cfg.trackSelectedQso = requested.rotatorTrackSelectedQso;
        cfg.trackOnlyWhenQsoActive = requested.rotatorTrackOnlyWhenQsoActive;
        cfg.targetToleranceDeg = qBound(0, rp.targetToleranceDeg, 45);
        cfg.azimuthMinDeg = rp.azimuthMinDeg;
        cfg.azimuthMaxDeg = rp.azimuthMaxDeg;
        cfg.elevationMinDeg = rp.elevationMinDeg;
        cfg.elevationMaxDeg = rp.elevationMaxDeg;
        cfg.azimuthMsPerDeg = rp.azimuthMsPerDeg;
        cfg.elevationMsPerDeg = rp.elevationMsPerDeg;
        cfg.startupDelayMs = rp.startupDelayMs;
        cfg.settleDelayMs = rp.settleDelayMs;
        cfg.txGuardMarginMs = rp.txGuardMarginMs;
        cfg.calibrationStampUtc = rp.calibrationStampUtc;
        if (requested.textMyCoordinatesEnabled &&
            std::isfinite(requested.textMyLatitudeDeg) &&
            std::isfinite(requested.textMyLongitudeDeg) &&
            requested.textMyLatitudeDeg >= -90.0 && requested.textMyLatitudeDeg <= 90.0 &&
            requested.textMyLongitudeDeg >= -180.0 && requested.textMyLongitudeDeg <= 180.0) {
            cfg.homeCoordinatesValid = true;
            cfg.homeLatitudeDeg = requested.textMyLatitudeDeg;
            cfg.homeLongitudeDeg = requested.textMyLongitudeDeg;
            cfg.homeAltitudeM = 0.0;
        }

        if (connectRequested) {
            if (cfg.hamlibModel <= 0 || cfg.path.trimmed().isEmpty()) {
                const QString message = uiText("rotator_connect_missing_endpoint", "Connection blocked: select rotator model and COM/rotctld endpoint first.");
                dialogPtr->setRotatorConnectionStatus(idx, false, message);
                appendLog(QStringLiteral("CatRotator: ") + message);
                return;
            }
            appendLog(QStringLiteral("CatRotator: connecting %1 from Settings.").arg(cfg.label));
            m_catRotatorController->configure(cfg);
            m_catRotatorController->setTrackingQsoTarget(cfg.trackSelectedQso);
            m_catRotatorController->connectRotator();
            dialogPtr->setRotatorConnectionStatus(idx,
                                                   m_catRotatorController->isConnected(),
                                                   m_catRotatorController->isConnected()
                                                       ? uiText("rotator_connected", "Connected")
                                                       : m_catRotatorController->statusText());
        } else {
            appendLog(QStringLiteral("CatRotator: disconnecting %1 from Settings.").arg(cfg.label));
            if (m_catRotatorController->isConnected()) {
                m_catRotatorController->disconnectRotator();
            }
            m_catRotatorController->configure(cfg);
            dialogPtr->setRotatorConnectionStatus(idx,
                                                   false,
    ...
    QElapsedTimer settingsApplyTimer;
    ...
    const bool deviceSelectionChanged =
        oldSettings.audioInputName != m_settings.audioInputName ||
        oldSettings.audioOutputName != m_settings.audioOutputName ||
        oldSettings.pttPortName != m_settings.pttPortName;
    if (deviceSelectionChanged) {
        // Do not rescan audio/serial devices on every Settings close.  On
        // Windows, QAudioDeviceInfo/QSerialPortInfo enumeration can block the
        // GUI for seconds.  The device lists are refreshed from the explicit
        // Refresh control; on OK we only re-select the already-known entries.
        selectComboByBackendName(ui->cmbAudioInput, m_settings.audioInputName);
        selectComboByBackendName(ui->cmbAudioOutput, m_settings.audioOutputName);
        selectComboByBackendName(ui->cmbPttPort, m_settings.pttPortName);
    }
    ...
    loadFt8SettingsToUi();
    applyPersistentSettingsToRuntime(&oldSettings);
    refreshTextMacroButtons();
    refreshFt8StandardMessages();
    updateBandSchedulerTabForMode(ui != nullptr && ui->cmbMode != nullptr ? ui->cmbMode->currentText() : QString());
    m_bandSchedulerTriggeredKeys.clear();

    const bool logbookVisualChanged = logbookPathChanged ||
        oldSettings.logbookStrikeWorkedCalls != m_settings.logbookStrikeWorkedCalls ||
        oldSettings.ftHighlightMyCallBackground != m_settings.ftHighlightMyCallBackground ||
        oldSettings.ftHighlightMyCallForeground != m_settings.ftHighlightMyCallForeground ||
        oldSettings.ftHighlightCqBackground != m_settings.ftHighlightCqBackground ||
        oldSettings.ftHighlightCqForeground != m_settings.ftHighlightCqForeground ||
        oldSettings.ftHighlightWorkedBackground != m_settings.ftHighlightWorkedBackground ||
        oldSettings.ftHighlightWorkedForeground != m_settings.ftHighlightWorkedForeground ||
        oldSettings.ftHighlightTxBackground != m_settings.ftHighlightTxBackground ||
        oldSettings.ftHighlightTxForeground != m_settings.ftHighlightTxForeground ||
        oldSettings.ftHighlightNewCountryEnabled != m_settings.ftHighlightNewCountryEnabled ||
        oldSettings.ftWatchListIconEnabled != m_settings.ftWatchListIconEnabled ||
        oldSettings.ftBlacklistCalls != m_settings.ftBlacklistCalls ||
        oldSettings.ftWatchListCalls != m_settings.ftWatchListCalls;
    const bool qsoMapInputsChanged = logbookPathChanged ||
        oldSettings.textMyCallsign != m_settings.textMyCallsign ||
        oldSettings.textMyLocator != m_settings.textMyLocator ||
        oldSettings.textMyCoordinatesEnabled != m_settings.textMyCoordinatesEnabled ||
        !qFuzzyCompare(oldSettings.textMyLatitudeDeg + 1.0, m_settings.textMyLatitudeDeg + 1.0) ||
        !qFuzzyCompare(oldSettings.textMyLongitudeDeg + 1.0, m_settings.textMyLongitudeDeg + 1.0);
    if (logbookVisualChanged || qsoMapInputsChanged) {
        QTimer::singleShot(150, this, [this, logbookVisualChanged, qsoMapInputsChanged]() {
            if (logbookVisualChanged) {
                refreshLogbookHighlights();
            }
            if (qsoMapInputsChanged) {
                refreshQsoMaps();
            }
        });
    }

    const qint64 settingsApplyMs = settingsApplyTimer.elapsed();
    appendLog(QStringLiteral("Settings updated in %1 ms.").arg(settingsApplyMs));
...
void MainWindow::showAboutMadModem()
{
    ...
    QLabel *title = new QLabel(QStringLiteral(
        "<b>%1</b><br>"
        "Amateur radio digital modem and station hub.")
        .arg(QStringLiteral(MADMODEM_VERSION_DISPLAY)), &dialog);
    ...
    body->setText(QStringLiteral(
        ...
        "SSTV, WEFAX, Feld Hell / FSK-105, MSK144 and Q65.</p>"
...
void MainWindow::startRx()
{
    ...
    if (m_ft8RxDecoder != nullptr) {
        ...
        QMetaObject::invokeMethod(m_ft8RxDecoder,
                                  "setLiveInputEnabled",
                                  enableConnection,
                                  Q_ARG(bool, enableFtLive));
    }

    if (RadioTelescopeMode::isMode(modeName)) {
        QString rotatorReason;
        if (!radioTelescopeRotatorReadyForScan(&rotatorReason)) {
            appendLog(QStringLiteral("Radio Telescope RX start blocked: ") + rotatorReason);
            if (m_lblRadioTelescopeStatus != nullptr) {
                m_lblRadioTelescopeStatus->setText(rotatorReason);
            }
            if (m_lblRadioTelescopeProgress != nullptr) {
                m_lblRadioTelescopeProgress->setText(uiText("radio_telescope_progress_blocked", "Scan blocked: rotator not ready."));
            }
            return;
        }
    }
    ...
    if (modeName == WeatherFaxDecoder::modeName()) {
        ...
        if (!preserveTextTerminal && m_txtCwRxB != nullptr) {
            m_txtCwRxB->clear();
        }
    } else if (modeName == HellschreiberDecoder::modeName()) {
        m_hellDecoder->reset();
    ...
    if (modeName == WeatherFaxDecoder::modeName()) {
        ...
        appendLog(uiText("log_cw_settings_ab", "CW settings: RX A %1 Hz, %2 WPM/%3; RX B %4, %5 WPM/%6; TX %7 WPM.")
                      .arg(m_spinCwToneHz != nullptr ? m_spinCwToneHz->value() : 700)
                      .arg(m_spinCwWpmA != nullptr ? m_spinCwWpmA->value() : 20)
                      .arg((m_chkCwAutoWpmA != nullptr && m_chkCwAutoWpmA->isChecked()) ? "AUTO" : "MAN")
                      .arg(m_cwSecondaryEnabled ? QString::number(m_cwSecondaryToneHz) + QStringLiteral(" Hz") : QStringLiteral("off"))
                      .arg(m_spinCwWpmB != nullptr ? m_spinCwWpmB->value() : 20)
                      .arg((m_chkCwAutoWpmB != nullptr && m_chkCwAutoWpmB->isChecked()) ? "AUTO" : "MAN")
                      .arg(m_spinCwTxWpm != nullptr ? m_spinCwTxWpm->value() : 20));
    } else if (modeName == HellschreiberDecoder::modeName()) {
        const HellschreiberDecoder::Variant variant = (m_cmbHellVariant != nullptr)
                                                         ? HellschreiberDecoder::variantFromKey(m_cmbHellVariant->currentData().toString())
                                                         : HellschreiberDecoder::Variant::FeldHell;
        ...
        appendLog(QStringLiteral("Radio Telescope settings: audio slice %1-%2 Hz, beam %3°, dwell %4 ms, settle %5 ms, interpolation %6.")
                      .arg(m_spinRadioTelescopeLowHz != nullptr ? m_spinRadioTelescopeLowHz->value() : RadioTelescopeMode::defaultLowFrequencyHz())
                      .arg(m_spinRadioTelescopeHighHz != nullptr ? m_spinRadioTelescopeHighHz->value() : RadioTelescopeMode::defaultHighFrequencyHz())
                      .arg(m_spinRadioTelescopeBeamWidthDeg != nullptr ? QString::number(m_spinRadioTelescopeBeamWidthDeg->value(), 'f', 1) : QStringLiteral("15.0"))
                      .arg(m_spinRadioTelescopeDwellMs != nullptr ? m_spinRadioTelescopeDwellMs->value() : RadioTelescopeMode::defaultDwellMs())
                      .arg(m_spinRadioTelescopeSettleMs != nullptr ? m_spinRadioTelescopeSettleMs->value() : 0)
                      .arg((m_chkRadioTelescopeInterpolate != nullptr && m_chkRadioTelescopeInterpolate->isChecked()) ? QStringLiteral("ON") : QStringLiteral("OFF")));
    } else if (Msk144Mode::isMode(modeName)) {
        const int period = (m_cmbMsk144Period != nullptr) ? m_cmbMsk144Period->currentData().toInt() : 15;
...
{
    if (m_rxAudioRecordingActive || !m_pendingRxRecordingPath.isEmpty()) {
        return;
    }

    QString directory = QStandardPaths::writableLocation(QStandardPaths::MusicLocation);
    ...
    QString mode = ui != nullptr && ui->cmbMode != nullptr
        ? ui->cmbMode->currentText() : QStringLiteral("RX");
    mode.replace(QRegularExpression(QStringLiteral("[^A-Za-z0-9_-]+")),
                 QStringLiteral("_"));
    const QString suggested = QDir(directory).filePath(
        QStringLiteral("MadModem_RX_%1_%2.wav")
            .arg(QDateTime::currentDateTimeUtc().toString(QStringLiteral("yyyyMMdd_HHmmss")),
                 mode));
    const QString fileName = QFileDialog::getSaveFileName(
        this,
        uiText("audio_recording_title", "RX audio recording"),
        suggested,
        uiText("audio_recording_filter", "WAV audio (*.wav)"));
    if (fileName.isEmpty()) {
        return;
    }

    m_pendingRxRecordingPath = fileName.endsWith(QStringLiteral(".wav"),
                                                  Qt::CaseInsensitive)
        ? fileName : fileName + QStringLiteral(".wav");
    updateRxAudioRecordingActions();

    if (m_audioEngine != nullptr && m_audioEngine->isRunning()) {
        beginPendingRxAudioRecording();
        return;
    }
...
void MainWindow::beginPendingRxAudioRecording()
{
    if (m_pendingRxRecordingPath.isEmpty() || m_rxAudioRecorder == nullptr ||
        m_audioEngine == nullptr || !m_audioEngine->isRunning()) {
        return;
    }
    const QString path = m_pendingRxRecordingPath;
    const int rate = m_audioEngine->sampleRate() > 0
        ? m_audioEngine->sampleRate() : m_settings.audioSampleRate;
    QMetaObject::invokeMethod(m_rxAudioRecorder, "startRecording",
                              Qt::QueuedConnection,
                              Q_ARG(QString, path),
                              Q_ARG(int, rate));
}

void MainWindow::stopRxAudioRecording()
{
    const bool hadPendingOrActive = m_rxAudioRecordingActive ||
                                    !m_pendingRxRecordingPath.isEmpty();
    m_pendingRxRecordingPath.clear();
    if (m_rxAudioRecorder != nullptr && hadPendingOrActive) {
        // startRecording() and stopRecording() are queued to the same worker.
        // This also cancels a start request whose recordingStarted signal has
        // not yet reached the GUI, avoiding a latent recording after Stop.
        QMetaObject::invokeMethod(m_rxAudioRecorder, "stopRecording",
                                  Qt::QueuedConnection);
    }
    updateRxAudioRecordingActions();
}

void MainWindow::updateRxAudioRecordingActions()
{
    const bool pending = !m_pendingRxRecordingPath.isEmpty();
    if (m_actionStartRxRecording != nullptr) {
        m_actionStartRxRecording->setText(uiText(
            "action.start_rx_audio_recording", "Start RX audio recording..."));
        m_actionStartRxRecording->setStatusTip(uiText(
            "tip.start_rx_audio_recording",
            "Record the exact mono audio stream used by the waterfall and decoders."));
        m_actionStartRxRecording->setEnabled(!m_rxAudioRecordingActive && !pending);
    }
    if (m_actionStopRxRecording != nullptr) {
        m_actionStopRxRecording->setText(uiText(
            "action.stop_rx_audio_recording", "Stop RX audio recording"));
        m_actionStopRxRecording->setStatusTip(uiText(
            "tip.stop_rx_audio_recording", "Finalize the current RX WAV file."));
        m_actionStopRxRecording->setEnabled(m_rxAudioRecordingActive || pending);
    }
}
...
void MainWindow::updateTxPreview()
{
    ...
    if (modeName == WeatherFaxDecoder::modeName()) {
        ...
        const int wpm = m_spinCwTxWpm != nullptr ? m_spinCwTxWpm->value() : 20;
        m_lblTxMode->setText(QString("CW TX: %1 Hz, %2 WPM").arg(tone).arg(wpm));
        const QString text = (m_txtCwTx != nullptr) ? m_txtCwTx->toPlainText() : QString();
...
void MainWindow::updateTxControlState()
{
    ...
    const bool radioTelescopeMode = RadioTelescopeMode::isMode(modeName);
    ...
    const bool rxOnlyTextMode = ft8Mode || radioTelescopeMode;
...
void MainWindow::startFtPreparedSlotTransmit()
{
    ...
    if (!modulator) {
        ...
        appendLog(missedDeadline
                      ...
                      : QStringLiteral("FT TX cancelled: unable to create prepared modulator."));
        if (m_pendingFt8PttKeyed || m_pendingFt8PttPrearmed) {
            unkeyPttAfterTx();
        }
        const bool restartRx = m_returnToRxAfterTx || liveRxRunning;
        m_returnToRxAfterTx = false;
        m_currentTxIsTextMode = false;
        m_ft8PendingTxArmed = false;
        m_ft8PendingTxToken.clear();
