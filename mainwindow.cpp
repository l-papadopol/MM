    ...
    if (ui != nullptr && ui->sideTabWidget != nullptr && m_ddspController != nullptr) {
        ...
        if (m_ft8RxDecoder != nullptr) {
            m_ft8RxDecoder->setMindAssistCallback(
                [mindController](const QString &mode,
                    ...
                    return mindController->scoreNativeFtCandidate(mode,
                                                                  candidateMagnitudes,
                                                                  predictedBits,
                                                                  confidencePercent);
                });
...
void MainWindow::handleFt8DecodePerformance(const Ft8RxDecoder::PerfStats &stats)
{
    ...
    } else if (statsModeLabel == QStringLiteral("FT4") && stats.decodeCount > 0) {
    ...
    if (statsModeLabel == QStringLiteral("FT4") && !stats.offline && stats.totalMs > 800.0) {
        appendLog(QStringLiteral("WARNING: FT4 live decode took %1 ms; target is <800 ms on this path. Check CPU load, live decode depth and MIND mode.")
                      .arg(stats.totalMs, 0, 'f', 0));
    }
    ...
    if (stats.mindReliabilitySamples > 0) {
        appendLog(QStringLiteral("FT MIND Reliability passive: decoded frames %1, symbols %2, avg reliability %3%, weak symbols %4%, QSB depth %5%")
                      .arg(stats.mindReliabilitySamples)
                      .arg(stats.mindReliabilityTotalSymbols)
                      .arg(stats.mindReliabilityAvgPercent, 0, 'f', 1)
                      .arg(stats.mindReliabilityWeakPercent, 0, 'f', 1)
                      .arg(stats.mindReliabilityQsbDepthPercent, 0, 'f', 1));
    }
