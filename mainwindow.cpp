...
QString rotatorSettingsKey(const AppSettings &s)
{
    ...
    parts << (s.rotatorEnabled ? QStringLiteral("1") : QStringLiteral("0"))
          ...
          << (s.rotatorBlockFtTxUntilReady ? QStringLiteral("1") : QStringLiteral("0"))
...
void MainWindow::scheduleFt8SequencerMessage(const QString &message, const QString &tag)
{
    ...
    const bool sameValidArm = m_ft8PendingTxArmed &&
                              ...
                              m_pendingFt8TxTag.trimmed().toUpper() == txTag &&
    ...
    // Do not create a scheduler token for a slot already known to be blocked by
    // the rotator. Stay in RX and arm once, only after the guard is ready.
    QString initialRotatorWaitReason;
    int initialRotatorEtaMs = 0;
    if (!ftRotatorReadyForPendingTx(&initialRotatorWaitReason, &initialRotatorEtaMs)) {
        deferPendingFtTxForRotator(initialRotatorEtaMs, initialRotatorWaitReason);
        return;
    }
...
void MainWindow::deferPendingFtTxForRotator(int etaMs, const QString &reason)
{
    ...
    const quint64 deferGeneration = ++m_ft8RotatorDeferGeneration;
    const QString cleanReason = reason.trimmed().isEmpty()
    ...
    QTimer::singleShot(delay, this, [this, txMessage, txTag, deferGeneration]() {
        if (deferGeneration != m_ft8RotatorDeferGeneration || m_ft8OperatorStopLatched) {
            return;
        }
        if (m_txRunning || m_ftTxWorkerRunning) {
            return;
