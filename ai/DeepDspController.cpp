...
constexpr int kMinFtAssistSamples = 768;
constexpr int kMinFtAssistPositiveSamples = 24;
constexpr int kMinFtAssistNegativeSamples = 96;
...
bool mindAssistReadyForFt(int validationCount,
                          int replaySamples,
                          int positiveSamples,
                          int negativeSamples,
                          double classifierAccuracy,
                          double bestClassifierAccuracy)
{
    /*
     * MIND Ranker readiness is per profile (FT8 and FT4 mature separately).
     * A fresh user/profile starts in Learning: MIND may score and collect
     * telemetry, but it must not prune or open extra recovery until the local
     * positive/negative dataset and validation window prove stable.
     */
    return replaySamples >= kMinFtAssistSamples &&
           positiveSamples >= kMinFtAssistPositiveSamples &&
           negativeSamples >= kMinFtAssistNegativeSamples &&
           validationCount >= kMinReadyValidation &&
           classifierAccuracy >= 84.0 &&
           bestClassifierAccuracy >= 88.0;
}

QString mindAssistLifecycleStateForFt(int validationCount,
                                      int replaySamples,
                                      int positiveSamples,
                                      int negativeSamples,
                                      double classifierAccuracy,
                                      double bestClassifierAccuracy,
                                      bool assistRequested)
{
    if (mindAssistReadyForFt(validationCount, replaySamples, positiveSamples, negativeSamples,
                             classifierAccuracy, bestClassifierAccuracy)) {
        return assistRequested ? QStringLiteral("Assist Active") : QStringLiteral("Assist Ready");
    }
    if (replaySamples < 32) {
        return QStringLiteral("Cold");
    }
    if (replaySamples < kMinFtAssistSamples ||
        positiveSamples < kMinFtAssistPositiveSamples ||
        negativeSamples < kMinFtAssistNegativeSamples) {
...
QString mindAssistReadinessReason(int validationCount,
                                  int replaySamples,
                                  int positiveSamples,
                                  int negativeSamples,
                                  double classifierAccuracy,
...
{
    if (replaySamples < 32) {
        return QStringLiteral("cold start: collecting first ranker samples");
    }
    if (replaySamples < kMinFtAssistSamples) {
        return QStringLiteral("learning: collecting FT candidate samples");
    }
    if (positiveSamples < kMinFtAssistPositiveSamples) {
        return QStringLiteral("learning: collecting successful FT decodes");
    }
    if (negativeSamples < kMinFtAssistNegativeSamples) {
        return QStringLiteral("learning: collecting failed FT candidates");
    }
    if (validationCount < kMinReadyValidation) {
        return QStringLiteral("validating ranker stability");
    }
    if (classifierAccuracy < 84.0 || bestClassifierAccuracy < 88.0) {
        return QStringLiteral("waiting for stable ranker accuracy");
    }
...
DeepDspController::Status DeepDspController::status() const
{
    ...
    auto polarityStatsForProfile = [this](const QString &profile, int *positive, int *negative) {
        const QString d = normalizedDomain(profile);
        int pos = 0;
        int neg = 0;
        for (const Sample &sample : m_samples) {
            if (normalizedDomain(sample.mode) != d || sample.target.isEmpty()) continue;
            if (sample.target.constFirst() >= 0.5f) ++pos;
            else ++neg;
        }
        if (positive != nullptr) *positive = pos;
    ...
    polarityStatsForProfile(statusProfile, &s.activeProfilePositiveSamples, &s.activeProfileNegativeSamples);
    ...
    if (mskProfile) {
        ...
        s.readinessReason = mindAssistReadinessReasonForMsk144(s.activeProfileSamples,
                                                               ...
                                                               s.activeProfileRankerAccuracy,
                                                               s.activeProfileBestRankerAccuracy);
        s.activeProfileLifecycleState = s.assistReady
            ? (s.assistRequested ? QStringLiteral("Assist Active") : QStringLiteral("Assist Ready"))
            : (s.activeProfileSamples < 32 ? QStringLiteral("Cold")
                                           : (s.activeProfileValidationCount < kMinMsk144ReadyValidation
                                              ? QStringLiteral("Learning") : QStringLiteral("Validating")));
    } else {
        s.assistReady = mindAssistReadyForFt(s.activeProfileValidationCount,
                                             s.activeProfileSamples,
                                             s.activeProfilePositiveSamples,
                                             s.activeProfileNegativeSamples,
                                             s.activeProfileRankerAccuracy,
                                             s.activeProfileBestRankerAccuracy);
        s.readinessReason = mindAssistReadinessReason(s.activeProfileValidationCount,
                                                       s.activeProfileSamples,
                                                       s.activeProfilePositiveSamples,
                                                       s.activeProfileNegativeSamples,
                                                       s.activeProfileRankerAccuracy,
                                                       s.activeProfileBestRankerAccuracy);
        s.activeProfileLifecycleState = mindAssistLifecycleStateForFt(s.activeProfileValidationCount,
                                                                      s.activeProfileSamples,
                                                                      s.activeProfilePositiveSamples,
                                                                      s.activeProfileNegativeSamples,
                                                                      s.activeProfileRankerAccuracy,
    ...
    if (mskProfile) {
        ...
        const double sampleReadiness = qMin(1.0, static_cast<double>(s.activeProfileValidationCount) / static_cast<double>(kMinReadyValidation));
        const double ftGoldReadiness = qMin(1.0, static_cast<double>(s.activeProfileSamples) / 768.0);
        const double classifierProgress = qBound(0.0, (s.activeProfileRankerAccuracy - 50.0) / 34.0, 1.0);
...
void DeepDspController::setAssistMode(const QString &mode)
{
    ...
    QString profile;
    {
        ...
        if (profile == QStringLiteral("MSK144")) {
            ...
            int profileValidationCount = 0;
            double profileAccuracySum = 0.0;
            double profileBestAccuracy = 0.0;
            auto modeIt = m_validationModeWindow.begin();
            auto accIt = m_bitAccuracyWindow.begin();
            for (; modeIt != m_validationModeWindow.end() && accIt != m_bitAccuracyWindow.end(); ++modeIt, ++accIt) {
                if (normalizedDomain(*modeIt) != profile) continue;
                ++profileValidationCount;
                profileAccuracySum += *accIt;
                if (*accIt > profileBestAccuracy) profileBestAccuracy = *accIt;
            }
            const double profileAccuracy = profileValidationCount > 0
                ? profileAccuracySum / static_cast<double>(profileValidationCount)
                : 0.0;
            int profileSamples = 0;
            int profilePos = 0;
            int profileNeg = 0;
            for (const Sample &sample : m_samples) {
                if (normalizedDomain(sample.mode) != profile) continue;
                ++profileSamples;
                if (sample.target.isEmpty()) continue;
                if (sample.target.constFirst() >= 0.5f) ++profilePos;
                else ++profileNeg;
            }
            readyNow = mindAssistReadyForFt(profileValidationCount,
                                            profileSamples,
                                            profilePos,
                                            profileNeg,
                                            profileAccuracy,
                                            profileBestAccuracy);
            readinessReason = mindAssistReadinessReason(profileValidationCount,
                                                         profileSamples,
                                                         profilePos,
                                                         profileNeg,
                                                         profileAccuracy,
...
void DeepDspController::saveStats()
{
    ...
    const double mskClassifierAccuracy = mskValidationCount > 0
        ? mskAccuracySum / static_cast<double>(mskValidationCount)
        : 0.0;
    auto ftValidationStatsFor = [this](const QString &profile, int *count, double *avg, double *best) {
        const QString d = normalizedDomain(profile);
        int n = 0;
        double sum = 0.0;
        double maxv = 0.0;
        auto modeIt2 = m_validationModeWindow.begin();
        auto accIt2 = m_bitAccuracyWindow.begin();
        for (; modeIt2 != m_validationModeWindow.end() && accIt2 != m_bitAccuracyWindow.end(); ++modeIt2, ++accIt2) {
            if (normalizedDomain(*modeIt2) != d) continue;
            ++n;
            sum += *accIt2;
            if (*accIt2 > maxv) maxv = *accIt2;
        }
        if (count != nullptr) *count = n;
        if (avg != nullptr) *avg = (n > 0) ? (sum / static_cast<double>(n)) : 0.0;
        if (best != nullptr) *best = maxv;
    };
    auto ftPolarityStatsFor = [this](const QString &profile, int *positive, int *negative) {
        const QString d = normalizedDomain(profile);
        int pos = 0;
        int neg = 0;
        for (const Sample &sample : m_samples) {
    ...
    int ft8ValidationCount = 0;
    double ft8ClassifierAccuracy = 0.0;
    double ft8BestAccuracy = 0.0;
    int ft8PositiveSamples = 0;
    int ft8NegativeSamples = 0;
    ftValidationStatsFor(QStringLiteral("FT8"), &ft8ValidationCount, &ft8ClassifierAccuracy, &ft8BestAccuracy);
    ftPolarityStatsFor(QStringLiteral("FT8"), &ft8PositiveSamples, &ft8NegativeSamples);
    int ft4ValidationCount = 0;
    double ft4ClassifierAccuracy = 0.0;
    double ft4BestAccuracy = 0.0;
    int ft4PositiveSamples = 0;
    int ft4NegativeSamples = 0;
    ftValidationStatsFor(QStringLiteral("FT4"), &ft4ValidationCount, &ft4ClassifierAccuracy, &ft4BestAccuracy);
    ftPolarityStatsFor(QStringLiteral("FT4"), &ft4PositiveSamples, &ft4NegativeSamples);
    ...
    const bool assistReady = (activeStatsProfile == QStringLiteral("MSK144"))
        ...
        : (activeStatsProfile == QStringLiteral("FT4")
            ? mindAssistReadyForFt(ft4ValidationCount, m_ft4Samples, ft4PositiveSamples, ft4NegativeSamples, ft4ClassifierAccuracy, ft4BestAccuracy)
            : mindAssistReadyForFt(ft8ValidationCount, m_ft8Samples, ft8PositiveSamples, ft8NegativeSamples, ft8ClassifierAccuracy, ft8BestAccuracy));
    obj.insert(QStringLiteral("ft8_validation_count"), ft8ValidationCount);
    obj.insert(QStringLiteral("ft8_positive_samples"), ft8PositiveSamples);
    obj.insert(QStringLiteral("ft8_negative_samples"), ft8NegativeSamples);
    obj.insert(QStringLiteral("ft8_ranker_accuracy_percent"), ft8ClassifierAccuracy);
    obj.insert(QStringLiteral("ft8_best_ranker_accuracy_percent"), ft8BestAccuracy);
    obj.insert(QStringLiteral("ft4_validation_count"), ft4ValidationCount);
    obj.insert(QStringLiteral("ft4_positive_samples"), ft4PositiveSamples);
    obj.insert(QStringLiteral("ft4_negative_samples"), ft4NegativeSamples);
    obj.insert(QStringLiteral("ft4_ranker_accuracy_percent"), ft4ClassifierAccuracy);
    obj.insert(QStringLiteral("ft4_best_ranker_accuracy_percent"), ft4BestAccuracy);
    ...
    obj.insert(QStringLiteral("assist_readiness_reason"),
               activeStatsProfile == QStringLiteral("MSK144")
                   ...
                   : (activeStatsProfile == QStringLiteral("FT4")
                      ? mindAssistReadinessReason(ft4ValidationCount, m_ft4Samples, ft4PositiveSamples, ft4NegativeSamples, ft4ClassifierAccuracy, ft4BestAccuracy)
                      : mindAssistReadinessReason(ft8ValidationCount, m_ft8Samples, ft8PositiveSamples, ft8NegativeSamples, ft8ClassifierAccuracy, ft8BestAccuracy)));
...
void DeepDspController::loadStats()
{
    ...
    auto restoreProfileWindow = [this](const QString &profile, int count, double accuracyPercent) {
        if (count <= 0 || accuracyPercent <= 0.0) return;
        const int remaining = qMax(0, 1000 - static_cast<int>(m_validationModeWindow.size()));
        const int cappedCount = qMin(count, remaining);
        if (cappedCount <= 0) return;
        const int okCount = qBound(0, static_cast<int>(std::lround((accuracyPercent / 100.0) * cappedCount)), cappedCount);
        for (int i = 0; i < cappedCount; ++i) {
            m_bitAccuracyWindow.push_back(accuracyPercent);
            m_validationWindow.push_back(i < okCount);
            m_validationModeWindow.push_back(normalizedDomain(profile));
        }
    };

    const int ft8ValidationCount = obj.value(QStringLiteral("ft8_validation_count")).toInt(0);
    const double ft8Accuracy = obj.value(QStringLiteral("ft8_ranker_accuracy_percent")).toDouble(0.0);
    const int ft4ValidationCount = obj.value(QStringLiteral("ft4_validation_count")).toInt(0);
    const double ft4Accuracy = obj.value(QStringLiteral("ft4_ranker_accuracy_percent")).toDouble(0.0);
    const int mskValidationCount = obj.value(QStringLiteral("msk144_validation_count")).toInt(0);
    const double mskAccuracy = obj.value(QStringLiteral("msk144_ranker_accuracy_percent")).toDouble(0.0);

    if (ft8ValidationCount > 0 || ft4ValidationCount > 0 || mskValidationCount > 0) {
        restoreProfileWindow(QStringLiteral("FT8"), ft8ValidationCount, ft8Accuracy);
        restoreProfileWindow(QStringLiteral("FT4"), ft4ValidationCount, ft4Accuracy);
        restoreProfileWindow(QStringLiteral("MSK144"), mskValidationCount, mskAccuracy);
    } else {
        // Legacy compact stats belong to the FT8-era ranker.
        restoreProfileWindow(QStringLiteral("FT8"), loadedValidationCount, m_loadedBitAccuracyPercent);
    }
...
bool DeepDspController::scoreNativeFtCandidate(const QString &mode,
                                               const QVector<float> &candidateMagnitudes,
                                               ...
                                               double *confidencePercent)
{
    ...
    QMutexLocker locker(&m_mutex);
    const QString assistMode = normalizedAssistMode(m_assistMode);
    const QString domain = normalizedDomain(mode);
    if (!m_enabled || assistMode == QStringLiteral("off") || m_network == nullptr ||
        (domain != QStringLiteral("FT8") && domain != QStringLiteral("FT4"))) {
        return false;
    }
    ...
    auto accIt = m_bitAccuracyWindow.begin();
    for (; modeIt != m_validationModeWindow.end() && accIt != m_bitAccuracyWindow.end(); ++modeIt, ++accIt) {
        if (normalizedDomain(*modeIt) != domain) continue;
        ++profileValidationCount;
        profileAccuracySum += *accIt;
        if (*accIt > profileBestAccuracy) profileBestAccuracy = *accIt;
    }
    const double profileClassifierAccuracy = profileValidationCount > 0
        ? profileAccuracySum / static_cast<double>(profileValidationCount)
        : 0.0;

    int profileSamples = 0;
    int profilePositiveSamples = 0;
    int profileNegativeSamples = 0;
    for (const Sample &s : m_samples) {
        if (normalizedDomain(s.mode) != domain) continue;
        ++profileSamples;
        if (s.target.isEmpty()) continue;
        if (s.target.constFirst() >= 0.5f) ++profilePositiveSamples;
        else ++profileNegativeSamples;
    }

    const bool readyNow = mindAssistReadyForFt(profileValidationCount,
                                               profileSamples,
                                               profilePositiveSamples,
                                               profileNegativeSamples,
                                               profileClassifierAccuracy,
                                               profileBestAccuracy);
    m_assistEnabled = (assistMode == QStringLiteral("assisted")) && readyNow;
    m_activeProfile = domain;

    // MIND Phase 2: scoring is per FT profile.  FT8 and FT4 may share the same
    // lightweight network file for now, but the readiness/prune gate is no
    // longer borrowed from the other mode.  This makes FT4 MIND telemetry
    // measurable without pretending that an FT8-trained ranker is FT4-ready.
    ...
    if (predictedBits != nullptr) {
        ...
        out.append((assistMode == QStringLiteral("assisted") && readyNow) ? 1.0f : 0.0f);
