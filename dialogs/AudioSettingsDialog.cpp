    ...
    m_settings(initialSettings)
{
    ...
    // 0.5.78-lab5: in the unified full-screen Settings workbench there is
    // enough horizontal room.  Put Audio devices and PTT control side by side
    // instead of stacking two wide blocks vertically.
    if (ui->mainLayout != nullptr && ui->grpAudio != nullptr && ui->grpPtt != nullptr) {
        const int insertAt = qMax(0, ui->mainLayout->indexOf(ui->grpAudio));
        ui->mainLayout->removeWidget(ui->grpAudio);
        ui->mainLayout->removeWidget(ui->grpPtt);
        QHBoxLayout *audioPttRow = new QHBoxLayout;
        audioPttRow->setContentsMargins(0, 0, 0, 0);
        audioPttRow->setSpacing(12);
        ui->grpAudio->setMinimumWidth(420);
        ui->grpPtt->setMinimumWidth(330);
        ui->cmbAudioInput->setMinimumWidth(220);
        ui->cmbAudioOutput->setMinimumWidth(220);
        ui->cmbSampleRate->setMinimumWidth(180);
        ui->cmbPttMethod->setMinimumWidth(190);
        ui->cmbPttPort->setMinimumWidth(190);
        audioPttRow->addWidget(ui->grpAudio, 3);
        audioPttRow->addWidget(ui->grpPtt, 2);
        ui->mainLayout->insertLayout(insertAt, audioPttRow);
    }

    // Compact, native-size dialog: the v1.45 high-DPI scaler made this
    // window much too tall on 1920x1080 Windows desktops.
    resize(860, 240);
    setMinimumSize(760, 220);
