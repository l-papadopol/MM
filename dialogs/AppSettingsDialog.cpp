      ...
      m_schedulerTranslator(std::move(schedulerTranslator))
{
    ...
    if (QTabBar *bar = m_tabs->tabBar()) {
        ...
        // 0.5.78-lab5: keep all Settings tabs visible on compact/fullscreen
        // layouts.  The global cockpit stylesheet is intentionally roomy, but
        // the long tab labels were clipping the rightmost tab on 1366px-wide
        // desktops.  Reduce horizontal chrome without changing label text.
        bar->setStyleSheet(QStringLiteral("QTabBar::tab { min-width: 72px; padding-left: 5px; padding-right: 5px; margin-right: 0px; }"));
    }
    outer->addWidget(m_tabs, 1);

    // Settings opened in about 1.5 s because every heavy page was constructed
    // before the dialog became visible.  Keep the everyday pages immediate and
    // lazily build the heavy pages only when the user actually opens them.
    // collectSettings() already preserves m_initialSettings for pages that were
    // never built, so closing Settings no longer commits blank/default data for
    // deferred pages.
    auto shouldBuildNow = [initialPage](InitialPage tabPage) {
        if (tabPage == InitialPage::UserQthMacros ||
            tabPage == InitialPage::AudioPtt ||
            tabPage == InitialPage::Logbook) {
            return true;
        }
        if (tabPage == initialPage) {
            return true;
        }
        if (tabPage == InitialPage::AudioPtt && initialPage == InitialPage::RadioCat) {
            return true;
        }
        if (tabPage == InitialPage::Logbook && initialPage == InitialPage::Appearance) {
            return true;
        }
        return false;
    };

    addSettingsTab(InitialPage::UserQthMacros, L(QStringLiteral("User / QTH / Macros")), shouldBuildNow(InitialPage::UserQthMacros));
    addSettingsTab(InitialPage::AudioPtt, L(QStringLiteral("Audio / PTT / CAT")), shouldBuildNow(InitialPage::AudioPtt));
    addSettingsTab(InitialPage::Logbook, L(QStringLiteral("Logbook / FT colours")), shouldBuildNow(InitialPage::Logbook));
    addSettingsTab(InitialPage::Rotator, L(QStringLiteral("Rotator")), shouldBuildNow(InitialPage::Rotator));
    m_autoQsoFlowTabIndex = addSettingsTab(InitialPage::AutoQsoFlow, L(QStringLiteral("MM Flow Studio")), shouldBuildNow(InitialPage::AutoQsoFlow));
    addSettingsTab(InitialPage::Scheduler, L(QStringLiteral("Scheduler")), shouldBuildNow(InitialPage::Scheduler));
    addSettingsTab(InitialPage::SoundCardCalibration, L(QStringLiteral("Soundcard calibration")), shouldBuildNow(InitialPage::SoundCardCalibration));

    connect(m_tabs, &QTabWidget::currentChanged, this, [this](int index) {
        ensureSettingsTabBuilt(index);
        updateAutoQsoFlowWindowMode(index);
    });
...
QWidget *AppSettingsDialog::makeDeferredSettingsPage(const QString &title)
{
    ...
    QLabel *label = new QLabel(L(QStringLiteral("This Settings page will be loaded when selected.")), page);
    label->setAlignment(Qt::AlignCenter);
    label->setWordWrap(true);
    label->setProperty("cockpitDeferredSettingsPage", true);
    QFont font = label->font();
    font.setPointSizeF(qMax(10.0, font.pointSizeF() + 1.0));
    label->setFont(font);
    layout->addWidget(label);

    QLabel *name = new QLabel(title, page);
    name->setAlignment(Qt::AlignCenter);
    name->setProperty("cockpitDeferredSettingsPageTitle", true);
...
QWidget *AppSettingsDialog::buildSettingsPage(InitialPage page)
{
    switch (page) {
    case InitialPage::UserQthMacros:
        return makeUserQthMacrosPage();
    case InitialPage::AudioPtt:
    case InitialPage::RadioCat:
        return makeAudioCatPage();
    case InitialPage::Logbook:
    case InitialPage::Appearance:
        return makeLogbookPage();
    case InitialPage::Rotator:
        m_rotatorPage = makeRotatorPage();
        applyStoredRotatorConnectionStatuses();
        return m_rotatorPage;
    case InitialPage::AutoQsoFlow:
        return makeAutoQsoFlowPage();
    case InitialPage::Scheduler:
        m_schedulerPage = new BandSchedulerDialog(m_initialSchedulerEntries,
                                                  m_schedulerTranslator,
                                                  this);
        hideEmbeddedIntroLabels(m_schedulerPage);
        return embedDialogPage(m_schedulerPage);
    case InitialPage::SoundCardCalibration:
        m_calibrationPage = new SoundCardCalibrationDialog(m_initialSettings, this);
        m_calibrationPage->setTextTranslator([this](const QString &source) { return L(source); });
        return embedDialogPage(m_calibrationPage);
    }
    return makeDeferredSettingsPage(L(QStringLiteral("Settings")));
}

int AppSettingsDialog::addSettingsTab(InitialPage page, const QString &title, bool buildNow)
{
    QWidget *widget = buildNow ? buildSettingsPage(page) : makeDeferredSettingsPage(title);
    const int index = m_tabs != nullptr ? m_tabs->addTab(widget, title) : -1;
    if (!buildNow && index >= 0) {
        m_deferredSettingsTabs.insert(index, page);
    }
    return index;
}

void AppSettingsDialog::ensureSettingsTabBuilt(int index)
{
    if (m_tabs == nullptr || !m_deferredSettingsTabs.contains(index)) {
        return;
    }

    const InitialPage page = m_deferredSettingsTabs.take(index);
    const QString tabText = m_tabs->tabText(index);
    const QIcon tabIcon = m_tabs->tabIcon(index);
    QWidget *oldPage = m_tabs->widget(index);
    ...
    QWidget *newPage = buildSettingsPage(page);
    m_tabs->insertTab(index, newPage, tabIcon, tabText);
    if (page == InitialPage::AutoQsoFlow) {
        m_autoQsoFlowTabIndex = index;
    }
...
QWidget *AppSettingsDialog::makeUserQthMacrosPage()
{
    ...
    QGroupBox *coordinatesGroup = new QGroupBox(L(QStringLiteral("Precise station coordinates")), variablesContent);
    coordinatesGroup->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Maximum);
    QGridLayout *coordinatesGrid = new QGridLayout(coordinatesGroup);
    coordinatesGrid->setContentsMargins(10, 12, 10, 10);
    coordinatesGrid->setHorizontalSpacing(8);
    coordinatesGrid->setVerticalSpacing(6);
    coordinatesGrid->setColumnStretch(1, 1);
    coordinatesGrid->setColumnStretch(3, 1);

    m_chkUsePreciseCoordinates = new QCheckBox(L(QStringLiteral("Use precise latitude/longitude instead of locator centre")), coordinatesGroup);
    m_chkUsePreciseCoordinates->setChecked(m_initialSettings.textMyCoordinatesEnabled);
    m_chkUsePreciseCoordinates->setToolTip(L(QStringLiteral("When enabled, rotator and map modules may use exact station coordinates instead of the approximate Maidenhead locator centre.")));

    m_spinMyLatitude = new QDoubleSpinBox(coordinatesGroup);
    m_spinMyLatitude->setRange(-90.0, 90.0);
    m_spinMyLatitude->setDecimals(6);
    m_spinMyLatitude->setSuffix(QStringLiteral(" °"));
    m_spinMyLatitude->setSingleStep(0.000100);
    m_spinMyLatitude->setValue(qBound(-90.0, m_initialSettings.textMyLatitudeDeg, 90.0));
    m_spinMyLatitude->setToolTip(L(QStringLiteral("Station latitude in decimal degrees. North is positive, south is negative.")));

    m_spinMyLongitude = new QDoubleSpinBox(coordinatesGroup);
    m_spinMyLongitude->setRange(-180.0, 180.0);
    m_spinMyLongitude->setDecimals(6);
    m_spinMyLongitude->setSuffix(QStringLiteral(" °"));
    m_spinMyLongitude->setSingleStep(0.000100);
    m_spinMyLongitude->setValue(qBound(-180.0, m_initialSettings.textMyLongitudeDeg, 180.0));
    m_spinMyLongitude->setToolTip(L(QStringLiteral("Station longitude in decimal degrees. East is positive, west is negative.")));

    auto updateCoordinateEditors = [this]() {
        const bool enabled = (m_chkUsePreciseCoordinates != nullptr && m_chkUsePreciseCoordinates->isChecked());
        if (m_spinMyLatitude != nullptr) m_spinMyLatitude->setEnabled(enabled);
        if (m_spinMyLongitude != nullptr) m_spinMyLongitude->setEnabled(enabled);
    };
    connect(m_chkUsePreciseCoordinates, &QCheckBox::toggled, this, [updateCoordinateEditors](bool) { updateCoordinateEditors(); });
    updateCoordinateEditors();

    coordinatesGrid->addWidget(m_chkUsePreciseCoordinates, 0, 0, 1, 4);
    coordinatesGrid->addWidget(new QLabel(L(QStringLiteral("Latitude")), coordinatesGroup), 1, 0);
    coordinatesGrid->addWidget(m_spinMyLatitude, 1, 1);
    coordinatesGrid->addWidget(new QLabel(L(QStringLiteral("Longitude")), coordinatesGroup), 1, 2);
    coordinatesGrid->addWidget(m_spinMyLongitude, 1, 3);

    QLabel *coordinatesHint = new QLabel(L(QStringLiteral("Leave disabled to derive the station position from the Maidenhead locator.")), coordinatesGroup);
    coordinatesHint->setWordWrap(true);
    coordinatesGrid->addWidget(coordinatesHint, 2, 0, 1, 4);
    ...
    variablesLayout->addWidget(coordinatesGroup);
...
QWidget *AppSettingsDialog::makeLogbookPage()
{
    ...
    appearancePanel->setParent(holder);
    appearancePanel->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Maximum);
    logbookGroup->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Maximum);
    topRow->addWidget(logbookGroup, 1);
    topRow->addWidget(appearancePanel, 1);
    ...
    m_comboDecodeTablePreset = new QComboBox(decodeTableGroup);
    m_comboDecodeTablePreset->addItem(L(QStringLiteral("Custom")), QStringLiteral("custom"));
    m_comboDecodeTablePreset->addItem(L(QStringLiteral("Compact")), QStringLiteral("compact"));
    m_comboDecodeTablePreset->addItem(L(QStringLiteral("Normal")), QStringLiteral("normal"));
    m_comboDecodeTablePreset->addItem(L(QStringLiteral("Large")), QStringLiteral("large"));
    m_comboDecodeTablePreset->addItem(L(QStringLiteral("Low vision")), QStringLiteral("low_vision"));
    m_comboDecodeTablePreset->setToolTip(L(QStringLiteral("Choose a readability preset for FT/MSK/Q65 decode and message tables.")));

    m_comboDecodeTableFontFamily = new QComboBox(decodeTableGroup);
    m_comboDecodeTableFontFamily->setEditable(false);
    m_comboDecodeTableFontFamily->addItem(L(QStringLiteral("Use global UI font")), QString());
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
    const QStringList decodeFamilies = QFontDatabase::families();
#else
    const QStringList decodeFamilies = QFontDatabase().families();
#endif
    for (const QString &family : decodeFamilies) {
        m_comboDecodeTableFontFamily->addItem(family, family);
    }
    const QString decodeFamily = m_initialSettings.decodeTableFontFamily.trimmed();
    if (decodeFamily.isEmpty()) {
        m_comboDecodeTableFontFamily->setCurrentIndex(0);
    } else {
        int decodeFontIndex = m_comboDecodeTableFontFamily->findText(decodeFamily, Qt::MatchFixedString);
        if (decodeFontIndex < 0) {
            m_comboDecodeTableFontFamily->insertItem(1, decodeFamily, decodeFamily);
            decodeFontIndex = 1;
        }
        m_comboDecodeTableFontFamily->setCurrentIndex(decodeFontIndex);
    }
    m_comboDecodeTableFontFamily->setToolTip(L(QStringLiteral("Font family used only by FT/MSK/Q65 decode tables. Use the global UI font to inherit the selected application font.")));
    ...
    auto setPresetComboToCustom = [this]() {
        if (m_comboDecodeTablePreset == nullptr) return;
        if (m_comboDecodeTablePreset->currentData().toString() == QStringLiteral("custom")) return;
        const int customIndex = m_comboDecodeTablePreset->findData(QStringLiteral("custom"));
        if (customIndex >= 0) {
            m_comboDecodeTablePreset->setCurrentIndex(customIndex);
        }
    };
    auto applyDecodePreset = [this](const QString &preset) {
        if (preset == QStringLiteral("custom") || m_spinDecodeTableFontSize == nullptr ||
            m_spinDecodeTableRowHeight == nullptr || m_comboDecodeTableFontFamily == nullptr) {
            return;
        }
        int fontPt = 9;
        int rowPx = 20;
        if (preset == QStringLiteral("compact")) {
            fontPt = 8;
            rowPx = 16;
        } else if (preset == QStringLiteral("large")) {
            fontPt = 12;
            rowPx = 28;
        } else if (preset == QStringLiteral("low_vision")) {
            fontPt = 14;
            rowPx = 36;
        }
        const QSignalBlocker b1(m_spinDecodeTableFontSize);
        const QSignalBlocker b2(m_spinDecodeTableRowHeight);
        const QSignalBlocker b3(m_comboDecodeTableFontFamily);
        m_spinDecodeTableFontSize->setValue(fontPt);
        m_spinDecodeTableRowHeight->setValue(rowPx);
        m_comboDecodeTableFontFamily->setCurrentIndex(0);
    };
    connect(m_comboDecodeTablePreset, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            [this, applyDecodePreset](int) {
                if (m_comboDecodeTablePreset != nullptr) {
                    applyDecodePreset(m_comboDecodeTablePreset->currentData().toString());
                }
            });
    connect(m_spinDecodeTableFontSize, QOverload<int>::of(&QSpinBox::valueChanged), this,
            [setPresetComboToCustom](int) { setPresetComboToCustom(); });
    connect(m_spinDecodeTableRowHeight, QOverload<int>::of(&QSpinBox::valueChanged), this,
            [setPresetComboToCustom](int) { setPresetComboToCustom(); });
    connect(m_comboDecodeTableFontFamily, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            [setPresetComboToCustom](int) { setPresetComboToCustom(); });

    auto deriveDecodePreset = [this]() -> QString {
        const QString stored = m_initialSettings.decodeTablePreset.trimmed().toLower();
        const QString family = m_initialSettings.decodeTableFontFamily.trimmed();
        const int fontPt = qBound(8, m_initialSettings.decodeTableFontPointSize, 18);
        const int rowPx = qBound(16, m_initialSettings.decodeTableRowHeightPx, 48);
        if (family.isEmpty()) {
            if (fontPt == 8 && rowPx == 16) return QStringLiteral("compact");
            if (fontPt == 9 && rowPx == 20) return QStringLiteral("normal");
            if (fontPt == 12 && rowPx == 28) return QStringLiteral("large");
            if (fontPt == 14 && rowPx == 36) return QStringLiteral("low_vision");
        }
        if (stored == QStringLiteral("compact") || stored == QStringLiteral("normal") ||
            stored == QStringLiteral("large") || stored == QStringLiteral("low_vision")) {
            return stored;
        }
        return QStringLiteral("custom");
    };
    const int presetIndex = m_comboDecodeTablePreset->findData(deriveDecodePreset());
    if (presetIndex >= 0) {
        const QSignalBlocker blocker(m_comboDecodeTablePreset);
        m_comboDecodeTablePreset->setCurrentIndex(presetIndex);
    }

    decodeTableGrid->addWidget(new QLabel(L(QStringLiteral("Preset")), decodeTableGroup), 0, 0);
    decodeTableGrid->addWidget(m_comboDecodeTablePreset, 0, 1, 1, 3);
    decodeTableGrid->addWidget(new QLabel(L(QStringLiteral("Decode font family")), decodeTableGroup), 1, 0);
    decodeTableGrid->addWidget(m_comboDecodeTableFontFamily, 1, 1, 1, 3);
    decodeTableGrid->addWidget(new QLabel(L(QStringLiteral("Font size")), decodeTableGroup), 2, 0);
    decodeTableGrid->addWidget(m_spinDecodeTableFontSize, 2, 1);
    decodeTableGrid->addWidget(new QLabel(L(QStringLiteral("Row height")), decodeTableGroup), 2, 2);
    decodeTableGrid->addWidget(m_spinDecodeTableRowHeight, 2, 3);
...
QWidget *AppSettingsDialog::makeAppearancePage()
{
    ...
    QGroupBox *themeGroup = new QGroupBox(L(QStringLiteral("General UI theme")), holder);
    auto *themeGrid = new QGridLayout(themeGroup);
    themeGrid->setColumnStretch(1, 1);

    m_comboUiTheme = new QComboBox(themeGroup);
    m_comboUiTheme->addItem(L(QStringLiteral("Avionica")), QStringLiteral("avionica"));
    m_comboUiTheme->addItem(L(QStringLiteral("Qt Default")), QStringLiteral("qt_default"));
    m_comboUiTheme->addItem(L(QStringLiteral("Hacker Green")), QStringLiteral("hacker_green"));
    m_comboUiTheme->addItem(L(QStringLiteral("Classic Dark")), QStringLiteral("classic_dark"));
    m_comboUiTheme->addItem(L(QStringLiteral("High Contrast")), QStringLiteral("high_contrast"));
    const int themeIndex = m_comboUiTheme->findData(m_initialSettings.uiTheme.trimmed().isEmpty()
                                                        ? QStringLiteral("avionica")
                                                        : m_initialSettings.uiTheme.trimmed().toLower());
    m_comboUiTheme->setCurrentIndex(themeIndex >= 0 ? themeIndex : 0);
    m_comboUiTheme->setToolTip(L(QStringLiteral("Select the global MadModem colour palette.")));

    m_chkUiUseThemeFont = new QCheckBox(L(QStringLiteral("Use theme default font")), themeGroup);
    m_chkUiUseThemeFont->setChecked(m_initialSettings.uiUseThemeFont);
    m_chkUiUseThemeFont->setToolTip(L(QStringLiteral("When enabled, each theme applies its recommended UI font. Disable it to choose a custom font family and size.")));

    m_comboUiFontFamily = new QComboBox(themeGroup);
    m_comboUiFontFamily->setEditable(false);
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
    ...
    for (const QString &family : families) {
        m_comboUiFontFamily->addItem(family, family);
    }
    QString initialFamily = m_initialSettings.uiFontFamily.trimmed();
    if (initialFamily.isEmpty()) {
        initialFamily = QApplication::font().family();
    }
    int fontIndex = m_comboUiFontFamily->findText(initialFamily, Qt::MatchFixedString);
    if (fontIndex < 0) {
        m_comboUiFontFamily->insertItem(0, initialFamily, initialFamily);
        fontIndex = 0;
    }
    m_comboUiFontFamily->setCurrentIndex(fontIndex);
    m_comboUiFontFamily->setToolTip(L(QStringLiteral("Global font family for labels, buttons, tabs and settings pages when theme default font is disabled.")));

    m_spinUiFontSize = new QSpinBox(themeGroup);
    m_spinUiFontSize->setRange(8, 24);
    m_spinUiFontSize->setSuffix(QStringLiteral(" pt"));
    m_spinUiFontSize->setValue(qBound(8, m_initialSettings.uiFontPointSize, 24));
    m_spinUiFontSize->setToolTip(L(QStringLiteral("Global UI font size. Use a larger value when controls or labels are difficult to read.")));

    auto updateFontOverrideEnabled = [this]() {
        const bool custom = (m_chkUiUseThemeFont == nullptr || !m_chkUiUseThemeFont->isChecked());
        if (m_comboUiFontFamily != nullptr) m_comboUiFontFamily->setEnabled(custom);
        if (m_spinUiFontSize != nullptr) m_spinUiFontSize->setEnabled(custom);
    };
    connect(m_chkUiUseThemeFont, &QCheckBox::toggled, this, [updateFontOverrideEnabled](bool) { updateFontOverrideEnabled(); });
    updateFontOverrideEnabled();

    QPushButton *reset = new QPushButton(L(QStringLiteral("Reset theme defaults")), themeGroup);
    reset->setToolTip(L(QStringLiteral("Restore Avionica theme and theme-managed font settings.")));
    connect(reset, &QPushButton::clicked, this, [this, updateFontOverrideEnabled]() {
        if (m_comboUiTheme != nullptr) {
            const int idx = m_comboUiTheme->findData(QStringLiteral("avionica"));
            if (idx >= 0) m_comboUiTheme->setCurrentIndex(idx);
        }
        if (m_chkUiUseThemeFont != nullptr) m_chkUiUseThemeFont->setChecked(true);
        if (m_spinUiFontSize != nullptr) m_spinUiFontSize->setValue(10);
        updateFontOverrideEnabled();
    });

    themeGrid->addWidget(new QLabel(L(QStringLiteral("Theme")), themeGroup), 0, 0);
    themeGrid->addWidget(m_comboUiTheme, 0, 1, 1, 2);
    themeGrid->addWidget(m_chkUiUseThemeFont, 1, 0, 1, 3);
    themeGrid->addWidget(new QLabel(L(QStringLiteral("Font family")), themeGroup), 2, 0);
    themeGrid->addWidget(m_comboUiFontFamily, 2, 1, 1, 2);
    themeGrid->addWidget(new QLabel(L(QStringLiteral("Font size")), themeGroup), 3, 0);
    themeGrid->addWidget(m_spinUiFontSize, 3, 1);
    themeGrid->addWidget(reset, 4, 0, 1, 3);

    QLabel *hint = new QLabel(L(QStringLiteral("Theme changes are applied after closing Settings with OK. Some window chrome details may require restarting MadModem.")), holder);
...
QWidget *AppSettingsDialog::makeRotatorPage()
{
    ...
    rotTabs->setObjectName(QStringLiteral("rotatorProfileTabs"));
    ...
    auto makeRotatorTab = [this](int index, QTabWidget *parentTabs) {
        ...
        QLabel *connLed = new QLabel(connGroup);
        connLed->setObjectName(QStringLiteral("rotatorConnectionLed_%1").arg(index));
        connLed->setFixedSize(18, 18);
        connLed->setStyleSheet(QStringLiteral("QLabel { background-color: #202020; border: 1px solid #707070; border-radius: 9px; }"));
        connLed->setToolTip(L(QStringLiteral("Connection state for this rotator profile.")));
        QLabel *connStatus = new QLabel(L(QStringLiteral("Disconnected")), connGroup);
        connStatus->setObjectName(QStringLiteral("rotatorConnectionStatus_%1").arg(index));
        connStatus->setWordWrap(true);
        QPushButton *connButton = new QPushButton(L(QStringLiteral("Connect")), connGroup);
        connButton->setObjectName(QStringLiteral("rotatorConnectButton_%1").arg(index));
        connButton->setProperty("connected", false);
        connButton->setToolTip(L(QStringLiteral("Connect or disconnect this rotator profile immediately, without leaving Settings.")));
        ...
        conn->addWidget(new QLabel(L(QStringLiteral("State")), connGroup), 3, 0);
        conn->addWidget(connLed, 3, 1, Qt::AlignLeft | Qt::AlignVCenter);
        conn->addWidget(connStatus, 3, 2);
        conn->addWidget(connButton, 3, 3);
        ...
        conn->addWidget(endpointHint, 4, 0, 1, 4);
        ...
        connect(connButton, &QPushButton::clicked, this, [this, index, connButton, connLed, connStatus]() {
            if (m_chkRotatorEnabled != nullptr) {
                m_chkRotatorEnabled->setChecked(true);
            }
            const bool requestConnect = !connButton->property("connected").toBool();
            connButton->setEnabled(false);
            connStatus->setText(requestConnect ? L(QStringLiteral("Connecting...")) : L(QStringLiteral("Disconnecting...")));
            connLed->setStyleSheet(QStringLiteral("QLabel { background-color: #ffb300; border: 1px solid #ffe082; border-radius: 9px; }"));
            collectSettings();
            emit rotatorConnectionRequested(index, requestConnect);
            QTimer::singleShot(900, this, [connButton]() {
                if (connButton != nullptr) connButton->setEnabled(true);
            });
        });
        ...
        QDoubleSpinBox *antennaBeam = new QDoubleSpinBox(settingsGroup);
        antennaBeam->setObjectName(QStringLiteral("rotatorAntennaBeamWidth_%1").arg(index));
        antennaBeam->setRange(2.0, 120.0);
        antennaBeam->setDecimals(1);
        antennaBeam->setSuffix(QStringLiteral("°"));
        antennaBeam->setValue(qBound(2.0, profile.antennaBeamWidthDeg, 120.0));
        antennaBeam->setToolTip(L(QStringLiteral("Main antenna beam width used by Radio Telescope tiling and peak search.")));
        ...
        const QList<QWidget *> compactRotatorEditors = { parkAz, parkEl, tolerance, poll,
                                                         azMin, azMax, elMin, elMax,
                                                         stopDeg, noMoveDeg, azMs, elMs,
                                                         startupMs, settleMs, guardMs, noMoveMs,
                                                         antennaBeam };
        for (QWidget *editor : compactRotatorEditors) {
            if (editor != nullptr) {
        ...
        settingsGrid->addWidget(new QLabel(L(QStringLiteral("Antenna beam")), settingsGroup), 10, 2);
        settingsGrid->addWidget(antennaBeam, 10, 3);
        ...
        settingsGrid->setColumnStretch(2, 0);
        settingsGrid->setColumnStretch(3, 0);
        settingsGrid->setColumnStretch(4, 0);
        ...
        algorithmGroup->setMaximumWidth(460);
        algorithmGroup->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Preferred);
        profileTopLayout->addWidget(algorithmGroup, 1);
        v->addLayout(profileTopLayout);

        QLabel *bandNote = new QLabel(L(QStringLiteral("Band assignment is configured once below, in the common Band → Rotator table. This profile page only contains the hardware and search algorithm for Rotator %1.")).arg(index + 1), tab);
        bandNote->setWordWrap(true);
        v->addWidget(bandNote);
        v->addStretch(1);
    ...
    QGroupBox *commonBandGroup = new QGroupBox(L(QStringLiteral("Band → rotator assignment and peak-search span")), page);
    commonBandGroup->setToolTip(L(QStringLiteral("One compact table assigns each operating band to Manual, Rotator 1, Rotator 2 or Rotator 3. The search spans are used by QSO tracking, auto-peak and Radio Telescope peak refinement.")));
    QVBoxLayout *commonBandLayout = new QVBoxLayout(commonBandGroup);
    QTableWidget *bandTable = new QTableWidget(commonBandGroup);
    bandTable->setObjectName(QStringLiteral("rotatorCommonBandTable"));
    bandTable->setColumnCount(5);
    bandTable->setHorizontalHeaderLabels(QStringList()
        << L(QStringLiteral("Band"))
        << L(QStringLiteral("Auto peak"))
        << L(QStringLiteral("Rotator"))
        << L(QStringLiteral("Az ±"))
        << L(QStringLiteral("El ±")));
    bandTable->verticalHeader()->setVisible(false);
    bandTable->setAlternatingRowColors(true);
    bandTable->setSelectionMode(QAbstractItemView::NoSelection);
    bandTable->setEditTriggers(QAbstractItemView::NoEditTriggers);
    bandTable->setShowGrid(true);
    const QVector<AppSettings::RotatorBandSettings> commonBandRows = defaultRotatorBandRowsForUi();
    bandTable->setRowCount(commonBandRows.size());
    for (int rowIndex = 0; rowIndex < commonBandRows.size(); ++rowIndex) {
        const AppSettings::RotatorBandSettings baseRow = commonBandRows.at(rowIndex);
        AppSettings::RotatorBandSettings effectiveRow = baseRow;
        int ownerProfile = 0; // 0 = manual/unassigned, 1..3 = rotator profile.
        for (int profileIndex = 0; profileIndex < 3; ++profileIndex) {
            const QVector<AppSettings::RotatorBandSettings> rows = rotatorBandRowsForProfile(m_initialSettings.rotatorProfiles[profileIndex]);
            for (const AppSettings::RotatorBandSettings &candidate : rows) {
                if (rotatorBandObjectKey(candidate.band).compare(rotatorBandObjectKey(baseRow.band), Qt::CaseInsensitive) != 0) {
                    continue;
                }
                if (ownerProfile == 0) {
                    effectiveRow.azimuthSearchSpanDeg = candidate.azimuthSearchSpanDeg;
                    effectiveRow.elevationSearchSpanDeg = candidate.elevationSearchSpanDeg;
                    effectiveRow.autoPeakEnabled = candidate.autoPeakEnabled;
                }
                if (candidate.enabled && ownerProfile == 0) {
                    ownerProfile = profileIndex + 1;
        ...
        QTableWidgetItem *bandItem = new QTableWidgetItem(rotatorBandDisplayName(baseRow.band));
        bandItem->setData(Qt::UserRole, baseRow.band);
        bandItem->setFlags(Qt::ItemIsEnabled);
        bandTable->setItem(rowIndex, 0, bandItem);

        QComboBox *owner = new QComboBox(bandTable);
        owner->setObjectName(QStringLiteral("rotatorBandOwner_%1").arg(rowIndex));
        owner->addItem(L(QStringLiteral("Manual")), 0);
        for (int profileIndex = 0; profileIndex < 3; ++profileIndex) {
            QString label = m_initialSettings.rotatorProfiles[profileIndex].label.trimmed();
            if (label.isEmpty()) label = QStringLiteral("Rotator %1").arg(profileIndex + 1);
            owner->addItem(label, profileIndex + 1);
        }
        const int ownerIdx = owner->findData(ownerProfile);
        owner->setCurrentIndex(ownerIdx >= 0 ? ownerIdx : 0);
        owner->setToolTip(L(QStringLiteral("Choose which rotator controls this band. Manual means no automatic band assignment.")));
        // Column order is intentionally compact: Band | Auto peak | Rotator | Az | El.
        bandTable->setCellWidget(rowIndex, 2, owner);

        QDoubleSpinBox *azSpan = new QDoubleSpinBox(bandTable);
        azSpan->setObjectName(QStringLiteral("rotatorCommonBandAzSpan_%1").arg(rowIndex));
        azSpan->setRange(0.0, 180.0);
        azSpan->setDecimals(1);
        azSpan->setSuffix(QStringLiteral("°"));
        azSpan->setKeyboardTracking(false);
        azSpan->setValue(qBound(0.0, effectiveRow.azimuthSearchSpanDeg, 180.0));
        azSpan->setToolTip(L(QStringLiteral("Azimuth search half-span around the theoretical bearing. 30 means ±30 degrees.")));
        bandTable->setCellWidget(rowIndex, 3, azSpan);

        QDoubleSpinBox *elSpan = new QDoubleSpinBox(bandTable);
        elSpan->setObjectName(QStringLiteral("rotatorCommonBandElSpan_%1").arg(rowIndex));
        elSpan->setRange(0.0, 180.0);
        elSpan->setDecimals(1);
        elSpan->setSuffix(QStringLiteral("°"));
        elSpan->setKeyboardTracking(false);
        elSpan->setValue(qBound(0.0, effectiveRow.elevationSearchSpanDeg, 180.0));
        elSpan->setToolTip(L(QStringLiteral("Elevation search half-span. Azimuth-only rotators ignore this value.")));
        bandTable->setCellWidget(rowIndex, 4, elSpan);

        QWidget *autoPeakCell = new QWidget(bandTable);
        QHBoxLayout *autoPeakLayout = new QHBoxLayout(autoPeakCell);
        autoPeakLayout->setContentsMargins(0, 0, 0, 0);
        autoPeakLayout->setAlignment(Qt::AlignCenter);
        QCheckBox *autoPeak = new QCheckBox(autoPeakCell);
        autoPeak->setObjectName(QStringLiteral("rotatorCommonBandAutoPeak_%1").arg(rowIndex));
        autoPeak->setChecked(effectiveRow.autoPeakEnabled);
        autoPeak->setToolTip(L(QStringLiteral("Allow automatic signal peak search/refinement on this band.")));
        autoPeakLayout->addWidget(autoPeak);
        bandTable->setCellWidget(rowIndex, 1, autoPeakCell);
    }
    bandTable->horizontalHeader()->setStretchLastSection(false);
    for (int column = 0; column < bandTable->columnCount(); ++column) {
        bandTable->horizontalHeader()->setSectionResizeMode(column, QHeaderView::Fixed);
    }
    bandTable->setColumnWidth(0, 66);
    bandTable->setColumnWidth(1, 92);
    bandTable->setColumnWidth(2, 210);
    bandTable->setColumnWidth(3, 86);
    bandTable->setColumnWidth(4, 86);
    bandTable->setMinimumWidth(570);
    bandTable->setMaximumWidth(650);
    bandTable->setMinimumHeight(330);
    bandTable->setMaximumHeight(560);
    commonBandLayout->setAlignment(bandTable, Qt::AlignLeft);
    commonBandLayout->addWidget(bandTable, 0, Qt::AlignLeft);
    QLabel *commonBandHint = new QLabel(L(QStringLiteral("One row per band: enable Auto peak, choose Manual/Rotator 1/2/3, then set the local Az/El search span.")), commonBandGroup);
    commonBandHint->setWordWrap(true);
    commonBandLayout->addWidget(commonBandHint);
    outer->addWidget(commonBandGroup);
...
void AppSettingsDialog::setInitialPage(InitialPage page)
{
    ...
    int index = 0;
    switch (page) {
    case InitialPage::UserQthMacros: index = 0; break;
    case InitialPage::AudioPtt: index = 1; break;
    case InitialPage::RadioCat: index = 1; break;
    case InitialPage::Logbook: index = 2; break;
    case InitialPage::Appearance: index = 2; break;
...
void AppSettingsDialog::applyStoredRotatorConnectionStatuses()
{
    for (int i = 0; i < 3; ++i) {
        if (m_rotatorConnectionKnown[i]) {
            setRotatorConnectionStatus(i, m_rotatorConnectionConnected[i], m_rotatorConnectionMessage[i]);
        }
    }
}

void AppSettingsDialog::setRotatorConnectionStatus(int profileIndex, bool connected, const QString &message)
{
    const int i = qBound(0, profileIndex, 2);
    m_rotatorConnectionKnown[i] = true;
    m_rotatorConnectionConnected[i] = connected;
    m_rotatorConnectionMessage[i] = message;
    if (m_rotatorPage == nullptr) {
        return;
    }

    QLabel *led = m_rotatorPage->findChild<QLabel *>(QStringLiteral("rotatorConnectionLed_%1").arg(i));
    QLabel *label = m_rotatorPage->findChild<QLabel *>(QStringLiteral("rotatorConnectionStatus_%1").arg(i));
    QPushButton *button = m_rotatorPage->findChild<QPushButton *>(QStringLiteral("rotatorConnectButton_%1").arg(i));

    const QString cleanMessage = message.trimmed();
    const QString stateText = cleanMessage.isEmpty()
        ? (connected ? L(QStringLiteral("Connected")) : L(QStringLiteral("Disconnected")))
        : cleanMessage;

    if (led != nullptr) {
        led->setText(QString());
        led->setFixedSize(18, 18);
        led->setToolTip(stateText);
        led->setStyleSheet(connected
            ? QStringLiteral("QLabel { background-color: #00c853; border: 1px solid #8dffb2; border-radius: 9px; }")
            : QStringLiteral("QLabel { background-color: #3b0000; border: 1px solid #ff4b5c; border-radius: 9px; }"));
    }
    if (label != nullptr) {
        label->setText(stateText);
        label->setToolTip(stateText);
    }
    if (button != nullptr) {
        button->setProperty("connected", connected);
        button->setText(connected ? L(QStringLiteral("Disconnect")) : L(QStringLiteral("Connect")));
    }
}
...
void AppSettingsDialog::collectSettings()
{
    ...
    if (m_editMyCallsign != nullptr) {
        ...
        merged.textMyCoordinatesEnabled = (m_chkUsePreciseCoordinates != nullptr && m_chkUsePreciseCoordinates->isChecked());
        merged.textMyLatitudeDeg = m_spinMyLatitude != nullptr ? m_spinMyLatitude->value() : 0.0;
        merged.textMyLongitudeDeg = m_spinMyLongitude != nullptr ? m_spinMyLongitude->value() : 0.0;
        ...
        merged.textMyCoordinatesEnabled = text.textMyCoordinatesEnabled;
        merged.textMyLatitudeDeg = text.textMyLatitudeDeg;
        merged.textMyLongitudeDeg = text.textMyLongitudeDeg;
    ...
    if (m_chkUiUseThemeFont != nullptr) {
        merged.uiUseThemeFont = m_chkUiUseThemeFont->isChecked();
    }
    if (m_comboUiFontFamily != nullptr) {
        merged.uiFontFamily = m_comboUiFontFamily->currentText().trimmed();
    }
    if (m_spinUiFontSize != nullptr) {
        merged.uiFontPointSize = m_spinUiFontSize->value();
    }
    if (m_comboDecodeTablePreset != nullptr) {
        merged.decodeTablePreset = m_comboDecodeTablePreset->currentData().toString().trimmed().toLower();
    }
    if (m_comboDecodeTableFontFamily != nullptr) {
        merged.decodeTableFontFamily = m_comboDecodeTableFontFamily->currentData().toString().trimmed();
    }
    ...
    if (QTabWidget *rotTabs = (m_rotatorPage != nullptr ? m_rotatorPage->findChild<QTabWidget *>(QStringLiteral("rotatorProfileTabs")) : nullptr)) {
        merged.rotatorActiveProfile = qBound(0, rotTabs->currentIndex(), 2);
    } else {
        merged.rotatorActiveProfile = qBound(0, merged.rotatorActiveProfile, 2);
    }
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (QDoubleSpinBox *w = findDSpin(QStringLiteral("rotatorAntennaBeamWidth"))) rp.antennaBeamWidthDeg = w->value();
    ...
    if (QTableWidget *bandTable = (m_rotatorPage != nullptr ? m_rotatorPage->findChild<QTableWidget *>(QStringLiteral("rotatorCommonBandTable")) : nullptr)) {
        QVector<AppSettings::RotatorBandSettings> perProfileRows[3];
        for (int profileIndex = 0; profileIndex < 3; ++profileIndex) {
            perProfileRows[profileIndex] = defaultRotatorBandRowsForUi();
            for (AppSettings::RotatorBandSettings &row : perProfileRows[profileIndex]) {
                row.enabled = false;
                row.autoPeakEnabled = false;
            }
        }
        QStringList enabledBands[3];
        const QVector<AppSettings::RotatorBandSettings> baseRows = defaultRotatorBandRowsForUi();
        const int rowCount = qMin(bandTable->rowCount(), baseRows.size());
        for (int rowIndex = 0; rowIndex < rowCount; ++rowIndex) {
            int ownerProfile = 0;
            if (QComboBox *owner = bandTable->findChild<QComboBox *>(QStringLiteral("rotatorBandOwner_%1").arg(rowIndex))) {
                ownerProfile = owner->currentData().toInt();
            }
            double azSpan = baseRows.at(rowIndex).azimuthSearchSpanDeg;
            double elSpan = baseRows.at(rowIndex).elevationSearchSpanDeg;
            bool autoPeak = false;
            if (QDoubleSpinBox *w = bandTable->findChild<QDoubleSpinBox *>(QStringLiteral("rotatorCommonBandAzSpan_%1").arg(rowIndex))) {
                azSpan = w->value();
            }
            if (QDoubleSpinBox *w = bandTable->findChild<QDoubleSpinBox *>(QStringLiteral("rotatorCommonBandElSpan_%1").arg(rowIndex))) {
                elSpan = w->value();
            }
            if (QCheckBox *w = bandTable->findChild<QCheckBox *>(QStringLiteral("rotatorCommonBandAutoPeak_%1").arg(rowIndex))) {
                autoPeak = w->isChecked();
            }
            for (int profileIndex = 0; profileIndex < 3; ++profileIndex) {
                if (rowIndex >= perProfileRows[profileIndex].size()) continue;
                AppSettings::RotatorBandSettings &row = perProfileRows[profileIndex][rowIndex];
                row.band = baseRows.at(rowIndex).band;
                row.azimuthSearchSpanDeg = qBound(0.0, azSpan, 180.0);
                row.elevationSearchSpanDeg = qBound(0.0, elSpan, 180.0);
                row.enabled = (ownerProfile == profileIndex + 1);
                row.autoPeakEnabled = row.enabled && autoPeak;
                if (row.enabled) {
                    enabledBands[profileIndex] << row.band;
                }
            }
        }
        for (int profileIndex = 0; profileIndex < 3; ++profileIndex) {
            merged.rotatorProfiles[profileIndex].bandSettings = perProfileRows[profileIndex];
            merged.rotatorProfiles[profileIndex].bandsCsv = enabledBands[profileIndex].join(QStringLiteral(","));
        }
    }
