...
void putField(LogbookEntry *entry, const QString &name, const QString &value)
{
    ...
    if (entry->adifFields.contains(key) && entry->adifFields.value(key).trimmed() == value.trimmed()) return;
...
int endOfEohBytes(const QByteArray &bytes)
{
    int cursor = 0;
    while (cursor < bytes.size()) {
        const int lt = bytes.indexOf('<', cursor);
        if (lt < 0) break;
        const int gt = bytes.indexOf('>', lt + 1);
        if (gt < 0) break;
        const auto descriptor = bytes.mid(lt + 1, gt - lt - 1).trimmed().split(':');
        const auto name = descriptor.value(0).trimmed().toUpper();
        if (name == "EOH" && descriptor.size() == 1) return gt + 1;
        if (name == "EOR" || name == "CALL") return -1; // headerless records
        cursor = gt + 1;
        if (descriptor.size() >= 2) {
            bool ok = false;
            const int length = descriptor.at(1).toInt(&ok);
            if (!ok || length < 0 || length > bytes.size() - cursor) return -1;
            cursor += length;
        }
    }
    return -1;
...
ParsedRecord parseRecordFieldsBytes(const QByteArray &record)
{
    ...
    while (i < n) {
        ...
        if (valueStart > n || byteLength > n - valueStart) {
            break;
        }
...
QVector<LogbookEntry> parseAdifBytes(const QByteArray &bytes, QString *headerText = nullptr)
{
    ...
    const int eoh = endOfEohBytes(bytes);
    if (headerText != nullptr) {
        if (eoh >= 0) {
            *headerText = QString::fromUtf8(bytes.left(eoh)).trimmed() + QStringLiteral("\n");
        } else {
            *headerText = QStringLiteral("Generated by MadModem\n<PROGRAMID:8>MadModem <EOH>\n");
        }
    }

    const QByteArray body = (eoh >= 0) ? bytes.mid(eoh) : bytes;
...
bool writeLogbookAtomically(const QString &fileName,
                            ...
                            QString *errorMessage, QByteArray *expectedDigest)
{
    ...
    if (!lock.tryLock(0)) {
        if (errorMessage) *errorMessage = QStringLiteral("Logbook is being modified by another process. Retry after it finishes.");
        return false;
    }
    ...
    if (existing.exists()) {
        if (!existing.open(QIODevice::ReadOnly)) {
            if (errorMessage) *errorMessage = existing.errorString();
            return false;
        }
        currentBytes = existing.readAll();
        if (existing.error() != QFile::NoError) {
            if (errorMessage) *errorMessage = existing.errorString();
            return false;
        }
    }
    existing.close(); // Release the read handle before atomic replacement on Windows.
    if (expectedDigest && QCryptographicHash::hash(currentBytes, QCryptographicHash::Sha256) != *expectedDigest) {
        if (errorMessage) *errorMessage = QStringLiteral("Logbook changed externally. Reload it before saving; no data was overwritten.");
        return false;
    }
    ...
    if (expectedDigest) *expectedDigest = digest.result();
    return true;
}
...
QString AdifLogbook::defaultPath()
{
    ...
    const QString legacy = executable.filePath(QStringLiteral("logbook.adi"));
    if (QFileInfo::exists(legacy) || QFileInfo::exists(executable.filePath(QStringLiteral("portable.flag")))) return legacy;
    return QDir(QStandardPaths::writableLocation(QStandardPaths::AppLocalDataLocation)).filePath(QStringLiteral("logbook.adi"));
}

AdifLogbook::AdifLogbook(const QString &fileName)
    : m_fileName(fileName), m_fileDigest(QCryptographicHash::hash(QByteArray(), QCryptographicHash::Sha256))
{
}
...
bool AdifLogbook::load(QString *errorMessage)
{
    ...
    if (!file.exists()) {
        ...
        m_records.clear();
        m_fileDigest = QCryptographicHash::hash(QByteArray(), QCryptographicHash::Sha256);
...
{
    return writeLogbookAtomically(m_fileName, m_headerText, m_records, errorMessage, &m_fileDigest);
}
...
bool AdifLogbook::append(const LogbookEntry &entry, QString *errorMessage)
{
    ...
    if (!writeLogbookAtomically(m_fileName, m_headerText, nextRecords, errorMessage, &m_fileDigest)) {
        return false;
    }
    m_records.swap(nextRecords);
    m_callsignIndex.insert(normalized.callsign);
    return true;
}
...
int AdifLogbook::removeEntries(const QVector<LogbookEntry> &entries, QString *errorMessage)
{
    ...
    if (!writeLogbookAtomically(m_fileName, m_headerText, remaining, errorMessage, &m_fileDigest)) {
        return -1;
    }
