...
Packaging-only hotfix over 0.5.76w. Linux and macOS are left as-is because they run. Windows remains a GitHub-built MSYS2/MinGW64 dynamic standalone package, but the packager now explicitly bundles the MinGW/Qt indirect runtime DLL families that windeployqt can miss, including ICU, pcre2, zstd, harfbuzz, md4c, libpng and libgomp. The release validator fails the build if those DLL families are missing from the Windows ZIP.

No modem, DSP, CAT/PTT, FT8/FT4, audio, MIND, decoder or UI runtime logic is changed.
