...
GitHub/macOS arm64 CI hotfix over 0.5.76n. This release fixes a Qt6/AppleClang strict-conversion error in the MFSK transmitter where raw STX/EOT integer control codes were passed to a `QChar` parameter. The values are now passed explicitly as `QChar(char16_t{...})`, preserving the intended control characters while satisfying Qt6. The macOS portability preflight now rejects this pattern before the long Ninja build. No FT8/FT4 DSP, CAT/PTT, audio, or UI runtime retune is intended.
