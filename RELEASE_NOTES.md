# 0.5.76y - Windows standalone runtime closure hotfix

- Based on 0.5.76w, keeping the validated Linux and macOS packaging path unchanged.
- Windows MSYS2/MinGW standalone packaging now runs the recursive runtime DLL closure before creating ZIP artifacts.
- The package script scans MadModem.exe, Qt DLLs, plugins and copied DLLs with objdump, copies non-system dependencies from the MSYS2/MinGW runtime, and fails if anything remains unresolved.
