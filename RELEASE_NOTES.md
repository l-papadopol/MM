...
This release starts the 0.5.77 line from the validated 0.5.76 baseline.

Highlights:
- Cleaner Windows standalone packages: user-facing files in the root, runtime files in `bin/`, and a `MadModem.lnk` launcher.
- New Appearance / UI settings page with Avionica, Qt Default, Hacker Green, Classic Dark and High Contrast themes.
- Global UI font family/size controls for operators who need larger or different fonts.
- Multilingual labels/tooltips for the new appearance controls.
- FT QSO activity/history double-click activation for valid FT messages.

The decoder/DSP core is intentionally left unchanged from the 0.5.76 line.
