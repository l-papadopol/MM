...
- Windows release artifacts remain GitHub-built MXE static packages, split into legacy CPU baseline and AVX2 variants.
...
Packaging-only GitHub release hardening over 0.5.76u. This release prevents stale assets from previous releases from remaining downloadable after a re-run: the release job now deletes existing assets before upload and rejects any package whose filename does not contain the current MADMODEM_VERSION.txt value. Windows remains GitHub-built MXE static legacy/AVX2. macOS bundles explicitly include and verify QtSerialPort.framework. Linux publishes a single tar.gz. No modem/DSP/CAT/audio/UI runtime code is changed.

# MadModem 0.5.76u

Packaging-only GitHub release correction over 0.5.76p. Windows release artifacts are now built by GitHub using MXE static Qt5/MinGW, with separate legacy and AVX2 ZIP packages. macOS packaging verifies that every linked Qt framework is present inside the .app bundle, including QtSerialPort. Linux now publishes a single tar.gz package.
