...
<h2>Signal peak tracking</h2><p>When QSO tracking is active, a band row with <strong>Auto peak</strong> enabled can refine the geometric bearing from a real received signal. MadModem uses discrete, rate-limited probes rather than continuous conical motion: at most 5 measurements on an azimuth-only rotor or 7 on Alt-Az, with at least 90 seconds between cycles. Pattern search is the recommended low-wear default; Golden section is available only for azimuth-only systems and Nelder-Mead only for Alt-Az. A valid QSO callsign and Maidenhead locator are required. The search stops before local TX and returns to the logical QSO target after PTT is released.</p>
...
<h2>Inseguimento del massimo del segnale</h2><p>Quando è attivo l'inseguimento QSO, una riga di banda con <strong>Auto peak</strong> abilitato può rifinire il puntamento geometrico usando il segnale realmente ricevuto. MadModem usa sonde discrete e limitate nel tempo, non una scansione conica continua: al massimo 5 misure con rotore solo azimutale o 7 con Alt-Az, con almeno 90 secondi fra due cicli. Pattern search è il default consigliato a bassa usura; Golden section è disponibile solo per sistemi azimutali e Nelder-Mead solo per Alt-Az. Sono necessari nominativo QSO e locator Maidenhead validi. La ricerca si arresta prima del TX locale e torna al bersaglio QSO logico dopo il rilascio del PTT.</p>
...
<h2>Suivi du maximum du signal</h2><p>Lorsque le suivi QSO est actif, une ligne de bande avec <strong>Auto peak</strong> activé peut affiner le pointage géométrique à partir du signal réellement reçu. MadModem utilise des sondes discrètes et limitées, et non un balayage conique continu : au maximum 5 mesures avec un rotor azimutal seul ou 7 en Alt-Az, avec au moins 90 secondes entre deux cycles. Pattern search est le choix recommandé à faible usure ; Golden section est réservé aux systèmes azimutaux et Nelder-Mead aux systèmes Alt-Az. Un indicatif QSO et un locator Maidenhead valides sont requis. La recherche s'arrête avant la TX locale et revient à la cible QSO logique après relâchement du PTT.</p>
...
<h2>Signalmaximum-Nachführung</h2><p>Bei aktiver QSO-Nachführung kann eine Bandzeile mit aktiviertem <strong>Auto peak</strong> die geometrische Peilung anhand des tatsächlich empfangenen Signals verfeinern. MadModem verwendet diskrete, begrenzte Messpunkte statt kontinuierlicher konischer Bewegung: höchstens 5 Messungen bei reinem Azimutrotor oder 7 bei Alt-Az, mit mindestens 90 Sekunden zwischen den Zyklen. Pattern Search ist die empfohlene verschleißarme Voreinstellung; Golden Section steht nur für Azimutsysteme und Nelder-Mead nur für Alt-Az zur Verfügung. Ein gültiges QSO-Rufzeichen und ein Maidenhead-Locator sind erforderlich. Die Suche stoppt vor lokaler TX und kehrt nach dem Lösen des PTT zum logischen QSO-Ziel zurück.</p>
...
<h2>Sporing av signalmaksimum</h2><p>Når QSO-sporing er aktiv, kan en båndrad med <strong>Auto peak</strong> aktivert finjustere geometrisk peiling fra det faktisk mottatte signalet. MadModem bruker diskrete og hastighetsbegrensede målepunkter, ikke kontinuerlig konisk bevegelse: maksimalt 5 målinger med ren asimutrotor eller 7 med Alt-Az, og minst 90 sekunder mellom syklusene. Pattern search er anbefalt standard med lav slitasje; Golden section er bare tilgjengelig for asimutsystemer og Nelder-Mead bare for Alt-Az. Gyldig QSO-kallesignal og Maidenhead-lokator kreves. Søket stopper før lokal TX og går tilbake til det logiske QSO-målet etter at PTT er sluppet.</p>
...
<h2>Sledování maxima signálu</h2><p>Při aktivním sledování QSO může řádek pásma se zapnutým <strong>Auto peak</strong> zpřesnit geometrický směr podle skutečně přijímaného signálu. MadModem používá diskrétní a časově omezené sondy, nikoli nepřetržitý kuželový pohyb: nejvýše 5 měření pro pouze azimutální rotátor nebo 7 pro Alt-Az, s odstupem alespoň 90 sekund mezi cykly. Pattern search je doporučená výchozí volba s nízkým opotřebením; Golden section je pouze pro azimutální systémy a Nelder-Mead pouze pro Alt-Az. Je vyžadována platná QSO volací značka a lokátor Maidenhead. Hledání se zastaví před místním TX a po uvolnění PTT se vrátí k logickému cíli QSO.</p>
