...
{
    const double clamped = clampElevation(alt);
    if (!m_haveFilteredAlt) {
        m_alt = clamped;
        m_haveFilteredAlt = true;
    } else {
        const double delta = clamped - m_alt;
        // Wind and backlash can make the raw rotator readout bounce around the
        // target.  Smooth only the instrument display; the controller still uses
        // raw poll values for safety and movement decisions.
        if (qAbs(delta) >= 0.08) {
            m_alt += delta * 0.23;
        }
    }
...
{
    m_overlapActive = az > 360.0;
    if (!m_haveFilteredAz) {
        m_az = az;
        m_haveFilteredAz = true;
    } else {
        const double delta = shortestAngularDelta(az, m_az);
        // Deadband + EMA: suppress cosmetic jitter without hiding large moves.
        if (qAbs(delta) >= 0.12) {
            m_az += delta * 0.23;
        }
    }
...
double NavballWidget::shortestAngularDelta(double targetDeg, double currentDeg)
{
    double delta = normalize360(targetDeg) - normalize360(currentDeg);
