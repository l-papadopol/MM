...
#include "widgets/RadioTelescopeHeatmapWidget.h"
#include "widgets/CwDiagnosticWidget.h"
...
private slots:
    ...
    /** Starts recording the exact live mono RX stream to a WAV file. */
    void startRxAudioRecording();

    /** Stops and finalizes the active RX WAV recording. */
    void stopRxAudioRecording();
    ...
    /**
     * @brief Clears only the RX A CW terminal and acquisition state.
     */
    void clearCwRxAText();

    /**
     * @brief Clears only the RX B CW terminal and acquisition state.
     */
    void clearCwRxBText();
    ...
    /**
     * @brief Applies radio-telescope UI values and updates the marker pair/heatmap status.
     */
    void applyRadioTelescopeSettings();
    void setupRadioTelescopePage();
    void loadRadioTelescopeSettings();
    void saveRadioTelescopeSettings();
    void clearRadioTelescopeHeatmap();
    void startRadioTelescopeScan();
    void stopRadioTelescopeScan(bool automatic = false);
    void advanceRadioTelescopeScan();
    void updateRadioTelescopeHeatmapWidget();
    bool radioTelescopeRotatorReadyForScan(QString *reason = nullptr) const;
    bool radioTelescopeTileWithinRotatorLimits(double azimuthDeg, double elevationDeg, bool useElevation) const;
    void exportRadioTelescopeCsv();
    void pointRadioTelescopeMaxNoise();
    bool continueRadioTelescopePeakRefinement();
    bool radioTelescopeTileMeasuredNear(double azimuthDeg, double elevationDeg, double toleranceDeg = 0.35) const;
    void syncRadioTelescopeBeamFromActiveRotator();
    void handleRadioTelescopeRotatorPosition(double azimuthDeg, double elevationDeg);
    double measureRadioTelescopeBandPower(const AudioBlock &block) const;
    ...
    void handleFt8QsoHistoryDoubleClicked(QTableWidgetItem *item);
     ...
     */
    void handleFt8DecodeBatchStarted(qint64 slotStartUtcMs, const QString &phase);
    void handleFt8DecodeReady(const Ft8RxDecoder::Decode &decode);
    void handleFt8DecodeBatchFinished(qint64 slotStartUtcMs, const QString &phase);
    ...
    void queueFt8SequencerDecode(const Ft8RxDecoder::Decode &decode);
    int selectBestFt8SequencerDecode(const QVector<Ft8RxDecoder::Decode> &decodes) const;
    // Passive caller queue: direct callers are remembered while another QSO is
    // active, but the existing linear sequencer remains the sole owner of TX.
    void considerFt8CallerForQueue(const Ft8RxDecoder::Decode &decode, bool blacklistedDecode);
    bool activateNextFt8QueuedCaller();
    void removeFt8QueuedCaller(const QString &call);
    void clearFt8CallerQueue(const QString &reason = QString());
    void updateFt8SessionQueueUi();
...
private:
    ...
    /**
     * @brief Builds the dedicated dual-receiver CW page with separate RX A/RX B terminals.
     */
    QWidget *createCwTerminalPage();
     ...
     */
    void applyPersistentSettingsToRuntime(const AppSettings *previousSettings = nullptr);
     ...
     */
    void sendRttyContestMacro(int index);
...
private:
    ...
    void applyUiAppearanceSettings();
    ...
    void shutdownLog(const QString &message) const;
    void beginPendingRxAudioRecording();
    void updateRxAudioRecordingActions();
    ...
    QAction *m_actionStartRxRecording = nullptr;
    QAction *m_actionStopRxRecording = nullptr;
    ...
    bool m_rxAudioRecordingActive = false;
    ...
    QTabWidget *m_cwDisplayTabs = nullptr;
    CwDiagnosticWidget *m_cwDiagnosticWidget = nullptr;
    ...
    QLineEdit *m_editRttyContestQsoCall = nullptr;
    QLineEdit *m_editRttyContestQsoBand = nullptr;
    QLineEdit *m_editRttyContestQsoRstSent = nullptr;
    QLineEdit *m_editRttyContestQsoRstReceived = nullptr;
    QLineEdit *m_editRttyContestQsoMode = nullptr;
    QLineEdit *m_editRttyContestQsoGrid = nullptr;
    QLineEdit *m_editRttyContestQsoUtc = nullptr;
    QPushButton *m_btnRttyContestAddQso = nullptr;
    ...
    QCheckBox *m_chkCwAutoWpmA = nullptr;
    QCheckBox *m_chkCwAutoWpmB = nullptr;
    QLabel *m_lblCwTrackedWpmA = nullptr;
    QLabel *m_lblCwTrackedWpmB = nullptr;
    ...
    QPlainTextEdit *m_txtCwRxB = nullptr;
    QPlainTextEdit *m_txtCwTx = nullptr;
    QPushButton *m_btnCwClearRx = nullptr;
    QPushButton *m_btnCwClearRxB = nullptr;
    ...
    QComboBox *m_cmbMsk144FrequencyTolerance = nullptr;
    ...
    struct RadioTelescopeTileRecord
    {
        double azimuthDeg = 0.0;
        double elevationDeg = 0.0;
        double beamWidthDeg = 15.0;
        double noiseDb = -120.0;
        // Normalized sky-plane center used by the Radio Telescope honeycomb renderer.
        // For Alt-Az scans: x/y are in the unit sky disk, zenith at (0,0), horizon at r=1.
        // For azimuth-only scans: x/y are on the annular midline.
        double skyX = 0.0;
        double skyY = 0.0;
    ...
    enum class RadioTelescopeScanPhase
    {
        Idle,
        MovingToPark,
        WaitingForRotator,
    ...
    QWidget *m_radioTelescopeDisplayPage = nullptr;
    QWidget *m_pageRadioTelescopeSettings = nullptr;
    RadioTelescopeHeatmapWidget *m_radioTelescopeHeatmapWidget = nullptr;
    QTableWidget *m_tableRadioTelescopeSamples = nullptr;
    QPushButton *m_btnRadioTelescopeStartScan = nullptr;
    QPushButton *m_btnRadioTelescopeStopScan = nullptr;
    QPushButton *m_btnRadioTelescopeClearMap = nullptr;
    QPushButton *m_btnRadioTelescopeExportCsv = nullptr;
    QPushButton *m_btnRadioTelescopePointMax = nullptr;
    QComboBox *m_cmbRadioTelescopeRotatorMode = nullptr;
    QDoubleSpinBox *m_spinRadioTelescopeBeamWidthDeg = nullptr;
    QSpinBox *m_spinRadioTelescopeLowHz = nullptr;
    QSpinBox *m_spinRadioTelescopeHighHz = nullptr;
    QSpinBox *m_spinRadioTelescopeDwellMs = nullptr;
    QSpinBox *m_spinRadioTelescopeSettleMs = nullptr;
    QCheckBox *m_chkRadioTelescopeInterpolate = nullptr;
    QCheckBox *m_chkRadioTelescopeAllowNoRotator = nullptr;
    QCheckBox *m_chkRadioTelescopeAutoPointPeak = nullptr;
    QLabel *m_lblRadioTelescopeSummary = nullptr;
    QLabel *m_lblRadioTelescopeStatus = nullptr;
    QLabel *m_lblRadioTelescopeProgress = nullptr;
    QTimer m_radioTelescopeScanTimer;
    QVector<RadioTelescopeTileRecord> m_radioTelescopeTiles;
    RadioTelescopeScanPhase m_radioTelescopeScanPhase = RadioTelescopeScanPhase::Idle;
    int m_radioTelescopeCurrentTileIndex = -1;
    qint64 m_radioTelescopePhaseDeadlineMs = 0;
    double m_radioTelescopeAccumulatedPower = 0.0;
    int m_radioTelescopeAccumulatedBlocks = 0;
    double m_radioTelescopeCurrentAzimuthDeg = 0.0;
    double m_radioTelescopeCurrentElevationDeg = 0.0;
    double m_radioTelescopeTargetAzimuthDeg = 0.0;
    double m_radioTelescopeTargetElevationDeg = 0.0;
    double m_radioTelescopeReturnAzimuthDeg = 0.0;
    double m_radioTelescopeReturnElevationDeg = 0.0;
    bool m_radioTelescopeReturnPositionValid = false;
    ...
    QCheckBox *m_chkFt8CallerQueue = nullptr;
    QLabel *m_lblFt8SessionQueue = nullptr;
    QCheckBox *m_chkFt8EvilMode = nullptr;
    bool m_ft8EvilModeUnlocked = false;
    bool m_ft8OperatorStopLatched = false;
    bool m_ft8SelectionOnly = false;
    QVector<Ft8FullAutoCqCandidate> m_ft8FullAutoCqCandidates;
    QTimer m_ft8FullAutoCqSelectionTimer;
    bool m_ft8DecodeBatchActive = false;
    ...
    struct Ft8QueuedCaller
    {
        Ft8RxDecoder::Decode latestDecode;
        QString call;
        QString grid;
        QDateTime firstSeenUtc;
        QDateTime lastSeenUtc;
    };
    QVector<Ft8QueuedCaller> m_ft8CallerQueue;
    int m_ftSessionQsoCount = 0;
    QString m_ftCallerQueueMode;
    ...
    // duplicate decisions or the sequencer.  They exist to prove whether a
    // CRC-valid FT4/FT8 decode reached the visible tables.
    QString m_ftDisplayDiagSlotKey;
    int m_ftDisplayDiagDecodeSignals = 0;
    int m_ftDisplayDiagRowsInserted = 0;
    int m_ftDisplayDiagRowsUpdated = 0;
    int m_ftDisplayDiagBlacklisted = 0;
    int m_ftDisplayDiagRowsBefore = 0;
    int m_ftDisplayDiagRowsAfter = 0;
