...
from pathlib import Path
import re, sys
# Qt6 intentionally makes many QChar integer constructors explicit. Calls to
# functions that take QChar must pass QChar(...) explicitly, not bare int codes.
qchar_param_names = [
    'appendStandardCharacter',
...
name_re = '|'.join(re.escape(n) for n in qchar_param_names)
pat = re.compile(r'\b(?:' + name_re + r')\s*\(\s*[0-9]+\s*(?:[,)]|//)')
hits = []
for p in Path('.').rglob('*'):
    if p.suffix not in ('.cpp', '.h', '.hpp', '.cc'):
        continue
...
if hits:
    print('ERROR: Qt6/AppleClang rejects bare integer literals passed to QChar parameters.', file=sys.stderr)
    print('Use QChar(char16_t{N}) or QChar(static_cast<char16_t>(N)) explicitly.', file=sys.stderr)
    for h in hits:
        print(h, file=sys.stderr)
    sys.exit(1)
PYCHK_QCHAR
...
python3 - <<'PYCHK_TEXTSTREAM_CODEC'
from pathlib import Path
import re, sys
hits = []
pat = re.compile(r'\.setCodec\s*\(\s*(?:"UTF-8"|QStringLiteral\("UTF-8"\))\s*\)')
for p in Path('.').rglob('*'):
    if p.suffix not in ('.cpp', '.h', '.hpp', '.cc'):
        continue
    if any(part in ('reference', 'upstream_2765') for part in p.parts):
    ...
    for i, line in enumerate(lines):
        ...
        guarded = ('QT_VERSION < QT_VERSION_CHECK(6' in window or
                   'QT_VERSION_CHECK(6, 0, 0)' in window and '#else' in window)
        if not guarded:
            hits.append(f"{p}:{i+1}:{line.strip()}")
if hits:
    print('ERROR: QTextStream::setCodec("UTF-8") is Qt5-only and must be guarded or replaced with setEncoding(QStringConverter::Utf8) on Qt6.', file=sys.stderr)
    for h in hits:
        print(h, file=sys.stderr)
    sys.exit(1)
PYCHK_TEXTSTREAM_CODEC
