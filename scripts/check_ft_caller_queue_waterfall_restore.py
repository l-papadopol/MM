...
checks = [
    ...
    ('waterfall drops hidden/minimized display rows', 'display-only FFT rows are discarded until restore' in wf_cpp),
    ('waterfall resets GL history on restore', 'm_gpuClearPending = true;' in wf_cpp and 'm_presentationSuspended' in wf_h),
]
...
if failed:
    sys.exit(1)
print('FT caller queue + waterfall restore source audit: PASS')
