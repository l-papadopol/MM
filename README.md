...
GitHub release packaging correction over 0.5.76p. Windows release artifacts are now built on GitHub with MXE static Qt5/MinGW, producing separate legacy and AVX2 ZIPs instead of MSYS2 dynamic packages. macOS packaging verifies that linked Qt frameworks such as QtSerialPort are present inside the .app bundle. Linux release output is a single tar.gz.
...
### GitHub CI note for 0.5.76s

0.5.76s is a packaging-only release line: Windows release artifacts are MXE static legacy/AVX2 builds produced by GitHub, macOS bundles are checked for missing Qt frameworks, and Linux emits a single tar.gz. No modem/DSP/CAT/audio/MIND runtime retune is intended.
