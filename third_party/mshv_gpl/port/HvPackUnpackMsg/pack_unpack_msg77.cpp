...
void PackUnpackMsg77::unpacktext77(bool *c77,QString &c13)
{
    ...
    /*int c_77_txt_cou = c_77_txt.size();
...
{
    //QString aaa = "A4A";   qDebug()<<aaa.size();
    int MAXGRID4 = 32400;
    ...
    else if (i3==1 || i3==2)
    {
        ...
        {
            int i=call_1.size();//2.21 A4A count=3  i=index(call_1,' ')
        ...
        {
            int i=call_2.size();//2.21 A4A count=3  i=index(call_2,' ')
    ...
    //if (msg.size()<1) unpk77_success=false;//2.76.6
    return msg;
}
////////////// END Unpack //////////////////////////////////////////////////////////////
...
QString PackUnpackMsg77::RemBegEndWSpaces(QString str)
{
    ...
    for (msg_count = str.size()-1; msg_count>=0; msg_count--)
    {
        if (str.at(msg_count)!=' ')
            break;
    }
    ...
    msg_count = 0;
    for (msg_count = 0; msg_count<s.size(); msg_count++)
    {
        if (s.at(msg_count)!=' ')
            break;
    }
    s = s.mid(msg_count,(s.size()-msg_count));*/
    s = str.trimmed();
    return s;
}
...
void PackUnpackMsg77::chkcall(QString w,QString &bc,bool &cok)
{
    ...
    cok=true;
    bc=w.mid(0,6);//(1:6)
    int n1=w.indexOf(" ");//qDebug()<<n1;//w.size();//n1=len_trim(w)
...
int PackUnpackMsg77::ihashcall(QString c0,int m)
{
    ...
    c1=c0;
    if (c1.at(0)==(QChar)'<') c1=c1.mid(1,c1.size()-1)+"             "; //if(c1(1:1).eq.'<') c1=c1(2:)
    int i=c1.indexOf(">");//i=index(c1,'>')
    if (i>-1) //c1.replace(">"," ");//c1(i:)='         ';
    {
        for (int j = i; j<c1.size(); ++j) c1[j]=(QChar)' ';
    }
...
void PackUnpackMsg77::pack28(QString c13,int &n28)
{
    ...
    if (c13.mid(0,2)=="3X" && c13.at(2).toLatin1()>=(int)'A' && mm_qchar_latin1(c13.at(2)) <= 'Z') callsign="Q"+c13.mid(2,5)+"      ";//c13(3:6)
...
void PackUnpackMsg77::pack77_5(int nwords,QString *w,int &i3,int &n3,bool *c77)
{
    ...
    if (nwords==4 || nwords==5 || nwords==6)
    {
        ...
        loc = loc.trimmed();
        if (irpt==-1 || loc.size()!=2) return; //if(irpt.eq.-1 .or. len(trim(w(nwords))).ne.2) go to 900
        //c2=w[nwords](1:2)
...
void PackUnpackMsg77::packtext77(QString c13,int &i3,int &n3,bool *c71)
{
    ...
    for (int i = 0; i < 13; ++i)
    {
        ...
        Int32ToBin(c_77_txt.size(),c_c_arr,BITS_71);
...
c5:
    ...
    //1005 format(3z6)
    bool ok;
    ntel[0]=c18.mid(0,6).toInt(&ok,16);
    if (!ok) ntel[0]= -1;
    ntel[1]=c18.mid(6,6).toInt(&ok,16);
    if (!ok) ntel[1]= -1;
    ntel[2]=c18.mid(12,6).toInt(&ok,16);
    if (!ok) ntel[2]= -1;
