...
//#include <QtGui>
// MSHV sources compare QString/QChar elements with ASCII codepoints in many
// Fortran-port expressions. Qt 6 on AppleClang is stricter than old Qt5/GCC,
// so force those relational comparisons through Latin-1 integer values.
namespace {
inline int mm_qchar_latin1(QChar ch) { return static_cast<unsigned char>(ch.toLatin1()); }
}
...
void PackUnpackMsg::copy_qstr(QString &to,QString from,int b_from,int e_from)
{
    for (int i = 0; i < e_from-b_from; i++)
        to[i]=from[i+b_from];
    for (int i = e_from-b_from; i < to.size(); i++)
        to[i]=' ';
}
...
void PackUnpackMsg::packcall(QString callsign,int &ncall,bool &text)
{
    ...
    if (qstr_beg_end(callsign,0,3)=="CQ ")
    {
        // qDebug()<<"CQ"<<callsign;
        ncall=NBASE + 1;
        if (mm_qchar_latin1(callsign[3]) >= '0' && mm_qchar_latin1(callsign[3]) <= '9' && mm_qchar_latin1(callsign[4]) >= '0' && mm_qchar_latin1(callsign[4]) <= '9' &&
                mm_qchar_latin1(callsign[5]) >= '0' && mm_qchar_latin1(callsign[5]) <= '9')
        {
    ...
    //tmp="      ";
    if (mm_qchar_latin1(callsign[2]) >= '0' && mm_qchar_latin1(callsign[2]) <= '9')
        copy_qstr(tmp,callsign,0,callsign.size());//tmp=callsign;
    else if (mm_qchar_latin1(callsign[1]) >= '0' && mm_qchar_latin1(callsign[1]) <= '9')
    {
        if (callsign[5]!=' ')
        {
            text=true;
    ...
    int n1=0;
    if ((mm_qchar_latin1(tmp[0]) >= 'A' && mm_qchar_latin1(tmp[0]) <= 'Z') || tmp[0]==' ') n1=1;
    if (mm_qchar_latin1(tmp[0]) >= '0' && mm_qchar_latin1(tmp[0]) <= '9') n1=1;
    int n2=0;
    if (mm_qchar_latin1(tmp[1]) >= 'A' && mm_qchar_latin1(tmp[1]) <= 'Z') n2=1;
    if (mm_qchar_latin1(tmp[1]) >= '0' && mm_qchar_latin1(tmp[1]) <= '9') n2=1;
    int n3=0;
    if (mm_qchar_latin1(tmp[2]) >= '0' && mm_qchar_latin1(tmp[2]) <= '9') n3=1;
    int n4=0;
    if ((mm_qchar_latin1(tmp[3]) >= 'A' && mm_qchar_latin1(tmp[3]) <= 'Z') || tmp[3]==' ') n4=1;
    int n5=0;
    if ((mm_qchar_latin1(tmp[4]) >= 'A' && mm_qchar_latin1(tmp[4]) <= 'Z') || tmp[4]==' ') n5=1;
    int n6=0;
    if ((mm_qchar_latin1(tmp[5]) >= 'A' && mm_qchar_latin1(tmp[5]) <= 'Z') || tmp[5]==' ') n6=1;
...
void PackUnpackMsg::getpfx1(QString &callsign,int &k,int &nv2)
{
    ...
    //callsign0=qstr_beg_end(callsign,0,callsign.size());
    nv2=1;
...
c10:
    if (islash!=-1 && k==0) //if(islash.ne.0 .and.k.eq.0)
    {
        ...
        rof=qstr_beg_end(callsign0,islash+1,callsign0.size());//rof=callsign0(islash+1:)
        //qDebug()<<"LOV ROT="<<lof<<rof;
        //llof=len_trim(lof)//c++   ==.EQ. !=.NE. >.GT. <.LT. >=.GE. <=.LE.
        //lrof=len_trim(rof)
        //qDebug()<<"IS PFX SFX="<<ispfx<<issfx;
        if (lof.size()>0 && lof.size()<=4) ispfx=true;//ispfx=(llof.gt.0 .and. llof.le.4)
        if (rof.size()>0 && rof.size()<=3) issfx=true;////issfx=(lrof.gt.0 .and. lrof.le.3)
        //qDebug()<<"IS PFX SFX="<<ispfx<<issfx;
        if (!ispfx || !issfx) invalid = false;//invalid=.not.(ispfx.or.issfx)
        if (ispfx && issfx) //if(ispfx.and.issfx)
        {
            if (lof.size()<3) issfx=false;//if(llof.lt.3) issfx=.false.
            if (rof.size()<3) ispfx=false;//if(lrof.lt.3) ispfx=.false.
        ...
        else
        {
            ...
            if (ispfx)
            {
                ...
                copy_qstr(callsign,callsign0,i+1,callsign0.size());//callsign=callsign0(i+1:);
                //qDebug()<<"TPFX="<<tpfx;
            }
            if (issfx)
            {
...
void PackUnpackMsg::grid2deg(QString grid0,double &dlong,double &dlat)
{
    ...
    /*if ((int)grid[0].toLatin1()>=(int)'a' && mm_qchar_latin1(grid[0]) <= 'z') grid[0]=char((int)((int)grid[0].toLatin1())+(int)('A')-(int)('a'));
    if ((int)grid[1].toLatin1()>=(int)'a' && mm_qchar_latin1(grid[1]) <= 'z') grid[1]=char((int)((int)grid[1].toLatin1())+(int)('A')-(int)('a'));
    if ((int)grid[4].toLatin1()>=(int)'A' && mm_qchar_latin1(grid[4]) <= 'Z') grid[4]=char((int)((int)grid[4].toLatin1())-(int)('A')+(int)('a'));
    if ((int)grid[5].toLatin1()>=(int)'A' && mm_qchar_latin1(grid[5]) <= 'Z') grid[5]=char((int)((int)grid[5].toLatin1())-(int)('A')+(int)('a'));*/
    if (mm_qchar_latin1(grid[0]) >= 'a' && mm_qchar_latin1(grid[0]) <= 'z') grid[0]=char((int)grid[0].toLatin1()+(int)'A'-(int)'a');
    if (mm_qchar_latin1(grid[1]) >= 'a' && mm_qchar_latin1(grid[1]) <= 'z') grid[1]=char((int)grid[1].toLatin1()+(int)'A'-(int)'a');
    if (mm_qchar_latin1(grid[4]) >= 'A' && mm_qchar_latin1(grid[4]) <= 'Z') grid[4]=char((int)grid[4].toLatin1()-(int)'A'+(int)'a');
    if (mm_qchar_latin1(grid[5]) >= 'A' && mm_qchar_latin1(grid[5]) <= 'Z') grid[5]=char((int)grid[5].toLatin1()-(int)'A'+(int)'a');
...
void PackUnpackMsg::packgrid(QString grid,int &ng,bool &text,bool msk144ms)
{
    ...
    if (grid[0]=='-')
    {
        //if (mm_qchar_latin1(grid[1]) >= '0' && mm_qchar_latin1(grid[1]) <= '9' && mm_qchar_latin1(grid[2]) >= '0' && mm_qchar_latin1(grid[2]) <= '9')
    ...
    {
        //grid[2].isDigit()
        //if (mm_qchar_latin1(grid[2]) >= '0' && mm_qchar_latin1(grid[2]) <= '9' && mm_qchar_latin1(grid[3]) >= '0' && mm_qchar_latin1(grid[3]) <= '9')
...
c30:
    ...
    if (mm_qchar_latin1(grid[0]) < 'A' || mm_qchar_latin1(grid[0]) > 'R') text=true;
    if (mm_qchar_latin1(grid[1]) < 'A' || mm_qchar_latin1(grid[1]) > 'R') text=true;
    if (mm_qchar_latin1(grid[2]) < '0' || mm_qchar_latin1(grid[2]) > '9') text=true;
    if (mm_qchar_latin1(grid[3]) < '0' || mm_qchar_latin1(grid[3]) > '9') text=true;
    if (text) goto c900;
...
void PackUnpackMsg::packmsg(char *msg0,int *dat,int &itype,bool msk144ms)
{
    ...
    //and CQ 0..9 HV 
    if(qstr_beg_end(msg,0,3) =="CQ " && mm_qchar_latin1(msg[3]) >= '0' && mm_qchar_latin1(msg[3]) <= '9' && msg[4]==' ') msg="CQ 00"+qstr_beg_end(msg,3,msg.size());
    ...
    if (qstr_beg_end(msg,0,3)=="CQ " &&
            mm_qchar_latin1(msg[3]) >= 'A' && mm_qchar_latin1(msg[3]) <= 'Z' &&
            mm_qchar_latin1(msg[4]) >= 'A' && mm_qchar_latin1(msg[4]) <= 'Z' &&
            msg[5]==' ') msg="E9"+qstr_beg_end(msg,3,msg.size());
    ...
    {/*if(msg(1:3).eq.'CQ ') then
        ...
        if (mm_qchar_latin1(msg[3]) >= '0' && mm_qchar_latin1(msg[3]) <= '9' && mm_qchar_latin1(msg[4]) >= '0' && mm_qchar_latin1(msg[4]) <= '9' && mm_qchar_latin1(msg[5]) >= '0' && mm_qchar_latin1(msg[5]) <= '9')
            i=6;//mai 6
        goto cc1;
    }
