...
void PackUnpackMsg::unpackcall(int ncall,QString &word,int &iv2,QString &psfx)
{
    ...
    goto c999;
c10:
    word=qstr_beg_end(word,i,word.size());
    goto c999;
...
c999:
    if (qstr_beg_end(word,0,3)=="3D0") word="3DA0"+qstr_beg_end(word,3,word.size());//word(4:)
    if (qstr_beg_end(word,0,1)=="Q") word="3X"+qstr_beg_end(word,1,word.size());
...
c100: //continue
    ...
    if (qstr_beg_end(msg,0,2)=="E9" &&
            mm_qchar_latin1(msg[2]) >= 'A' && mm_qchar_latin1(msg[2]) <= 'Z' &&
            mm_qchar_latin1(msg[3]) >= 'A' && mm_qchar_latin1(msg[3]) <= 'Z' &&
            msg[4]==' ') msg="CQ "+qstr_beg_end(msg,2,msg.size());
...
c100: //continue
    ...
    //and CQ 0..9 HV  
    if(qstr_beg_end(msg,0,5)=="CQ 00" && mm_qchar_latin1(msg[5]) >= '0' &&  mm_qchar_latin1(msg[5]) <= '9') msg="CQ "+qstr_beg_end(msg,5,msg.size());
