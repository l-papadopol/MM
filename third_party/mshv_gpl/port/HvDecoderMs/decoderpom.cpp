...
static int setup_c2c_d2c_(bool &wait,fftw_plan &p,mshv_complex *a,int nfft,int isign,int iform,double *d = 0)
{
    ...
    if 		(isign==-1 && iform==1) p=fftw_plan_dft_1d(nfft, reinterpret_cast<fftw_complex*>(a), reinterpret_cast<fftw_complex*>(a), FFTW_FORWARD, FFTW_ESTIMATE_PATIENT);
    else if (isign==1 &&  iform==1) p=fftw_plan_dft_1d(nfft, reinterpret_cast<fftw_complex*>(a), reinterpret_cast<fftw_complex*>(a), FFTW_BACKWARD, FFTW_ESTIMATE_PATIENT);
    else if (isign==-1 && iform==0) p=fftw_plan_dft_r2c_1d(nfft, d, reinterpret_cast<fftw_complex*>(a), FFTW_ESTIMATE_PATIENT);
    else if (isign==1 && iform==-1) p=fftw_plan_dft_c2r_1d(nfft, reinterpret_cast<fftw_complex*>(a), d, FFTW_ESTIMATE_PATIENT);
...
bool PomAll::isStandardCall(QString w)//2.61 same as  MultiAnswerModW
{
    ...
    static const QRegularExpression rx(QStringLiteral("^\\s*([A-Z]{0,2}|[A-Z][0-9]|[0-9][A-Z])([0-9][A-Z]{0,3})(/R|/P)?\\s*$"),
                                       QRegularExpression::CaseInsensitiveOption);
    bool res0 = rx.match(w).hasMatch(); //qDebug()<<w<<res0;
    return res0;
}
