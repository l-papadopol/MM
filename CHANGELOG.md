...
- Windows standalone packages now keep `MadModem.exe`, Qt plugins, DLLs, OpenSSL runtime, help, tests and runtime data under `bin/`.
- The Windows package root now exposes a `MadModem.lnk` launcher with the application icon, plus README/RUN notes and license files.
...
- FT QSO activity/history table rows can now be double-clicked to select or arm valid FT transmissions, matching the workflow from the left decode table where possible.
