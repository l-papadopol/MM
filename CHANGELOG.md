## 0.5.76z - Windows standalone DLL closure safety-net

- Keeps the 0.5.76w Linux and macOS packaging unchanged.
- Windows MSYS2/MinGW64 standalone packages now copy an explicit runtime safety-net set from MINGW_PREFIX/bin after windeployqt, including ICU, pcre2, zstd, harfbuzz, md4c, png, libgomp and related MinGW/Qt indirect DLLs.
- Windows packaging still runs recursive objdump dependency closure after the safety-net copy and fails before ZIP creation if any non-system DLL remains unresolved.
- Release validation now rejects Windows ZIPs that lack the Qt indirect runtime families that broke on Wine/Windows.
- No DSP, CAT/PTT, FT8/FT4, audio, MIND, decoder or UI runtime changes.
