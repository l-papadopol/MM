## 0.5.77 translation audit

- Added localized runtime UI strings for Settings -> Appearance / UI.
- Added localized theme names/tooltips for Avionica, Qt Default, Hacker Green, Classic Dark and High Contrast.
- Added localized FT QSO activity/history activation tooltip and warning.
- Languages updated: EN, IT, FR, DE, NO, CS.
- Some non-primary translations remain available/in progress and should be refined by native speakers.

MIND autonomous trainer patch: removed user-facing trainer budget and model Save/Load/Reset controls. New visible strings are limited to MIND Assist and an autonomous-training explanatory label; runtime dictionaries should be refreshed with the normal translation audit script.

# Translation audit

Generated after MIND UI cleanup.

Runtime dictionary key counts remain aligned across the supported languages. This update mainly removes visible labels from the MIND tab and adds only short runtime strings for the compact progress bar/tooltips.

Audit result: no intentionally untranslated user-facing MIND cleanup strings were left outside the runtime dictionary path.


MIND full-speed trainer patch: no new untranslated runtime UI keys required beyond the existing trainer-budget text path.

## 0.5.78-lab3 MIND readiness UI

Added localized strings for the MIND gain readiness indicator:
Learning, Assist, Disabled, MIND learning, MIND assist active, MIND disabled, Profile, Samples, Val, Reason and readiness reasons.
Updated language dictionaries: EN, IT, FR, DE, NO, CS.
