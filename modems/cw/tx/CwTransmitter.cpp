      ...
      m_wpm(qBound(5.0, wpm, 60.0))
{
    ...
    const qint64 maxEdgeForSpeed = qMax<qint64>(qint64{1}, static_cast<qint64>(qRound(0.35 * m_dotSamples)));
    m_edgeSamples = qMax<qint64>(qint64{1}, qMin(preferredEdge, maxEdgeForSpeed));
...
{
    const qint64 samples = qMax<qint64>(qint64{1}, static_cast<qint64>(qRound(dotUnits * m_dotSamples)));
...
double CwTransmitter::shapedEnvelope(const Segment &seg) const
{
    ...
    const qint64 edge = qMax<qint64>(qint64{1}, qMin(m_edgeSamples, seg.samples / 2));
    if (edge <= 1) {
        return 1.0;
    }
    ...
    const qint64 remaining = qMax<qint64>(qint64{0}, seg.samples - m_segmentSampleIndex);
    if (remaining < edge) {
