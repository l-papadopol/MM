...
[[maybe_unused]] static float mean(const float* data, size_t n)
{
  if (n == 0)
    return 0.0f;
...
static float histogram_mean(const int data[], const int begin, const int end, int bin_width)
{
  ...
  for (int idx = begin; idx <= end; ++idx) {
    float bin_centre = (1.0f * static_cast<float>(idx) * static_cast<float>(bin_width)) +
                       (0.5f * static_cast<float>(bin_width));
    sum_data += data[idx] * bin_centre;
    sum_counts += data[idx];
  }
  if (sum_counts <= 0.0f) return 0.0f;
                              ...
                              int bin_width)
{
  ...
  if (sum_counts <= 0.0f) return static_cast<float>(bin_width);
...
[[maybe_unused]] static float stddev(const float* data, size_t n, float mean)
{
  if (n == 0)
    return 0.0f;
...
void c_morse_timing_classifier::update_on_model(const float* d, size_t n)
{
  ...
  dot_mu = std::max(10.0f, histogram_mean(smoothed_histogram, 0, valley_bin, BIN_WIDTH));
  ...
  dash_mu = std::max(dot_mu * 1.5f, histogram_mean(smoothed_histogram, valley_bin, end, BIN_WIDTH));
...
void c_morse_timing_classifier::update_off_model(const float* d, size_t n)
{
  ...
  // cluster dots
  gap1_mu = std::max(10.0f, histogram_mean(smoothed_histogram, 0, valley1_bin, BIN_WIDTH));
  ...
  int end = std::min(peak2 * 2, BIN_MAX / BIN_WIDTH - 1);
  gap3_mu = std::max(gap1_mu * 1.5f, histogram_mean(smoothed_histogram, valley1_bin, end, BIN_WIDTH));
