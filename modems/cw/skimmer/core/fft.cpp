    ...
    for (j = 0; j < span; ++j) {
      ...
      if (j == 0) {
        ...
        for (i = j; i < n; i += subdft_size) {
          ...
          reals[ip] = bottom_real >> 1;
          imaginaries[ip] = bottom_imaginary >> 1;
          reals[i] = top_real >> 1;
          imaginaries[i] = top_imaginary >> 1;
        }
      ...
      // Use full complex multiplier for other cases
        ...
        for (i = j; i < n; i += subdft_size) {
          ...
          temp_real =
              product(bottom_real, real_twiddle) - product(bottom_imaginary, imaginary_twiddle);
          temp_imaginary =
              product(bottom_real, imaginary_twiddle) + product(bottom_imaginary, real_twiddle);
