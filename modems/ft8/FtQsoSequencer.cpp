...
FtQsoSequencer::Decision FtQsoSequencer::evaluateDecode(const Decode &decode, const Context &context)
{
    ...
    if (parsed.rr73 || parsed.rrr || parsed.final73) {
        ...
        if (context.state == State::SendingRr73 && alreadySentFinal73) {
            // A final 73 is already on air (or has just been sent).  Never
            // replace it, never schedule another one, and never create a 73
            // retry loop.  The normal TX-complete transition closes the QSO.
            d.action = context.txActive ? Action::None : Action::Complete;
            d.nextState = context.txActive ? State::SendingRr73 : State::Completed;
            ...
            d.logLine = context.txActive
                ? QStringLiteral("FT sequencer: final acknowledgement received while final 73 is on air; no additional TX scheduled.")
                : QString();
            return d;
        }

        if (waitingForFinal && !alreadySentFinal73) {
            d.action = context.txActive ? Action::InterruptAndArmRow : Action::ArmRow;
            ...
            d.logLine = context.txActive
                ? QStringLiteral("FT sequencer: final acknowledgement received during obsolete TX; stopping it and sending one final 73.")
                : (parsed.rr73
