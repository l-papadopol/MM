                                 ...
                                 double *samplesPerPixelOut)
{
    ...
    const qint64 totalSamples64 = qMax<qint64>(qint64{1}, static_cast<qint64>(qCeil(static_cast<double>(totalPixels) * samplesPerPixel)));
    const int totalSamples = static_cast<int>(qMin<qint64>(totalSamples64, static_cast<qint64>(std::numeric_limits<int>::max())));
    ...
    for (int n = 0; n < totalSamples; ++n) {
        const qint64 pixelIndex = qMin<qint64>(totalPixels - qint64{1},
...
int HellschreiberTransmitter::generate(float *output, int sampleCount)
{
    ...
    const int available = static_cast<int>(qMin<qint64>(static_cast<qint64>(sampleCount), m_totalSamples - m_generatedSamples));
    if (available <= 0) {
        return 0;
    }
