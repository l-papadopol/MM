...
void AppSettings::load()
{
    ...
    uiTheme = settings.value("Display/uiTheme", uiTheme).toString().trimmed().toLower();
    if (uiTheme != "avionica" && uiTheme != "qt_default" && uiTheme != "hacker_green" &&
        uiTheme != "classic_dark" && uiTheme != "high_contrast") {
        uiTheme = "avionica";
    }
    uiUseThemeFont = settings.value("Display/uiUseThemeFont", uiUseThemeFont).toBool();
    uiFontFamily = settings.value("Display/uiFontFamily", uiFontFamily).toString().trimmed();
    uiFontPointSize = qBound(8, settings.value("Display/uiFontPointSize", uiFontPointSize).toInt(), 24);
    decodeTableFontFamily = settings.value("Display/decodeTableFontFamily", decodeTableFontFamily).toString().trimmed();
    decodeTablePreset = settings.value("Display/decodeTablePreset", decodeTablePreset).toString().trimmed().toLower();
    if (decodeTablePreset != "custom" && decodeTablePreset != "compact" && decodeTablePreset != "normal" &&
        decodeTablePreset != "large" && decodeTablePreset != "low_vision") {
        decodeTablePreset = "custom";
    }
    ...
    const int legacyCwWpm = qBound(5, settings.value("CW/wpm", 20).toInt(), 80);
    const bool legacyCwAutoWpm = settings.value("CW/autoWpm", true).toBool();
    cwWpmA = qBound(5, settings.value("CW/wpmA", legacyCwWpm).toInt(), 50);
    cwWpmB = qBound(5, settings.value("CW/wpmB", legacyCwWpm).toInt(), 50);
    cwTxWpm = qBound(5, settings.value("CW/txWpm", legacyCwWpm).toInt(), 80);
    cwBandwidthHz = settings.value("CW/bandwidthHz", cwBandwidthHz).toInt();
    cwAutoBandwidth = settings.value("CW/autoBandwidth", cwAutoBandwidth).toBool();
    ...
    cwAutoWpmA = settings.value("CW/autoWpmA", legacyCwAutoWpm).toBool();
    cwAutoWpmB = settings.value("CW/autoWpmB", legacyCwAutoWpm).toBool();
    ...
    ft8QueueCallers = settings.value("FT8/queueCallers", ft8QueueCallers).toBool();
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        rp.antennaBeamWidthDeg = settings.value(group + QStringLiteral("antennaBeamWidthDeg"), rp.antennaBeamWidthDeg).toDouble();
        if (rp.antennaBeamWidthDeg < 2.0 || rp.antennaBeamWidthDeg > 120.0) rp.antennaBeamWidthDeg = 15.0;
    ...
    textMyCoordinatesEnabled = settings.value("Text/myCoordinatesEnabled", textMyCoordinatesEnabled).toBool();
    textMyLatitudeDeg = settings.value("Text/myLatitudeDeg", textMyLatitudeDeg).toDouble();
    textMyLongitudeDeg = settings.value("Text/myLongitudeDeg", textMyLongitudeDeg).toDouble();
    if (textMyLatitudeDeg < -90.0 || textMyLatitudeDeg > 90.0 ||
        textMyLongitudeDeg < -180.0 || textMyLongitudeDeg > 180.0) {
        textMyCoordinatesEnabled = false;
        textMyLatitudeDeg = 0.0;
        textMyLongitudeDeg = 0.0;
    }
...
bool AppSettings::save() const
{
    ...
    settings.setValue("Display/uiTheme", uiTheme);
    settings.setValue("Display/uiUseThemeFont", uiUseThemeFont);
    settings.setValue("Display/uiFontFamily", uiFontFamily);
    settings.setValue("Display/uiFontPointSize", uiFontPointSize);
    settings.setValue("Display/decodeTableFontFamily", decodeTableFontFamily);
    settings.setValue("Display/decodeTablePreset", decodeTablePreset);
    ...
    settings.setValue("CW/wpmB", cwWpmB);
    settings.setValue("CW/txWpm", cwTxWpm);
    settings.setValue("CW/autoWpmA", cwAutoWpmA);
    settings.setValue("CW/autoWpmB", cwAutoWpmB);
    // Keep the legacy keys synchronized for rollback compatibility.
    settings.setValue("CW/wpm", cwWpmA);
    settings.setValue("CW/autoWpm", cwAutoWpmA);
    settings.setValue("CW/bandwidthHz", cwBandwidthHz);
    settings.setValue("CW/autoBandwidth", cwAutoBandwidth);
    ...
    settings.setValue("FT8/queueCallers", ft8QueueCallers);
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        settings.setValue(group + QStringLiteral("antennaBeamWidthDeg"), rp.antennaBeamWidthDeg);
    ...
    settings.setValue("Text/myCoordinatesEnabled", textMyCoordinatesEnabled);
    settings.setValue("Text/myLatitudeDeg", textMyLatitudeDeg);
    settings.setValue("Text/myLongitudeDeg", textMyLongitudeDeg);
    settings.remove("Text/remoteCallsign");
    settings.remove("Text/remoteName");
    settings.remove("Text/remoteQth");
    settings.remove("Text/report");
