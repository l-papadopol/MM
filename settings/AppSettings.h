...
public:
    ...
    QString ftHighlightCqBackground = "#E8FFE8";
    QString ftHighlightCqForeground = "#145014";
    QString ftHighlightWorkedBackground = "#F0F0F0";
    QString ftHighlightWorkedForeground = "#777777";
    ...
    // Visual FT AutoQSO flow editor JSON.  v4.13b stores and displays this
    // flow, but the live executor is intentionally kept on the stable built-in
    // sequencer until the visual runtime is validated.
    QString ftAutoQsoFlowJson;

    // v4.13g: run the saved AutoQSO flow in read-only shadow mode when
    // Evil AutoQSO is armed.  It only writes diagnostic [Flow][shadow]
    // log lines and never keys PTT/audio/CAT.
    bool ftAutoQsoFlowShadowMode = true;


    // MM Flow Studio / CatRotator integration. The original CatRotator WSJT-X/AirScout
    // tracking inputs are intentionally not exposed here; the rotator follows only
    // MadModem-selected QSO targets, manual commands, or future safe MM Flow actions.
