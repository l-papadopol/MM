...
- Experimental MSK144 RX core split: added `Msk144RxCore` backend boundary so the true WSJT-X/MSHV receive-chain port can be implemented/tested without touching UI/live audio plumbing.
- Settings layout cleanup: Rotator stop/stall editors are compact, the Auto peak algorithm sits beside the rotator profile settings, and User/QTH/Macros is merged into one horizontal split page.
- Rotator settings compact UI: Connection and Settings are side by side; common Band → Rotator table is left-aligned and no longer horizontally stretched.

- UI: extended the Logbook-style triple cockpit border to the main window while keeping the overlay mouse-transparent.
### 0.5.77.experimental
- UI/UX cleanup: logbook floating dialog gets stronger cockpit framing, dark table styling, icon+text toolbar buttons and descriptive tooltips.
- Rotator settings now use one compact common Band → Rotator assignment table instead of wide per-rotator band lists.
- FT decode table visual cleanup: CQ rows are more readable, ordinary non-CQ traffic is calmer, and the new-DXCC dashed outline obeys the setting when disabled.
 field-test patch
- Windows standalone package: replaced runner-generated absolute `.lnk` with portable `MadModem.cmd` launcher targeting `bin\MadModem.exe` with working directory `bin\`.
- Radio Telescope: Alt-Az scan plan now uses true projected honeycomb centers; the rotator points exactly at each visible hexagon center.
- Rotator navball: added display-only low-pass/deadband smoothing to reduce wind/backlash jitter without changing raw control readings.
...
### Radio Telescope UI and heatmap contrast

The experimental Radio Telescope mode now uses a less cramped side-panel layout, explicit Reset map action, keyboard-editable audio bounds, longer practical dwell/settle defaults, and final logarithmic heatmap contrast normalization to make small noise differences visible after a scan.
...
The unified Settings workbench now lazy-loads heavy pages such as Rotator, Scheduler and Soundcard calibration. The log prints open timing diagnostics.
...
### 0.5.77.experimental cleanup

- Removed FT8/FT4/MSK144 bundled auto-tests from the production UI.
- Removed all bundled WAV/JPG test media from the package.
- Removed MM Flow Studio and its visual flow runtime. Built-in FT AutoQSO remains unchanged.
