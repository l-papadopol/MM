# MadModem 0.5.78 architecture

MadModem is a Qt Widgets desktop application with one integrated main window and
mode-specific decoders/workers. Audio capture, transmit generation and decoding
are separated from the GUI through Qt delivery and worker threads where needed.
...
- Q65 uses the optional MSHV-derived full bridge when FFTW3 is available and the
  configured safe fallback otherwise.
